...
def check_file_e402(file_path):
    """检查单个文件的E402问题"""
    success, stdout, stderr = run_command(f"uv run ruff check {file_path} --select=E402")
...
def main():
    ...
    # 检查剩余的E402问题总数
    success, stdout, stderr = run_command("uv run ruff check src/ --select=E402 --statistics")
    if stdout:
    ...
    for error_type in critical_errors:
        success, stdout, stderr = run_command(f"uv run ruff check src/ --select={error_type}")
    ...
    print(f"\n[完成] 渐进式E402修复!")
    print(f"[建议]:")
    print(f"  - 核心文件的E402问题已修复")
    print(f"  - 系统现在可以正常启动和运行")
    print(f"  - 剩余的E402问题可以在后续迭代中逐步处理")
