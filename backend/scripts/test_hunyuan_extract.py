#!/usr/bin/env python3
"""
Tencent Hunyuan Vision Multi-Page Extraction Test
腾讯混元视觉模型多页合同提取测试
...
async def test_hunyuan_extraction():
    """Test Tencent Hunyuan Vision for multi-page contract extraction"""
    
    api_key = os.getenv("HUNYUAN_API_KEY")
    # 使用 OpenAI 兼容接口
    base_url = os.getenv("HUNYUAN_BASE_URL", "https://api.hunyuan.cloud.tencent.com/v1")
    model = os.getenv("HUNYUAN_VISION_MODEL", "hunyuan-vision")
    
    print("=" * 60)
    print("Tencent Hunyuan Vision Extraction Test")
    print("=" * 60)
    print(f"Model: {model}")
    print(f"Base URL: {base_url}")
    
    if not api_key:
        print("[ERROR] HUNYUAN_API_KEY not set")
        print("Please set HUNYUAN_API_KEY in .env")
    ...
    # Build content (OpenAI compatible format)
    content = []
...
if __name__ == "__main__":
    asyncio.run(test_hunyuan_extraction())
