...
def create_system_dictionaries_table(conn):
    ...
    # 检查表是否已存在
    cursor.execute(
        "SELECT name FROM sqlite_master WHERE type='table' AND name='system_dictionaries'"
    )
    if cursor.fetchone():
...
def create_custom_fields_table(conn):
    ...
    # 检查表是否已存在
    cursor.execute(
        "SELECT name FROM sqlite_master WHERE type='table' AND name='asset_custom_fields'"
    )
    if cursor.fetchone():
    ...
    # 创建索引
    cursor.execute(
        "CREATE INDEX idx_custom_fields_asset ON asset_custom_fields(asset_id)"
    )
    cursor.execute(
        "CREATE INDEX idx_custom_fields_name ON asset_custom_fields(field_name)"
    )
    cursor.execute(
        "CREATE INDEX idx_custom_fields_type ON asset_custom_fields(field_type)"
    )
...
def migrate_asset_data(conn):
    ...
    for asset_id, is_litigated in assets:
        ...
        cursor.execute(
            "UPDATE assets SET is_litigated = ? WHERE id = ?", (boolean_value, asset_id)
        )
...
def add_audit_fields_to_history(conn):
    ...
    for field_name, field_type in audit_fields:
        try:
            cursor.execute(
                f"ALTER TABLE asset_history ADD COLUMN {field_name} {field_type}"
            )
...
def create_performance_indexes(conn):
    ...
    indexes = [
        ...
        # 复合索引
        (
            "idx_assets_ownership_nature_combo",
            "assets",
...
def validate_migration(conn):
    ...
    # 检查触发器
    cursor.execute(
        "SELECT name FROM sqlite_master WHERE type='trigger' AND name LIKE 'tr_%'"
    )
