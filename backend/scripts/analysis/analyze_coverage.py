...
for file_path, file_data in data['files'].items():
    if 'security' in file_path.lower() or 'token_blacklist' in file_path or 'logging_security' in file_path:
...
for file_path, info in sorted(security_files.items()):
    ...
    print(f'{filename}:')
    print(f'  Coverage: {info["coverage"]} ({info["missing_lines"]} of {info["num_statements"]} lines missing)')
    if info['missing_lines'] > 0:
        ...
        for line in missing[1:]:
            if line == end + 1:
                ...
                start = end = line
        ranges.append(f'{start}-{end}' if start != end else str(start))
        print(f'  Missing lines: {", ".join(ranges[:10])}' + ('...' if len(ranges) > 10 else ''))
    print()
...
if security_file in security_files and security_files[security_file]['missing_lines'] > 0:
