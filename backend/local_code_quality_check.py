...
class CodeQualityChecker:
    ...
    def check_ruff_issues(self):
        ...
        for error_code, description in critical_checks:
            ...
            success, stdout, stderr = run_command(f"uv run ruff check src/ --select={error_code} --statistics")
    ...
    def check_mypy_types(self):
        ...
        for file_path in core_files:
            if Path(file_path).exists():
                print(f"\n[检查] {file_path}")
                success, stdout, stderr = run_command(f"uv run mypy {file_path} --ignore-missing-imports --no-error-summary")
    ...
    def check_import_organization(self):
        ...
        # 检查是否有未使用的导入
        success, stdout, stderr = run_command("uv run ruff check src/ --select=F401 --statistics")
    ...
    def check_complexity(self):
        ...
        for rule in complexity_rules:
            success, stdout, stderr = run_command(f"uv run ruff check src/ --select={rule}")
        ...
        if total_issues == 0:
            ...
            self.results["complexity"] = {"status": "COMPLEXITY_ISSUES", "count": total_issues}
    ...
    def generate_report(self):
        ...
        if "ruff" in self.results:
            ...
            total_errors = sum(result.get("count", 0) for result in ruff_results.values())
            if total_errors == 0:
        ...
        if "format" in self.results:
            ...
            summaries.append("[格式] 代码格式: 通过 ✅" if format_status == "PASS" else "[格式] 代码格式: 需要修复 ⚠️")

        # 类型检查
        if "mypy" in self.results:
            mypy_status = self.results["mypy"]["status"]
            summaries.append("[类型] MyPy检查: 通过 ✅" if mypy_status == "PASS" else "[类型] MyPy检查: 有问题 ⚠️")

        # 导入检查
        if "imports" in self.results:
            unused_count = self.results["imports"]["unused_count"]
            summaries.append(f"[导入] 未使用导入: {unused_count} 个" + (" ✅" if unused_count == 0 else " ⚠️"))

        # 复杂度检查
        if "complexity" in self.results:
            complexity_count = self.results["complexity"]["count"]
            summaries.append(f"[复杂度] 代码复杂度: {complexity_count} 个问题" + (" ✅" if complexity_count == 0 else " ⚠️"))
        ...
        critical_issues = 0
        if "ruff" in self.results:
            critical_issues += sum(result.get("count", 0) for result in self.results["ruff"].values())
