...
class AssetFactory(factory.Factory):
    ...
    ownership_category = factory.Faker(
        "random_element", elements=["国有企业", "民营企业", "外资企业", "合资企业"]
    )
    project_name = factory.Faker("sentence", nb_words=3)
    property_name = factory.Faker("sentence", nb_words=2)
    address = factory.Faker("address")
    ownership_status = factory.Faker(
        "random_element", elements=["已确权", "待确权", "确权中"]
    )
    property_nature = factory.Faker(
        "random_element", elements=["商业用途", "办公用途", "工业用途", "住宅用途"]
    )
    usage_status = factory.Faker(
        "random_element", elements=["使用中", "空置", "装修中", "待租"]
    )
    business_category = factory.Faker(
        "random_element", elements=["零售商业", "办公服务", "餐饮服务", "教育培训"]
    )
    ...
    # 用途相关字段 (2字段)
    certificated_usage = factory.Faker(
        "random_element", elements=["商业用地", "办公用地", "工业用地", "住宅用地"]
    )
    actual_usage = factory.Faker(
        "random_element", elements=["零售商场", "办公楼", "工厂", "住宅"]
    )

    # 租户相关字段 (2字段)
    tenant_name = factory.Faker("company")
    tenant_type = factory.Faker(
        "random_element", elements=["企业", "个人", "事业单位", "社会组织"]
    )

    # 合同相关字段 (8字段)
    lease_contract_number = factory.Sequence(
        lambda n: f"LC{datetime.now().year}{n:04d}"
    )
    contract_start_date = factory.LazyAttribute(
        lambda obj: date.today()
        - timedelta(days=factory.random_random.Random().randint(30, 365))
    )
    contract_end_date = factory.LazyAttribute(
        lambda obj: obj.contract_start_date
        + timedelta(days=factory.random_random.Random().randint(180, 1095))
    )
    ...
    # 管理相关字段 (3字段)
    business_model = factory.Faker(
        "random_element", elements=["自营", "委托经营", "租赁经营", "合作经营"]
    )
    operation_status = factory.Faker(
        "random_element", elements=["正常营业", "装修中", "暂停营业", "已关闭"]
    )
    manager_name = factory.Faker("name")

    # 接收相关字段 (3字段)
    operation_agreement_start_date = factory.LazyAttribute(
        lambda obj: obj.contract_start_date - timedelta(days=30)
    )
    operation_agreement_end_date = factory.LazyAttribute(
        lambda obj: obj.contract_end_date + timedelta(days=365)
    )
    ...
    # 项目相关字段
    project_phase = factory.Faker(
        "random_element", elements=["一期", "二期", "三期", "整体"]
    )
    ...
    def _create(cls, model_class, *args, **kwargs):
        ...
        if "contract_start_date" in kwargs and "contract_end_date" in kwargs:
            if kwargs["contract_end_date"] <= kwargs["contract_start_date"]:
                kwargs["contract_end_date"] = kwargs["contract_start_date"] + timedelta(
                    days=365
                )
...
class RoleFactory(factory.Factory):
    ...
    organization_id = "test_org_001"
    scope = factory.Faker(
        "random_element", elements=["organization", "department", "team"]
    )
...
class PermissionFactory(factory.Factory):
    ...
    resource = factory.Faker(
        "random_element", elements=["assets", "users", "roles", "reports"]
    )
    action = factory.Faker(
        "random_element", elements=["create", "read", "update", "delete", "admin"]
    )
...
def create_asset_with_contract(contract_length_months: int = 12, **kwargs) -> Asset:
    ...
    return AssetFactory.create(
        contract_start_date=start_date, contract_end_date=end_date, **kwargs
    )
