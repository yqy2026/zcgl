"""Regression tests for the user Party-scope batch receipt migration."""
...
def _load_migration_module() -> ModuleType:
    module_path = (
        ...
        / "alembic"
        / "versions"
        / "20260806_user_party_scope_batch_commit_receipts.py"
    )
    spec = spec_from_file_location(
        "user_party_scope_batch_receipts_migration",
        module_path,
    )
...
def test_migration_follows_organization_party_scope_batch_receipts() -> None:
...
def test_upgrade_creates_durable_user_party_scope_batch_receipt_table(
    ...
    assert created_tables == ["user_party_scope_batch_commits"]
...
def test_downgrade_removes_user_party_scope_batch_receipt_table(
    monkeypatch,
) -> None:
    module = _load_migration_module()
    ...
    assert dropped_tables == ["user_party_scope_batch_commits"]
