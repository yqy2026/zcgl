...
def setup_mock_db_counts(mock_counter, counts):
    mock_counter.side_effect = counts
    return mock_counter
...
class TestSecurityEventLoggerBasics:
    ...
    async def test_severity_enum(self):
...
class TestAuthFailureLogging:
    ...
    async def test_log_auth_failure_with_ipv6(self):
        ...
        with patch.object(logger, "_log_to_database", new_callable=AsyncMock) as mock_db_log:
            ...
            result = await logger.log_auth_failure("::ffff:192.168.1.1", "testuser")
...
class TestThresholdChecking:
    ...
    async def test_should_alert_below_threshold(self, mock_sessionlocal):
        ...
        should_alert = await logger.should_alert("192.168.1.1")
    ...
    async def test_should_alert_custom_threshold(self, mock_sessionlocal):
        ...
        should_alert = await logger.should_alert("192.168.1.1", threshold=10)
...
class TestEventCounting:
    ...
    async def test_get_event_count_different_types(self, mock_sessionlocal):
        ...
        count2 = await logger.get_event_count(
...
class TestAllEventTypes:
    ...
    async def test_log_auth_success(self):
        ...
        with patch.object(logger, "_log_to_database", new_callable=AsyncMock) as mock_db_log:
            ...
            result = await logger.log_auth_success("192.168.1.1", "testuser")
    ...
    async def test_log_suspicious_activity(self):
        ...
        with patch.object(logger, "_log_to_database", new_callable=AsyncMock) as mock_db_log:
            ...
            result = await logger.log_suspicious_activity(
    ...
    async def test_log_account_locked(self):
        ...
        with patch.object(logger, "_log_to_database", new_callable=AsyncMock) as mock_db_log:
            ...
            result = await logger.log_account_locked(
...
class TestSeverityLevels:
    ...
    async def test_auth_failure_severity(self):
    ...
    async def test_account_locked_severity(self):
...
class TestErrorHandling:
    ...
    async def test_invalid_ip_address(self):
        ...
        with patch.object(logger, "_log_to_database", new_callable=AsyncMock) as mock_db_log:
            ...
            await logger.log_auth_failure("", "testuser")
            await logger.log_auth_failure(None, "testuser")  # type: ignore
...
class TestIntegrationScenarios:
    ...
    async def test_multiple_auth_failures_trigger_alert(self, mock_sessionlocal):
        ...
        with patch.object(logger, "_log_to_database", new_callable=AsyncMock) as mock_db_log:
            ...
            # Log 5 auth failures
            for i in range(5):
                await logger.log_auth_failure("192.168.1.1", "testuser")
    ...
    async def test_different_ips_separate_tracking(self, mock_sessionlocal):
        ...
        with patch.object(logger, "_log_to_database", new_callable=AsyncMock) as mock_db_log:
            ...
            for _ in range(2):
                await logger.log_auth_failure("192.168.1.2", "testuser")

            # First IP should trigger alert
            assert await logger.should_alert("192.168.1.1") is True
    ...
    async def test_mixed_event_types_separate_counts(self, mock_sessionlocal):
        ...
        with patch.object(logger, "_log_to_database", new_callable=AsyncMock) as mock_db_log:
            ...
            assert await logger.should_alert("192.168.1.1") is False
...
class TestMethodSignatures:
    ...
    async def test_log_auth_failure_signature(self):
    ...
    async def test_log_permission_denied_signature(self):
    ...
    async def test_log_rate_limit_exceeded_signature(self):
    ...
    async def test_methods_return_security_event(self):
        ...
        with patch.object(logger, "_log_to_database", new_callable=AsyncMock) as mock_db_log:
            ...
            result = await logger.log_auth_failure(ip="192.168.1.1")
