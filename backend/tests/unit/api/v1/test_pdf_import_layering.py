...
def test_pdf_import_wrapper_include_routes_should_attach_require_authz() -> None:
    ...
    module_source = _read_module_source()
    expected_patterns = [
        r"router\.include_router\([\s\S]*?pdf_upload\.router[\s\S]*?dependencies=\[\s*Depends\(\s*require_authz\([\s\S]*?action=\"create\"[\s\S]*?resource_type=\"contract\"[\s\S]*?resource_context=_CONTRACT_CREATE_RESOURCE_CONTEXT",
        r"router\.include_router\([\s\S]*?pdf_sessions_module\.router[\s\S]*?dependencies=\[\s*Depends\(\s*require_authz\([\s\S]*?action=\"read\"[\s\S]*?resource_type=\"contract\"",
        r"router\.include_router\([\s\S]*?pdf_system\.router[\s\S]*?dependencies=\[\s*Depends\(\s*require_authz\([\s\S]*?action=\"read\"[\s\S]*?resource_type=\"contract\"",
    ]
