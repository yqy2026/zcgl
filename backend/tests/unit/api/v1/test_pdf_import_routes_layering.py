...
def test_pdf_import_routes_key_endpoints_should_use_require_authz() -> None:
    ...
    expected_patterns = [
        r"def get_pdf_import_info[\s\S]*?require_authz\([\s\S]*?action=\"read\"[\s\S]*?resource_type=\"contract\"",
        r"def get_pdf_import_sessions[\s\S]*?require_authz\([\s\S]*?action=\"read\"[\s\S]*?resource_type=\"contract\"",
        r"def upload_pdf_for_import[\s\S]*?require_authz\([\s\S]*?action=\"create\"[\s\S]*?resource_type=\"contract\"[\s\S]*?resource_context=_CONTRACT_CREATE_RESOURCE_CONTEXT",
    ]
