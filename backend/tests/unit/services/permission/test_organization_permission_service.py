...
async def test_accessible_organizations_include_user_organization_id(
    ...
    monkeypatch.setattr(organization_permission_service.cache_manager, "get", MagicMock(return_value=None))
    monkeypatch.setattr(organization_permission_service.cache_manager, "set", MagicMock())
