...
def upgrade():
    ...
    op.create_table(
        ...
        sa.Column(
            "event_type", sa.String(length=50), nullable=False, comment="Event type"
        ),
        sa.Column(
            "severity", sa.String(length=20), nullable=False, comment="Severity level"
        ),
        sa.Column(
            "user_id", sa.String(), nullable=True, comment="User ID if applicable"
        ),
        sa.Column(
            "ip_address",
            sa.String(length=45),
        ...
        sa.Column(
            "metadata", sa.JSON(), nullable=True, comment="Event metadata (JSON)"
        ),
        sa.Column(
...
def downgrade():
    # Drop indexes
    op.drop_index(
        "ix_security_events_severity_created_at", table_name="security_events"
    )
    ...
    op.drop_index(
        "ix_security_events_event_type_created_at", table_name="security_events"
    )
