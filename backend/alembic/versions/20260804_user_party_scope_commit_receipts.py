"""Add durable receipts and permission for user Party-scope commits."""
...
revision: str = "20260804_user_party_scope_commit_receipts"
down_revision: str | Sequence[str] | None = (
    "20260804_organization_party_scope_commit_receipts"
)
branch_labels: str | Sequence[str] | None = None
depends_on: str | Sequence[str] | None = None

PERMISSION_NAME = "user:manage_party_scope"
...
def _seed_permission(bind: sa.engine.Connection) -> str:
    ...
    bind.execute(
        ...
        ),
        {
            "id": permission_id,
            "name": PERMISSION_NAME,
            "display_name": "管理用户主体范围",
            "description": "预览并提交用户显式主体范围变更",
            "resource": "user",
...
def upgrade() -> None:
    ...
    op.create_index(
        "ix_user_party_scope_commits_user_id",
        "user_party_scope_commits",
        ["user_id"],
    )
...
def downgrade() -> None:
    ...
    op.drop_index(
        "ix_user_party_scope_commits_user_id",
        table_name="user_party_scope_commits",
    )
    op.drop_table("user_party_scope_commits")
