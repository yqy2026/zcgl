...
def upgrade():
    ...
    op.create_table(
        ...
        sa.Column(
            "monthly_rent_base", sa.Numeric(precision=15, scale=2), nullable=True
        ),
    ...
    op.create_table(
        ...
        sa.Column(
            "total_monthly_amount", sa.Numeric(precision=15, scale=2), nullable=True
        ),
        sa.Column("created_at", sa.DateTime(), nullable=True),
        sa.Column("updated_at", sa.DateTime(), nullable=True),
        sa.ForeignKeyConstraint(
    ...
    op.create_index(
        "idx_rent_contracts_ownership_id", "rent_contracts", ["ownership_id"]
    )
    op.create_index(
        "idx_rent_contracts_contract_number", "rent_contracts", ["contract_number"]
    )
    ...
    op.create_index(
        "idx_rent_contract_history_contract_id",
        "rent_contract_history",
        ["contract_id"],
    )
...
def downgrade():
    # 删除索引
    op.drop_index(
        "idx_rent_contract_history_contract_id", table_name="rent_contract_history"
    )
