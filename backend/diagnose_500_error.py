    ...
    try:
        ...
        try:
            result = await get_enum_field_types(
                skip=0, limit=5, category=None, status=None,
                is_system=None, keyword=None, db=db
            )
        ...
        if response.status_code == 200:
            ...
            if data:
                print(f"   第一个记录: {data[0].get('name', 'N/A')} ({data[0].get('code', 'N/A')})")
        else:
