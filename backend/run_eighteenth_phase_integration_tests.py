...
class EighteenthPhaseIntegrationTestRunner:
    ...
    def _print_test_summary(self, result_data: Dict[str, Any]):
        ...
        if "total_tests" in result_data and result_data["total_tests"] > 0:
            print(f"测试结果: {result_data['passed_tests']}/{result_data['total_tests']} 通过")
    ...
    def _generate_comprehensive_report(self, total_execution_time: float, end_time: datetime) -> Dict[str, Any]:
        """生成综合测试报告"""
        print(f"\n{'='*80}")
        ...
        overall_test_pass_rate = (total_passed / total_tests) * 100 if total_tests > 0 else 0
        ...
        if all_response_times:
            performance_summary["avg_response_time"] = sum(all_response_times) / len(all_response_times)
            performance_summary["max_response_time"] = max(all_response_times)
            performance_summary["min_response_time"] = min(all_response_times)

        if all_throughputs:
            performance_summary["avg_throughput"] = sum(all_throughputs) / len(all_throughputs)
        ...
        if all_protection_rates:
            security_summary["avg_protection_rate"] = sum(all_protection_rates) / len(all_protection_rates)

        if all_blocking_rates:
            security_summary["avg_blocking_rate"] = sum(all_blocking_rates) / len(all_blocking_rates)

        # 打印综合报告
        print(f"执行时间: {self.start_time.strftime('%Y-%m-%d %H:%M:%S')} - {end_time.strftime('%Y-%m-%d %H:%M:%S')}")
