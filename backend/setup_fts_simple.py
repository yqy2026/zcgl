...
def setup_fts_tables():
    ...
    try:
        ...
        try:
            ...
            print(f"Current records: {asset_count:,} assets, {project_count:,} projects, {ownership_count:,} ownerships")
...
def test_fts_performance():
    ...
    try:
        ...
        try:
            ...
            """)).fetchall()
            search_time = (datetime.now() - start_time).total_seconds()
            print(f"Advanced ranked search: {search_time:.3f}s ({len(results)} results)")
