...
logging.basicConfig(
    ...
    handlers=[logging.FileHandler("pdf_tests.log"), logging.StreamHandler()],
)
    ...
    async def test_ml_extractor(self):
        ...
        try:
            ...
            test_data = {
                ...
                "confidence_scores": {"text_extraction": 0.9, "field_detection": 0.8},
            }
            ...
            if result and result.get("contract_info"):
                ...
                self.log_test(
                    ...
                    f"验证分数: {validation.get('validation_score', 0):.2f}"
                    if validation
                    else "无验证结果",
                )
    ...
    async def test_field_mapper(self):
        ...
        try:
            ...
            self.log_test(
                "智能映射",
                "PASS" if result else "FAIL",
                f"映射置信度: {result.get('mapping_confidence', 0):.2f}"
                if result
                else "无映射结果",
            )
            ...
            if result and result.get("mapped_fields"):
                ...
                self.log_test(
                    "数据验证",
                    "PASS" if validation else "FAIL",
                    f"验证状态: {'通过' if validation.get('is_valid') else '失败'}"
                    if validation
                    else "无验证结果",
                )
    ...
    async def test_parallel_processor(self):
        ...
        try:
            ...
            self.log_test(
                ...
                f"最大工作线程: {queue_status.get('max_workers', 0)}"
                if queue_status
                else "无队列信息",
            )
        ...
        for test_name, test_func in test_suites:
            try:
                ...
                results["details"].append(
                    {"name": test_name, "status": status, "timestamp": time.time()}
                )
