...
def check_and_fix_issues():
    ...
    for file_path in critical_files:
        if Path(file_path).exists():
            try:
                # 尝试导入检查
                module_path = (
                    file_path.replace("/", ".").replace(".py", "").replace("src.", "")
                )
    ...
    for py_file in Path("src").rglob("*.py"):
        ...
        try:
            ...
            # 修复常见的类型问题
            content = re.sub(
                r"List\[.*?\].*?\]PDFImportSession\]", "List[PDFImportSession]", content
            )
    ...
    # 4. 检查数据库连接
    print("\n4. 检查数据库配置...")
    db_files = ["src/core/database.py", "src/models/__init__.py"]
    ...
    cache_dirs = ["__pycache__", ".pytest_cache", "*.pyc"]
    ...
    success_rate = (
        len(fixes_applied) / (len(fixes_applied) + len(issues_found)) * 100
        if fixes_applied or issues_found
        else 100
    )
