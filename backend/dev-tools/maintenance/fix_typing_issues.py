...
def fix_typing_imports(file_path: Path):
    ...
    # 3. 修复函数返回类型
    content = re.sub(
        r"-> List\[.*?\]:", lambda m: m.group(0).replace("List[", "list["), content
    )
    content = re.sub(
        r"-> Dict\[.*?\]:", lambda m: m.group(0).replace("Dict[", "dict["), content
    )
    ...
    if (
        "dict[str, Any]" in content or "list[Any]" in content
...
def fix_undefined_names(file_path: Path):
    ...
    # 添加常见未定义异常的导入
    if (
        "BusinessLogicError" in content
        and "BusinessLogicError" not in content.split("from")[0]
    ):
