...
def extract_imports(content: str) -> list[tuple[str, int]]:
    ...
    for i, line in enumerate(lines):
        ...
        if (
            ...
            or stripped.startswith("# 添加导入")
            or stripped.startswith("# 导入")
        ):
...
def separate_imports(
    ...
    for line, _ in imports:
        ...
        if stripped.startswith("#") and not (
            "导入" in stripped or "import" in stripped
        ):
            continue
...
def organize_imports(
    stdlib_imports: list[str], third_party_imports: list[str], local_imports: list[str]
) -> list[str]:
    """组织导入语句的顺序"""
    organized_imports = []
    ...
    if third_party_imports:
        ...
        organized_imports.extend(
            sorted(set(third_party_imports), key=lambda x: x.strip())
        )
        organized_imports.append("")
...
def fix_file_imports(file_path: Path) -> int:
    ...
    try:
        ...
        # 组织导入语句
        organized_imports = organize_imports(
