    ...
    for file_path, line_number in files_to_fix:
        ...
        try:
            ...
            for i, line in enumerate(lines):
                if (
                    "from ..exceptions import BusinessLogicError" in line
                    and i + 1 != line_number
                ):
                    # 确保这是我们要删除的行
                    if i + 1 == line_number:
                        ...
                        print(
                            f"修复 {file_path} 第{line_number}行: 移除重复的BusinessLogicError导入"
                        )
                        total_fixes += 1
                        break
    ...
    try:
        ...
        patterns = [
            (
                ...
                "移除重复的validate_asset_data定义",
            ),
            (
                r"async def get_ownership_entities\([^:]*?\):.*?(?=\nasync def|\n@|\nclass|\n#|\Z)",
                "移除重复的get_ownership_entities定义",
            ),
            (
                r"async def get_business_categories\([^:]*?\):.*?(?=\nasync def|\n@|\nclass|\n#|\Z)",
                "移除重复的get_business_categories定义",
            ),
        ]
