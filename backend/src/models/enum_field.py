...
class EnumFieldType(Base):
    ...
    is_hierarchical = Column(Boolean, nullable=False, default=False, comment="是否层级结构")
    ...
    # 关系定义
    enum_values = relationship("EnumFieldValue", back_populates="enum_type", cascade="all, delete-orphan")
...
class EnumFieldValue(Base):
    ...
    # 关联信息
    enum_type_id = Column(String, ForeignKey("enum_field_types.id"), nullable=False, comment="枚举类型ID")
    ...
    # 层级信息
    parent_id = Column(String, ForeignKey("enum_field_values.id"), comment="父级枚举值ID")
    ...
    children = relationship("EnumFieldValue", back_populates="parent", cascade="all, delete-orphan")
...
class EnumFieldHistory(Base):
    ...
    # 关联信息
    enum_type_id = Column(String, ForeignKey("enum_field_types.id"), comment="枚举类型ID")
    enum_value_id = Column(String, ForeignKey("enum_field_values.id"), comment="枚举值ID")
