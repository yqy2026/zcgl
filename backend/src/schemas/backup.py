...
class BackupInfo(BaseModel):
    ...
    original_size: Optional[int] = Field(None, description="原始大小（压缩文件）")
...
class BackupResponse(BaseModel):
    ...
    backup_info: Optional[BackupInfo] = Field(None, description="备份信息")
    async_backup: bool = Field(False, description="是否异步备份")
...
class BackupInfoResponse(BaseModel):
    ...
    success: bool = Field(..., description="操作是否成功")
    message: str = Field(..., description="响应消息")
    info: Optional[BackupInfo] = Field(None, description="备份详细信息")
...
class RestoreResponse(BaseModel):
    ...
    safety_backup: Optional[str] = Field(None, description="安全备份文件路径")
...
class SchedulerStatus(BaseModel):
    ...
    last_backup_time: Optional[str] = Field(None, description="上次备份时间")
    auto_backup_enabled: bool = Field(..., description="是否启用自动备份")
