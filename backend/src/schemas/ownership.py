...
class OwnershipBase(BaseModel):
    ...
    code: Optional[str] = Field(None, title="权属方编码", min_length=1, max_length=100)
    ...
    def validate_code(cls, v):
        ...
        if v is not None:
            ...
            if not re.match(pattern, v):
                raise ValueError('权属方编码格式必须为: [2字母前缀][4位年月][3位序号]，例如: OW2501001')
            return v.upper()
        return v
...
class OwnershipUpdate(BaseModel):
    """更新权属方模式"""
    name: Optional[str] = Field(None, title="权属方全称", min_length=1, max_length=200)
    code: Optional[str] = Field(None, title="权属方编码", min_length=1, max_length=100)
    short_name: Optional[str] = Field(None, title="权属方简称", max_length=100)
...
class OwnershipDeleteResponse(BaseModel):
    ...
    message: str
    id: str
    affected_assets: Optional[int] = Field(None, title="受影响的资产数量")


class OwnershipSearchRequest(BaseModel):
    """权属方搜索请求模式"""
    keyword: Optional[str] = Field(None, title="搜索关键词")
    is_active: Optional[bool] = Field(None, title="状态")
