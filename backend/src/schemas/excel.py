...
class ExcelImportStatus(BaseModel):
    ...
    status: ImportStatus = Field(..., description="导入状�?)
    progress: float = Field(0.0, description="导入进度�?-100�?)
    success: int = Field(0, description="成功导入的记录数")
    failed: int = Field(0, description="导入失败的记录数")
    total: int = Field(0, description="总记录数")
    errors: List[str] = Field(default_factory=list, description="错误信息列表")
    started_at: Optional[str] = Field(None, description="开始时�?)
    completed_at: Optional[str] = Field(None, description="完成时间")
    message: str = Field("", description="状态消�?)
...
class ExcelValidationResponse(BaseModel):
    ...
    total_rows: int = Field(..., description="总行�?)
    total_columns: int = Field(..., description="总列�?)
...
class ExcelExportRequest(BaseModel):
    ...
    filters: Optional[dict] = Field(None, description="筛选条�?)
    columns: Optional[List[str]] = Field(None, description="要导出的�?)
...
class ExcelExportResponse(BaseModel):
    ...
    file_name: str = Field(..., description="文件�?)
    file_size: int = Field(..., description="文件大小（字节）")
    total_records: int = Field(..., description="导出记录�?)
...
class ImportTemplateInfo(BaseModel):
    ...
    required_columns: List[str] = Field(..., description="必填�?)
...
class DataMappingRule(BaseModel):
    ...
    default_value: Optional[str] = Field(None, description="默认�?)
    validation_rules: List[str] = Field(default_factory=list, description="验证规则")
    transformation: Optional[str] = Field(None, description="数据转换规则")
...
class ImportConfiguration(BaseModel):
    ...
    start_row: int = Field(1, description="开始行号（�?开始）")
    ...
    batch_size: int = Field(1000, description="批处理大�?)
    mapping_rules: List[DataMappingRule] = Field(default_factory=list, description="字段映射规则")
    validation_enabled: bool = Field(True, description="是否启用数据验证")
    duplicate_handling: str = Field("skip", description="重复数据处理方式", pattern="^(skip|update|error)$")
...
class ImportPreviewResponse(BaseModel):
    ...
    column_mapping: dict = Field(..., description="列映射关�?)
