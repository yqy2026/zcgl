...
@router.get("/", response_model=List[SystemDictionaryResponse], summary="获取系统字典列表")
...
@router.get("/{dictionary_id}", response_model=SystemDictionaryResponse, summary="获取系统字典详情")
...
@router.post("/", response_model=SystemDictionaryResponse, summary="创建系统字典", status_code=201)
async def create_system_dictionary(
    ...
    try:
        ...
        existing = system_dictionary_crud.get_by_type_and_code(
            ...
            dict_code=dictionary_in.dict_code
        )
        if existing:
            raise HTTPException(
                status_code=400, 
...
@router.put("/{dictionary_id}", response_model=SystemDictionaryResponse, summary="更新系统字典")
async def update_system_dictionary(
    ...
    try:
        ...
        # 如果更新了类型或代码，检查是否重复
        if (dictionary_in.dict_type and dictionary_in.dict_type != dictionary.dict_type) or \
           (dictionary_in.dict_code and dictionary_in.dict_code != dictionary.dict_code):
            ...
            existing = system_dictionary_crud.get_by_type_and_code(db=db, dict_type=dict_type, dict_code=dict_code)
            if existing and existing.id != dictionary_id:
                raise HTTPException(
                    status_code=400, 
                    detail=f"类型 {dict_type} 中已存在代码 {dict_code}"
                )
        
        updated_dictionary = system_dictionary_crud.update(db=db, db_obj=dictionary, obj_in=dictionary_in)
...
@router.post("/batch-update", response_model=List[SystemDictionaryResponse], summary="批量更新系统字典")
