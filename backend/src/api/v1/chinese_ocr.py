...
from ...services.chinese_ocr_service import chinese_ocr_service, CHINESE_OCR_CONFIG
    ...
    try:
        ...
        try:
            ...
            if file_extension_with_dot == '.pdf':
                # PDF文件处理
                result = chinese_ocr_service.extract_chinese_text_from_pdf(temp_file_path)
            else:
                # 图像文件处理
                import numpy as np
                ...
                # 图像预处理
                processed_image = chinese_ocr_service._preprocess_for_chinese_ocr(image_array)

                # 文本提取
                result = chinese_ocr_service._extract_chinese_text_from_image(processed_image)
    ...
    try:
        features = {
            ...
            "image_preprocessing": {
                ...
                "enabled": chinese_ocr_service.preprocessing_config.get('image_enhancement', False)
            },
            "adaptive_threshold": {
                "name": "自适应阈值",
                "description": "使用自适应阈值处理提升扫描件质量",
                "enabled": chinese_ocr_service.preprocessing_config.get('adaptive_threshold', False)
            },
            "noise_reduction": {
                "name": "降噪处理",
                "description": "应用形态学操作减少图像噪声",
                "enabled": chinese_ocr_service.preprocessing_config.get('noise_reduction', False)
            },
