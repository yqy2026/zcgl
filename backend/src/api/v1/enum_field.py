...
from ...crud.enum_field import get_enum_field_type_crud, get_enum_field_value_crud, get_enum_field_usage_crud
from ...schemas.enum_field import (
    EnumFieldTypeCreate, EnumFieldTypeUpdate, EnumFieldTypeResponse,
    EnumFieldValueCreate, EnumFieldValueUpdate, EnumFieldValueResponse,
    EnumFieldUsageCreate, EnumFieldUsageUpdate, EnumFieldUsageResponse,
    EnumFieldTree, EnumFieldStatistics, EnumFieldBatchCreate, EnumFieldBatchUpdate,
    EnumFieldHistoryResponse
)
from ...models.enum_field import EnumFieldHistory, EnumFieldValue, EnumFieldUsage
...
async def get_enum_field_statistics(db: Session = Depends(get_db)):
    ...
    # 获取枚举值统计
    total_values = db.query(EnumFieldValue).filter(EnumFieldValue.is_deleted == False).count()
    active_values = db.query(EnumFieldValue).filter(
        and_(
    ...
    # 获取使用统计
    usage_count = db.query(EnumFieldUsage).filter(EnumFieldUsage.is_active == True).count()
...
async def get_enum_field_values(
    type_id: str,
    parent_id: Optional[str] = Query(None, description="父级枚举值ID"),
...
async def get_enum_field_values_tree(
    ...
    def build_tree_response(values: List) -> List[EnumFieldTree]:
        tree = []
        for value in values:
...
async def get_enum_field_usage(
    enum_type_id: Optional[str] = Query(None, description="枚举类型ID"),
...
async def create_enum_field_usage(
    ...
    if existing:
        raise HTTPException(status_code=400, detail=f"表 {usage.table_name} 的字段 {usage.field_name} 已存在使用记录")
...
async def get_enum_field_type_history(
    ...
    """获取枚举字段类型变更历史"""
    history_records = db.query(EnumFieldHistory).filter(
        EnumFieldHistory.enum_type_id == type_id
    ).order_by(EnumFieldHistory.created_at.desc()).offset(skip).limit(limit).all()
...
async def get_enum_field_value_history(
    ...
    """获取枚举字段值变更历史"""
    history_records = db.query(EnumFieldHistory).filter(
        EnumFieldHistory.enum_value_id == value_id
    ).order_by(EnumFieldHistory.created_at.desc()).offset(skip).limit(limit).all()
