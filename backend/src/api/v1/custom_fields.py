...
@router.get("/", response_model=List[AssetCustomFieldResponse], summary="获取自定义字段列表")
async def get_custom_fields(
    asset_id: Optional[str] = Query(None, description="资产ID筛选"),
    field_type: Optional[str] = Query(None, description="字段类型筛选"),
    is_required: Optional[bool] = Query(None, description="是否必填筛选"),
...
@router.get("/{field_id}", response_model=AssetCustomFieldResponse, summary="获取自定义字段详情")
...
@router.post("/", response_model=AssetCustomFieldResponse, summary="创建自定义字段", status_code=201)
async def create_custom_field(
    ...
    try:
        # 检查是否存在相同的字段名
        existing = custom_field_crud.get_by_field_name(db=db, field_name=field_in.field_name)
        if existing:
            raise HTTPException(
                status_code=400, 
                detail=f"字段名 {field_in.field_name} 已存在"
            )
...
@router.put("/{field_id}", response_model=AssetCustomFieldResponse, summary="更新自定义字段")
...
async def get_asset_custom_field_values(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"获取资产自定义字段值失败: {str(e)}")
...
async def update_asset_custom_field_values(
    ...
    try:
        updated_values = custom_field_crud.update_asset_field_values(
            db=db, 
            asset_id=asset_id, 
            values=values_update.values
        )
        return {"asset_id": asset_id, "values": updated_values}
        
    except Exception as e:
        raise HTTPException(status_code=500, detail=f"更新资产自定义字段值失败: {str(e)}")
...
async def batch_set_custom_field_values(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"批量设置自定义字段值失败: {str(e)}")