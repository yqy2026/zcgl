...
async def get_organization_tree(
    parent_id: Optional[str] = Query(None, description="父组织ID，为空则获取根组织"),
    db: Session = Depends(get_db)
):
    """获取组织树形结构"""
    crud = get_organization_crud(db)
    
    def build_tree(parent_id: Optional[str] = None) -> List[OrganizationTree]:
        """递归构建组织树"""
    ...
    for org_id in batch_request.organization_ids:
        try:
            if batch_request.action == "delete":
                ...
                if success:
                    results.append({"id": org_id, "status": "success", "message": "删除成功"})
                else:
...
async def advanced_search_organizations(
    ...
    if search_request.keyword:
        organizations = crud.search(search_request.keyword, search_request.skip, search_request.limit)
    else:
        organizations = crud.get_all(search_request.skip, search_request.limit)
    
    # 根据其他条件过滤
    if search_request.level:
        organizations = [org for org in organizations if org.level == search_request.level]
    if search_request.parent_id:
        organizations = [org for org in organizations if org.parent_id == search_request.parent_id]
