...
from ...models.asset import Asset, AssetDocument, AssetHistory
...
async def clear_database(
    ...
    if not confirm:
        raise HTTPException(
            status_code=400, detail="必须设置 confirm=true 才能执行清空操作"
        )
...
async def reset_database(
    ...
    if not confirm:
        raise HTTPException(
            status_code=400, detail="必须设置 confirm=true 才能执行重置操作"
        )
