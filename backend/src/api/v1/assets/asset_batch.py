...
async def batch_update_assets(
    ...
    def _sync(sync_db: Session) -> AssetBatchUpdateResponse:
        db = sync_db
        try:
            # 使用新的Service层
...
async def validate_asset_data(
    ...
    def _sync(sync_db: Session) -> AssetValidationResponse:
        db = sync_db
        try:
            # 使用新的Service层
            service = AssetBatchService(db)
...
async def batch_update_custom_fields(
    ...
    def _sync(sync_db: Session) -> BatchCustomFieldUpdateResponse:
        db = sync_db
        try:
