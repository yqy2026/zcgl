...
from ...crud.rbac import RoleCRUD, PermissionCRUD, UserRoleAssignmentCRUD
...
async def get_roles(
    ...
    category: Optional[str] = Query(None, description="角色类别"),
...
@router.post("", response_model=RoleDetailResponse, status_code=status.HTTP_201_CREATED, summary="创建角色")
...
@router.get("/{role_id}/users", response_model=RoleUserListResponse, summary="获取角色的用户列表")
    ...
    try:
        ...
        return {
            ...
            "data": {
                ...
                "custom_roles": db.query(Role).filter(Role.is_system_role == False).count(),
