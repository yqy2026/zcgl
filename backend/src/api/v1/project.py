        ...
        for relation in db_project.ownership_relations:
            if hasattr(relation, 'ownership') and relation.ownership:
                ownership_relations_data.append({
                    ...
                    'ownership_name': relation.ownership.name if relation.ownership else None,
                    'ownership_code': relation.ownership.code if relation.ownership else None,
                    'ownership_short_name': relation.ownership.short_name if relation.ownership else None,
...
@router.delete("/{project_id}", response_model=ProjectDeleteResponse, summary="删除项目")
...
async def get_projects(
    ...
    project_status: Optional[str] = Query(None, description="项目状态"),
    city: Optional[str] = Query(None, description="城市"),
    ownership_id: Optional[str] = Query(None, description="权属方ID"),
    ownership_entity: Optional[str] = Query(None, description="权属方名称")
):
    """获取项目列表"""
...
async def get_project_statistics(
    ...
    try:
        ...
        # 转换最近创建的项目
        recent_created = [ProjectResponse.model_validate(item) for item in stats["recent_created"]]
...
@router.post("/{project_id}/toggle-status", response_model=ProjectResponse, summary="切换项目状态")
