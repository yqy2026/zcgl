...
class DefectReport(BaseModel):
    """缺陷报告模型"""
    defect_id: Optional[str] = Field(None, description="缺陷ID")
    ...
    attachments: Optional[List[str]] = Field(default=[], description="附件")
    tags: Optional[List[str]] = Field(default=[], description="标签")
    test_coverage_impact: Optional[Dict[str, Any]] = Field(None, description="测试覆盖率影响")
    created_at: Optional[datetime] = Field(None, description="创建时间")
    updated_at: Optional[datetime] = Field(None, description="更新时间")
    resolved_at: Optional[datetime] = Field(None, description="解决时间")
    fix_version: Optional[str] = Field(None, description="修复版本")
...
def init_defect_db():
    ...
    cursor.execute("CREATE INDEX IF NOT EXISTS idx_defect_history_defect_id ON defect_history(defect_id)")
...
async def get_defects(
    status: Optional[DefectStatus] = Query(None, description="状态过滤"),
    severity: Optional[DefectSeverity] = Query(None, description="严重程度过滤"),
    category: Optional[DefectCategory] = Query(None, description="分类过滤"),
    module: Optional[str] = Query(None, description="模块过滤"),
    reporter: Optional[str] = Query(None, description="报告人过滤"),
    assigned_to: Optional[str] = Query(None, description="指派人过滤"),
...
async def update_defect(
    ...
    try:
        ...
        for field, value in updates.items():
            if field in ["title", "description", "severity", "priority", "status",
                        "category", "module", "assigned_to", "environment", "root_cause", "resolution", "fix_version"]:
        ...
        # 如果状态变更为解决，记录解决时间
        if updates.get("status") == DefectStatus.RESOLVED and current_defect["status"] != DefectStatus.RESOLVED:
            cursor.execute(
...
async def create_prevention_measure(
...
async def get_prevention_measures(
    category: Optional[DefectCategory] = Query(None, description="分类过滤"),
    priority: Optional[DefectPriority] = Query(None, description="优先级过滤")
):
    """
    获取预防措施列表
...
def _row_to_defect_report(row) -> DefectReport:
    ...
    return DefectReport(
        ...
        test_coverage_impact=json.loads(row['test_coverage_impact']) if row['test_coverage_impact'] else None,
