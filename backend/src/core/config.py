...
class Settings(BaseSettings):
    ...
    # 数据库配置
    DATABASE_URL: str = Field(default="sqlite:///./land_property.db", json_schema_extra={"env": "DATABASE_URL"})
    DATABASE_ECHO: bool = Field(default=False, json_schema_extra={"env": "DATABASE_ECHO"})

    # Redis缓存配置
    REDIS_HOST: Optional[str] = Field(default=None, json_schema_extra={"env": "REDIS_HOST"})
    ...
    # 文件上传配置
    MAX_FILE_SIZE: int = Field(default=50 * 1024 * 1024, json_schema_extra={"env": "MAX_FILE_SIZE"})  # 50MB
    ...
    LOG_FILE: Optional[str] = Field(default=None, json_schema_extra={"env": "LOG_FILE"})

    # 性能监控
    ENABLE_METRICS: bool = Field(default=True, json_schema_extra={"env": "ENABLE_METRICS"})
    SLOW_QUERY_THRESHOLD: float = Field(default=1.0, json_schema_extra={"env": "SLOW_QUERY_THRESHOLD"})
    ...
    LOCKOUT_DURATION: int = Field(default=900, json_schema_extra={"env": "LOCKOUT_DURATION"})  # 15分钟
    MAX_CONCURRENT_SESSIONS: int = Field(default=5, json_schema_extra={"env": "MAX_CONCURRENT_SESSIONS"})
    AUDIT_LOG_RETENTION_DAYS: int = Field(default=90, json_schema_extra={"env": "AUDIT_LOG_RETENTION_DAYS"})
