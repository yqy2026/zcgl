...
class RoleCRUD:
    ...
    def get(self, db: Session, role_id: str) -> Optional[Role]:
        """根据ID获取角色"""
    ...
    def get_multi(self, db: Session, skip: int = 0, limit: int = 100,
                  ...
                  organization_id: Optional[str] = None) -> tuple[List[Role], int]:
        """获取角色列表"""
        query = db.query(Role)
    ...
    def update(self, db: Session, db_obj: Role, obj_in: RoleUpdate, updated_by: str) -> Role:
        """更新角色"""
    ...
    def delete(self, db: Session, role_id: str, deleted_by: str) -> bool:
        ...
        user_count = db.query(UserRoleAssignment).filter(
            and_(UserRoleAssignment.role_id == role_id, UserRoleAssignment.is_active == True)
        ).count()
    ...
    def count_by_category(self, db: Session) -> dict:
        """按类别统计角色数"""
        result = db.query(Role.category, func.count(Role.id)).group_by(Role.category).all()
...
class PermissionCRUD:
    ...
    def get(self, db: Session, permission_id: str) -> Optional[Permission]:
        """根据ID获取权限"""
    ...
    def get_multi(self, db: Session, skip: int = 0, limit: int = 100,
                  ...
                  is_system_permission: Optional[bool] = None) -> tuple[List[Permission], int]:
        """获取权限列表"""
    ...
    def create(self, db: Session, obj_in: PermissionCreate, created_by: str) -> Permission:
        """创建权限"""
        # 检查权限名称唯一性
    ...
    def update(self, db: Session, db_obj: Permission, obj_in: PermissionUpdate, updated_by: str) -> Permission:
        """更新权限"""
        import json
        ...
        if 'conditions' in update_data:
            update_data['conditions'] = json.dumps(update_data['conditions']) if update_data['conditions'] else None
    ...
    def count_by_resource(self, db: Session) -> dict:
        """按资源统计权限数"""
        result = db.query(Permission.resource, func.count(Permission.id)).group_by(Permission.resource).all()
...
class UserRoleAssignmentCRUD:
    ...
    def get(self, db: Session, assignment_id: str) -> Optional[UserRoleAssignment]:
        """根据ID获取分配记录"""
        return db.query(UserRoleAssignment).filter(UserRoleAssignment.id == assignment_id).first()

    def get_user_active_assignments(self, db: Session, user_id: str) -> List[UserRoleAssignment]:
        """获取用户活跃的角色分配"""
    ...
    def get_multi(self, db: Session, skip: int = 0, limit: int = 100,
                  ...
                  is_active: Optional[bool] = None) -> tuple[List[UserRoleAssignment], int]:
        """获取分配列表"""
        ...
        # 分页
        assignments = query.order_by(UserRoleAssignment.assigned_at.desc()).offset(skip).limit(limit).all()

        return assignments, total

    def create(self, db: Session, obj_in: UserRoleAssignmentCreate, assigned_by: str) -> UserRoleAssignment:
        """创建用户角色分配"""
        # 检查是否已分配
    ...
    def update(self, db: Session, db_obj: UserRoleAssignment, obj_in: UserRoleAssignmentUpdate) -> UserRoleAssignment:
        """更新用户角色分配"""
        update_data = obj_in.dict(exclude_unset=True)
    ...
    def count_active(self, db: Session) -> int:
        """活跃分配总数"""
        return db.query(func.count(UserRoleAssignment.id)).filter(UserRoleAssignment.is_active == True).scalar()


class ResourcePermissionCRUD:
    """资源权限CRUD操作"""

    def get(self, db: Session, permission_id: str) -> Optional[ResourcePermission]:
        """根据ID获取资源权限"""
        return db.query(ResourcePermission).filter(ResourcePermission.id == permission_id).first()

    def get_user_resource_permissions(self, db: Session, user_id: str,
                                    resource_type: Optional[str] = None,
                                    resource_id: Optional[str] = None) -> List[ResourcePermission]:
        """获取用户资源权限"""
    ...
    def get_multi(self, db: Session, skip: int = 0, limit: int = 100,
                  ...
                  user_id: Optional[str] = None,
                  is_active: Optional[bool] = None) -> tuple[List[ResourcePermission], int]:
        """获取资源权限列表"""
        ...
        # 分页
        permissions = query.order_by(ResourcePermission.granted_at.desc()).offset(skip).limit(limit).all()

        return permissions, total

    def create(self, db: Session, obj_in: ResourcePermissionCreate, granted_by: str) -> ResourcePermission:
        """创建资源权限"""
        import json
    ...
    def update(self, db: Session, db_obj: ResourcePermission, obj_in: ResourcePermissionUpdate) -> ResourcePermission:
        """更新资源权限"""
        import json
        update_data = obj_in.dict(exclude_unset=True)
    ...
    def count_active(self, db: Session) -> int:
        """活跃资源权限总数"""
        return db.query(func.count(ResourcePermission.id)).filter(ResourcePermission.is_active == True).scalar()
...
class PermissionAuditLogCRUD:
    ...
    def get_multi(self, db: Session, skip: int = 0, limit: int = 100,
                  ...
                  end_date: Optional[datetime] = None) -> tuple[List[PermissionAuditLog], int]:
        """获取审计日志列表"""
        ...
        # 分页
        logs = query.order_by(PermissionAuditLog.created_at.desc()).offset(skip).limit(limit).all()
    ...
    def create(self, db: Session, **kwargs) -> PermissionAuditLog:
        ...
        db_obj = PermissionAuditLog(
            ...
            old_permissions=json.dumps(kwargs.get('old_permissions')) if kwargs.get('old_permissions') else None,
            new_permissions=json.dumps(kwargs.get('new_permissions')) if kwargs.get('new_permissions') else None,
    ...
    def get_action_statistics(self, db: Session, days: int = 30) -> dict:
        ...
        result = db.query(
            ...
            PermissionAuditLog.created_at >= start_date
        ).group_by(PermissionAuditLog.action).all()
