...
class CRUDSystemDictionary(CRUDBase[SystemDictionary, SystemDictionaryCreate, SystemDictionaryUpdate]):
    """系统字典CRUD操作类"""
    ...
    def get_by_type_and_code(
        ...
        *, 
        dict_type: str, 
        dict_code: str
    ) -> Optional[SystemDictionary]:
        """根据类型和代码获取字典项"""
        return db.query(SystemDictionary).filter(
            and_(
    ...
    def get_multi_with_filters(
        ...
        *, 
        ...
        return query.order_by(SystemDictionary.sort_order, SystemDictionary.created_at).offset(skip).limit(limit).all()
    ...
    def get_by_type(
        ...
        *, 
        ...
        """根据类型获取字典列表"""
        query = db.query(SystemDictionary).filter(SystemDictionary.dict_type == dict_type)
        
        if is_active is not None:
            query = query.filter(SystemDictionary.is_active == is_active)
            
        return query.order_by(SystemDictionary.sort_order, SystemDictionary.created_at).all()
