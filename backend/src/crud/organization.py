...
class OrganizationCRUD:
    ...
    def update(self, org_id: str, obj_in: OrganizationUpdate) -> Organization | None:
        """更新组织"""
    ...
    def delete(self, org_id: str, deleted_by: str | None = None) -> bool:
        """软删除组织"""
        db_obj = self.get(org_id)
        if not db_obj:
            return False
    ...
    def get_statistics(self) -> dict[str, Any]:
        ...
        for level_row in levels:
            ...
            count = (
                self.db.query(Organization)
                .filter(and_(not Organization.is_deleted, Organization.level == level))
                .count()
            )
