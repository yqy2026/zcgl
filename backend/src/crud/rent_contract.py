...
class CRUDRentContract(CRUDBase[RentContract, RentContractCreate, RentContractUpdate]):
    ...
    def create(self, db: Session, *, obj_in: RentContractCreate | dict[str, Any], **kwargs: Any) -> RentContract:
        """创建合同 - 加密敏感字段"""
    ...
    def get(self, db: Session, id: Any) -> RentContract | None:
        """获取合同 - 解密敏感字段"""
    ...
    def get_multi(self, db: Session, *, skip: int = 0, limit: int = 100) -> list[RentContract]:
        """获取多个合同 - 解密敏感字段"""
    ...
    def update(self, db: Session, *, db_obj: RentContract, obj_in: RentContractUpdate | dict[str, Any]) -> RentContract:
        """更新合同 - 加密新的敏感字段值"""
        if isinstance(obj_in, dict):
            update_data = obj_in
        else:
