...
class CRUDCustomField(CRUDBase[AssetCustomField, AssetCustomFieldCreate, AssetCustomFieldUpdate]):
    """自定义字段CRUD操作类"""
    ...
    def get_by_field_name(
        ...
        *, 
        field_name: str
    ) -> Optional[AssetCustomField]:
        """根据字段名获取字段配置"""
        return db.query(AssetCustomField).filter(
            AssetCustomField.field_name == field_name
        ).first()
        ...
        *, 
        ...
        if filters:
            ...
            if "field_type" in filters:
                query = query.filter(AssetCustomField.field_type == filters["field_type"])
            if "is_required" in filters:
                query = query.filter(AssetCustomField.is_required == filters["is_required"])
            if "is_active" in filters:
                query = query.filter(AssetCustomField.is_active == filters["is_active"])
        
        return query.order_by(AssetCustomField.sort_order, AssetCustomField.created_at).offset(skip).limit(limit).all()
    
    def get_active_fields(self, db: Session) -> List[AssetCustomField]:
        """获取所有启用的字段"""
        return db.query(AssetCustomField).filter(
            AssetCustomField.is_active == True
        ).order_by(AssetCustomField.sort_order, AssetCustomField.created_at).all()
        ...
        try:
            ...
            if field.field_type == "text":
                ...
                if field.validation_rules:
                    rules = json.loads(field.validation_rules) if isinstance(field.validation_rules, str) else field.validation_rules
                    max_length = rules.get("max_length")
                    min_length = rules.get("min_length")
                    
                    if max_length and len(value) > max_length:
                        return False, f"字段 {field.display_name} 长度不能超过 {max_length} 个字符"
                    if min_length and len(value) < min_length:
                        return False, f"字段 {field.display_name} 长度不能少于 {min_length} 个字符"
                ...
                try:
                    ...
                    if field.validation_rules:
                        ...
                        if max_value is not None and int_value > max_value:
                            return False, f"字段 {field.display_name} 值不能大于 {max_value}"
                        if min_value is not None and int_value < min_value:
                            return False, f"字段 {field.display_name} 值不能小于 {min_value}"
                ...
                try:
                    ...
                    return False, f"字段 {field.display_name} 日期格式不正确，应为 YYYY-MM-DD"
                ...
                if field.field_options:
                    options = json.loads(field.field_options) if isinstance(field.field_options, str) else field.field_options
                    valid_values = [opt.get("value") for opt in options if isinstance(opt, dict)]
                    
                    if field.field_type == "select":
                        if value not in valid_values:
                            return False, f"字段 {field.display_name} 选项不在允许范围内"
                    else:  # multiselect
                        if not isinstance(value, list):
                            return False, f"字段 {field.display_name} 必须为数组类型"
                        for v in value:
                            if v not in valid_values:
                                return False, f"字段 {field.display_name} 包含不允许的选项: {v}"
