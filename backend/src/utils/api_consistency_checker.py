...
class APIConsistencyChecker:
    ...
    def _parse_backend_file(self, file_path: Path):
        ...
        try:
            ...
            # 查找FastAPI路由装饰器
            router_pattern = r'@(?:router|app)\.(get|post|put|delete|patch)\([\'"]([^\'"]+)[\'"]'
        ...
        for key, backend_api in self.backend_apis.items():
            if key not in self.frontend_apis:
                self.inconsistencies.append(Inconsistency(
                    ...
                    suggestion=f'考虑在前端使用此端点或删除未使用的后端代码'
                ))
    ...
    def _generate_recommendations(self) -> List[str]:
        ...
        high_issues = [i for i in self.inconsistencies if i.severity == 'high']
        if high_issues:
            recommendations.append(f"⚠️ 发现 {len(high_issues)} 个高严重性问题，需要立即处理")
...
def check_api_consistency(frontend_dir: str = "../../frontend",
                         backend_dir: str = ".",
