...
class EnhancedSecurityMiddleware(BaseHTTPMiddleware):
    ...
    def _cleanup_expired_rate_limits(self, now: float):
        ...
        for ip, limits in self.rate_limits.items():
            ...
            limits["failed_logins"] = [
                fail_time for fail_time in limits["failed_logins"]
    ...
    async def _detect_suspicious_activity(self, request: Request, client_ip: str, user_agent: str):
        """检测可疑活动"""
    ...
    async def _track_login_attempt(self, request: Request, client_ip: str, user_agent: str):
        """跟踪登录尝试"""
        # 这里需要在请求处理完成后检查结果
    ...
    async def _check_brute_force_pattern(self, client_ip: str):
        ...
        # 检查短时间内的多次失败尝试
        recent_failures = [
            fail_time for fail_time in self.rate_limits[client_ip].get("failed_logins", [])
    ...
    async def _log_suspicious_activity(self, client_ip: str, activity_type: str, details: dict):
        ...
        if self.security_service:
            try:
                # 这里可以根据需要实现更复杂的日志记录
                print(f"[SECURITY] 可疑活动检测: {activity_type}, IP: {client_ip}, 详情: {details}")
...
class EnhancedTokenValidator:
    ...
    async def validate_token(
        ...
        if expected_fingerprint:
            current_fingerprint = self.security_service._generate_device_fingerprint(device_info)
