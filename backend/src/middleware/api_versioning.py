...
class APIVersionMiddleware(BaseHTTPMiddleware):
    ...
    def __init__(
        ...
        self.version_stats: dict[str, dict] = {
            version: {"count": 0, "last_used": None, "errors": 0}
    ...
    async def dispatch(self, request: Request, call_next: Callable) -> Response:
        ...
        if version in self.deprecated_versions:
            sunset_date = self.deprecated_versions[version] + timedelta(
        ...
        try:
            ...
            if version in self.deprecated_versions:
                ...
                response.headers["Sunset"] = (
                    request.state.version_sunset_date.isoformat()
                )
...
class APIVersionManager:
    ...
    def create_migration_plan(
        self, from_version: str, to_version: str, steps: list[str]
    ):
        """创建版本迁移计划"""
    ...
    def _get_version_status(
        self, deprecation_date: datetime | None, sunset_date: datetime | None
    ) -> str:
        """获取版本状态"""
        now = datetime.now()
    ...
    def _get_current_default_version(self) -> str:
        ...
        # 返回最新的活跃版本
        return (
            max(active_versions, key=lambda v: self.versions[v]["release_date"])
            if active_versions
            else "v1"
        )
...
version_manager.register_version(
    version="v1", description="初始API版本", release_date=datetime(2024, 1, 1)
)
