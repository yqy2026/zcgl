...
class ErrorRecoveryMiddleware(BaseHTTPMiddleware):
    ...
    async def _handle_exception_with_recovery(
        ...
        logger.error(
            f"请求异常 [ID: {request_id}]: {error_category.value} - {str(exc)}"
        )
        ...
        if recovery_result.success:
            # 恢复成功
            logger.info(
                f"错误恢复成功 [ID: {request_id}]: {recovery_result.strategy_used}"
            )
            ...
            # 恢复失败
            logger.error(
                f"错误恢复失败 [ID: {request_id}]: {recovery_result.final_error}"
            )
    ...
    def _classify_error(self, exc: Exception, request: Request) -> ErrorCategory:
        ...
        if any(
            keyword in error_message
            for keyword in ["database", "sql", "connection", "deadlock"]
        ) or any(keyword in error_type for keyword in ["database", "sql"]):
        ...
        ) or any(keyword in error_type for keyword in ["validation", "value"]):
            return ErrorCategory.VALIDATION

        # 认证相关错误
        if any(
            keyword in error_message
            for keyword in ["auth", "token", "unauthorized", "permission"]
        ) or any(keyword in error_type for keyword in ["auth", "permission"]):
            return ErrorCategory.AUTHENTICATION

        # 文件系统相关错误
        if any(
            keyword in error_message
            for keyword in ["file", "path", "directory", "disk"]
        ) or any(keyword in error_type for keyword in ["file", "path", "io"]):
            return ErrorCategory.FILE_SYSTEM

        # 外部API相关错误
        if any(
            keyword in error_message for keyword in ["api", "service", "external"]
        ) or any(keyword in error_type for keyword in ["api"]):
            return ErrorCategory.EXTERNAL_API

        # 处理相关错误
        if any(
            keyword in error_message for keyword in ["processing", "parse", "transform"]
        ) or any(keyword in error_type for keyword in ["processing", "parse"]):
    ...
    def _determine_error_severity(
        self, exc: Exception, error_category: ErrorCategory
    ) -> ErrorSeverity:
        """确定错误严重程度"""
    ...
    def _get_error_status_code(
        self, error_category: ErrorCategory, severity: ErrorSeverity
    ) -> int:
        """获取错误状态码"""
    ...
    def _get_error_suggestions(self, error_category: ErrorCategory) -> list[str]:
        ...
        suggestions_map = {
            ErrorCategory.NETWORK: ["检查网络连接", "稍后重试", "联系系统管理员"],
            ...
            ErrorCategory.FILE_SYSTEM: ["检查文件路径", "验证文件权限", "检查磁盘空间"],
