...
class RateLimiter:
    ...
    async def is_allowed(self, client_ip: str, max_requests: int = 100, window_seconds: int = 3600) -> bool:
        """检查是否允许请求（基于IP的速率限制）"""
    ...
    async def is_allowed_by_user(self, user_id: str, max_requests: int = 1000, window_seconds: int = 3600) -> bool:
        """检查是否允许请求（基于用户的速率限制）"""
        async with self.lock:
            now = time.time()
            # 清理过期的请求记录
