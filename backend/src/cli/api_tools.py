...
@cli.command()
@click.option('--app-path', '-a', default='src.main:app',
              help='FastAPI应用路径，格式: module.file:app')
@click.option('--output-dir', '-o', default='docs/api',
              help='文档输出目录')
@click.option('--format', '-f', type=click.Choice(['json', 'markdown', 'all']),
...
@cli.command()
@click.option('--frontend-dir', '-f', default='frontend',
              help='前端代码目录')
@click.option('--backend-dir', '-b', default='backend',
              help='后端代码目录')
@click.option('--output-dir', '-o', default='reports',
              help='报告输出目录')
@click.option('--severity', '-s', type=click.Choice(['high', 'medium', 'low', 'all']),
              default='all', help='只显示指定严重程度的问题')
...
@cli.command()
@click.option('--output-dir', '-o', default='analysis',
              help='分析输出目录')
