...
# Import PaddleOCR services (optional - graceful degradation)
try:
    from .markdown_contract_parser import get_markdown_contract_parser
...
class ContractExtractor:
    ...
    def extract_from_pdf(self, pdf_path: str, use_gpu: bool = False) -> dict[str, Any]:
        """
        使用 PaddleOCR 3.3 PP-StructureV3 从 PDF 文件提取合同信息
        ...
        Returns:
            提取结果字典
        """
        logger.info(f"开始使用 PaddleOCR PP-StructureV3 处理 PDF: {pdf_path}")

        # 检查 PaddleOCR 是否可用
        if not PADDLEOCR_AVAILABLE or get_paddleocr_service is None:
            logger.warning("PaddleOCR 不可用，回退到基础文本提取")
            return {
                "success": False,
                "error": "PaddleOCR 未安装，请运行: pip install paddleocr>=3.3.0",
        ...
        try:
            ...
            if not ocr_service or not ocr_service.is_available():
                ...
                return {
                    "success": False,
                    "error": "PaddleOCR 服务初始化失败",
                    "extraction_method": "paddleocr_unavailable",
                    "extracted_fields": {},
                    "overall_confidence": 0.0,
                }

            # 使用 PP-StructureV3 将 PDF 转换为 Markdown
            md_result = ocr_service.to_markdown(pdf_path)

            if not md_result.get("success"):
                logger.warning(f"PDF 转换失败: {md_result.get('error')}")
                return {
                    "success": False,
                    "error": f"PDF 转换失败: {md_result.get('error', '未知错误')}",
                    "extraction_method": "paddleocr_conversion_failed",
                    "extracted_fields": {},
                    "overall_confidence": 0.0,
                }
            ...
            if not markdown_content:
                logger.warning("PDF 转换产生空内容")
                return {
                    "success": False,
                    "error": "PDF 转换产生空内容",
                    "extraction_method": "paddleocr_empty_result",
                    "extracted_fields": {},
                    "overall_confidence": 0.0,
                }

            logger.info(f"PDF 转换成功，Markdown 内容长度: {len(markdown_content)}")

            # 使用 Markdown 合同解析器提取字段
            if get_markdown_contract_parser is not None:
                parser = get_markdown_contract_parser()
                contract_info = parser.parse(markdown_content)
                parsed_result = parser.to_dict(contract_info)

                # 同时使用规则引擎补充提取
                rule_result = self.extract_contract_info(markdown_content)

                # 合并结果（Markdown 解析器优先，规则引擎补充）
                merged_fields = {}
                ...
                for key, value in parsed_result.items():
                    if value is not None and key != "raw_text":
                        merged_fields[key] = {
                            "value": value,
                            "confidence": contract_info.confidence_score,
                            "extraction_method": "paddleocr_markdown",
                        }

                # 添加规则引擎的补充结果
                if rule_result.get("success"):
                    for key, field_data in rule_result.get(
                        "extracted_fields", {}
                    ).items():
                        if key not in merged_fields:
                            merged_fields[key] = field_data

                return {
                    "success": True,
                    "extracted_fields": merged_fields,
                    "overall_confidence": contract_info.confidence_score,
                    "fields_extracted": len(merged_fields),
                    "extraction_method": "paddleocr_pp_structure_v3",
                    "markdown_content": markdown_content,
                    "validation_passed": True,
                }
            else:
                # 只使用规则引擎
                return self.extract_contract_info(markdown_content)

        except Exception as e:
            logger.error(f"PaddleOCR PDF 提取失败: {e}")
            return {
                "success": False,
                "error": f"PaddleOCR 提取失败: {str(e)}",
                "extraction_method": "paddleocr_exception",
                "extracted_fields": {},
                "overall_confidence": 0.0,
            }
...
def extract_contract_from_pdf(pdf_path: str, use_gpu: bool = False) -> dict[str, Any]:
    """
    便捷函数：使用 PaddleOCR 从 PDF 提取合同信息
    ...
    """
    extractor = ContractExtractor()
    return extractor.extract_from_pdf(pdf_path, use_gpu=use_gpu)


async def extract_contract_from_pdf_cloud(pdf_path: str) -> dict[str, Any]:
    """
    便捷函数：使用 NVIDIA Cloud PaddleOCR 从 PDF 提取合同信息

    这是异步版本，使用远程 NVIDIA API 进行 OCR 处理。
    适用于本地没有 PaddleOCR 环境或需要 GPU 加速的场景。
    ...
    """
    try:
        # 导入 NVIDIA Cloud OCR 服务
        from .nvidia_cloud_ocr_service import get_nvidia_ocr_service
    except ImportError:
        return {
            "success": False,
            "error": "NVIDIA Cloud OCR 服务未安装",
            "extraction_method": "nvidia_cloud_unavailable",
            "extracted_fields": {},
            "overall_confidence": 0.0,
        }

    logger.info(f"开始使用 NVIDIA Cloud PaddleOCR 处理 PDF: {pdf_path}")

    try:
        # 获取 NVIDIA Cloud OCR 服务
        ocr_service = get_nvidia_ocr_service()

        if not ocr_service.is_available():
            logger.warning("NVIDIA Cloud OCR 服务不可用（API Key 未配置）")
            return {
                "success": False,
                "error": "NVIDIA Cloud OCR 服务不可用，请设置 NVIDIA_API_KEY 环境变量",
                "extraction_method": "nvidia_cloud_unavailable",
                "extracted_fields": {},
                "overall_confidence": 0.0,
            }

        # 使用 NVIDIA Cloud OCR 提取文本
        ocr_result = await ocr_service.extract_contract_text(pdf_path)

        if not ocr_result.get("success"):
            logger.warning(f"NVIDIA Cloud OCR 处理失败: {ocr_result.get('error')}")
            return {
                "success": False,
                "error": f"Cloud OCR 处理失败: {ocr_result.get('error', '未知错误')}",
                "extraction_method": "nvidia_cloud_failed",
                "extracted_fields": {},
                "overall_confidence": 0.0,
            }
        ...
        if not text_content:
            logger.warning("NVIDIA Cloud OCR 返回空内容")
            return {
                "success": False,
                "error": "Cloud OCR 返回空内容",
                "extraction_method": "nvidia_cloud_empty",
                "extracted_fields": {},
                "overall_confidence": 0.0,
            }

        logger.info(f"Cloud OCR 成功，文本长度: {len(text_content)}")
        ...
        if get_markdown_contract_parser is not None:
            parser = get_markdown_contract_parser()
            contract_info = parser.parse(text_content)
            parsed_result = parser.to_dict(contract_info)

            # 同时使用规则引擎补充提取
            rule_result = extractor.extract_contract_info(text_content)
            ...
            for key, value in parsed_result.items():
                if value is not None and key != "raw_text":
                    merged_fields[key] = {
                        "value": value,
                        "confidence": contract_info.confidence_score,
                        "extraction_method": "nvidia_cloud_markdown",
                    }

            # 添加规则引擎的补充结果
            if rule_result.get("success"):
                for key, field_data in rule_result.get("extracted_fields", {}).items():
                    if key not in merged_fields:
                        merged_fields[key] = field_data
            ...
            # 只使用规则引擎
            result = extractor.extract_contract_info(text_content)
            result["extraction_method"] = "nvidia_cloud_rule_based"
            return result

    except Exception as e:
        logger.error(f"NVIDIA Cloud OCR 处理异常: {e}")
        return {
            "success": False,
            "error": f"Cloud OCR 处理异常: {str(e)}",
            "extraction_method": "nvidia_cloud_exception",
            "extracted_fields": {},
            "overall_confidence": 0.0,
        }
