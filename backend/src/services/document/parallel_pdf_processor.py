...
class ParallelPDFProcessor:
    ...
    async def submit_task(
        ...
        logger.info(f"任务提交: {task_id}, 文件: {file_path}, 优先�? {priority.name}")
        return task_id
    ...
    async def process_task(self, task: ProcessingTask) -> ProcessingTask:
        ...
        try:
            # 使用统一的PDF处理服务（已整合增强功能）
            from .pdf_processing_service import pdf_processing_service

            # 执行PDF处理（使用智能配置）
            result = await pdf_processing_service.process_with_intelligent_config(
    ...
    def clear_cache(self):
        ...
        with self._cache_lock:
            ...
            logger.info(f"缓存已清空，清理�?{cleared_count} 个条�?)
