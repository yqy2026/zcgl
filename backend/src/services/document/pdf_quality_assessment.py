...
class PDFProcessingQualityAssessment:
    ...
    def __init__(self):
        ...
        self.quality_weights = {
            "text_completeness": 0.25,  # 文本完整性权�?
            "field_extraction_rate": 0.30,  # 字段提取率权�?
            "confidence_score": 0.20,  # 置信度权�?
            "ocr_accuracy": 0.15,  # OCR准确度权�?
            "structural_integrity": 0.10,  # 结构完整性权�?
        }
        ...
        self.expected_fields = {
            ...
            "property_area": r"面积|建筑面积|租赁面积|平方�?,
            "rent_amount": r"租金|月租金|租金金额|费用",
            "rent_period": r"租赁期限|租期|期限|个月",
            "party_a": r"甲方|出租方|业主|委托�?,
            "party_b": r"乙方|承租方|租户|使用�?,
            "start_date": r"起始日期|开始日期|生效日期|租赁开�?,
            "end_date": r"终止日期|结束日期|到期日期|租赁结束",
            "payment_method": r"支付方式|付款方式|缴纳方式",
            "deposit": r"保证金|押金|履约保证�?,
            "usage_purpose": r"用途|使用范围|经营用�?,
            "property_type": r"物业类型|房屋类型|建筑类型",
            "ownership_certificate": r"房产证|不动产权证|产权�?,
            "contact_info": r"联系电话|电话|联系方式",
            "legal_representative": r"法定代表人|法人代表|负责�?,
            "registration_number": r"统一社会信用代码|注册号|营业执照",
            "management_fee": r"管理费|物业费|服务�?,
        ...
        self.quality_levels = {
            ...
            "fair": {"min_score": 0.60, "description": "一�?, "color": "#faad14"},
    ...
    def _assess_text_completeness(
        ...
        # 期望的最小文本长度（基于合同类型�?
        min_expected_length = 500  # 最�?00字符
        optimal_length = 5000  # 最�?000字符
        ...
        for field_name, field_value in extracted_data.items():
            if field_value is not None and str(field_value).strip():
                ...
                if "date" in field_name.lower():
                    ...
                    type_consistency_score += 0.8  # 其他字段给部分分�?
    ...
    def _assess_text_quality(self, text: str) -> float:
        ...
        if chinese_ratio < 0.3:  # 中文字符应该占一定比�?
    ...
    def _identify_text_quality_issues(self, text: str) -> list[str]:
        ...
        if chinese_ratio < 0.2:
            issues.append("中文字符比例较低，可能影响字段识�?)

        if not re.search(r"\d{4}", text):
            issues.append("未检测到年份信息，可能影响日期字段提�?)
    ...
    def _assess_logic_consistency(self, extracted_data: dict[str, Any]) -> float:
        ...
        if start_date and end_date:
            ...
            if any(word in start_str for word in ["开�?, "起始", "start"]) and any(
    ...
    def _generate_processing_recommendations(
        ...
        if engine_used == "single_engine":
            recommendations.append("建议启用多引擎处理以提高提取准确�?)

        # 基于页面数量的推�?
        page_count = processing_metadata.get("page_count", 1)
        if page_count > 10:
            recommendations.append("对于长文档，建议分页处理以提高准确�?)

        # 基于OCR设置的推�?
        dpi_setting = processing_metadata.get("dpi", 150)
        if dpi_setting < 300:
            recommendations.append("建议将DPI设置提高�?00以上以获得更好的OCR效果")

        # 基于字段覆盖率的推荐
        field_coverage = len(extracted_data) / len(self.expected_fields)
        if field_coverage < 0.6:
            recommendations.append("建议检查文档类型是否匹配当前处理模�?)
