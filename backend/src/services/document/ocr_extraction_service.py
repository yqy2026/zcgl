...
PROPERTY_CERT_OCR_PROMPT = PROPERTY_CERT_EXTRACTION_PROMPT.replace(
    "图像", "OCR文本"
).strip()
...
class OCRExtractionService:
    ...
    def _build_field_sources(
        self, fields: dict[str, Any], source: str
    ) -> dict[str, str]:
    ...
    async def extract_property_cert(self, file_path: str) -> dict[str, Any]:
        ...
        try:
            extracted = await self._extract_with_llm(PROPERTY_CERT_OCR_PROMPT, ocr_text)
        except Exception as exc:
