...
"""
优化的OCR服务
解决PaddleOCR数据类型和API兼容性问题，提供高性能扫描件处�?
"""
...
class OptimizedOCRService:
    ...
    def _initialize_engines(self):
        ...
        try:
            ...
            logger.warning(f"PaddleOCR未安装: {e}")
    ...
    def _process_with_paddleocr(
        ...
        try:
            ...
            if result and len(result) > 0 and result[0]:
                ...
                logger.info(
                    f"页面 {page_num + 1} OCR成功: {len(texts)}行文�? 平均置信�? {avg_confidence:.3f}"
                )
    ...
    async def process_pdf_page(
        ...
        try:
            ...
            if native_text.strip():
                ...
                logger.info(f"页面 {page_num + 1} 原生文本提取成功，长度: {len(native_text)}")
        except Exception as e:
            logger.warning(f"页面 {page_num + 1} 原生文本提取失败: {e}")
        ...
        try:
            ...
            # 提取文本
            texts, confidences = await self._extract_text_with_paddleocr(img)
    ...
    async def process_pdf_document(
        ...
        try:
            ...
            logger.info(
                f"PDF处理完成: {total_pages}页, {len(total_text)}字符, 用时{total_processing_time:.2f}秒"
            )
...
try:
    ...
    async def _extract_text_with_paddleocr(self, image: Image.Image) -> tuple[list[str], list[float]]:
        """使用PaddleOCR提取文本"""
        try:
            # 转换为numpy数组
            img_array = np.array(image)
            ...
            # 如果PaddleOCR可用，使用它
            if self.ocr_engines["paddleocr"] is not None:
                result = self.ocr_engines["paddleocr"].ocr(img_array, cls=True)
            ...
            try:
                ...
                return [text], [0.8]  # 默认置信度
            except ImportError:
                logger.warning("PaddleOCR和pytesseract都不可用，返回空结果")
                return [], []
                
        except Exception as e:
            logger.error(f"PaddleOCR文本提取失败: {e}")
            return [], []
