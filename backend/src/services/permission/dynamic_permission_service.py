...
class DynamicPermissionService:
    ...
    def assign_dynamic_permission(
        ...
        """
        if not assigned_by:
            raise BusinessLogicError("必须指定分配�?)

        # 验证用户存在
        user = self.db.query(AuthUser).filter(AuthUser.id == user_id).first()
        if not user:
            raise BusinessLogicError(f"用户 {user_id} 不存�?)
        ...
        for permission_id in permission_ids:
            ...
            if not permission:
                raise BusinessLogicError(f"权限 {permission_id} 不存�?)
    ...
    def create_permission_template(
        ...
        if len(permissions) != len(permission_ids):
            raise BusinessLogicError("部分权限不存�?)
    ...
    def assign_permissions_from_template(
        ...
        if not template:
            raise BusinessLogicError(f"权限模板 {template_id} 不存在或已禁�?)
        ...
        return self.assign_dynamic_permission(
            ...
            assigned_by=assigned_by,
            reason=reason or f"从模�?{template.name} 分配权限",
        )
    ...
    def check_dynamic_permission(
        ...
        if not permission:
            return False, f"权限 {permission_code} 不存�?
        ...
        if cond_perm:
            if cond_perm.conditions and context:
                if self._evaluate_conditions(cond_perm.conditions, context):
                    return True, "条件权限通过条件检�?
                else:
                    return False, "条件权限条件不满�?
            elif not cond_perm.conditions:
                return True, "具有无条件权�?

        return False, "没有找到相应的动态权�?
