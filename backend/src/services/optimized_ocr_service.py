...
class OptimizedOCRService:
    ...
    def _convert_pdf_page_to_numpy(self, page) -> Optional[np.ndarray]:
        """将PDF页面转换为numpy数组"""
        try:
            import fitz  # PyMuPDF
    ...
    def _process_with_paddleocr(self, img_array: np.ndarray, page_num: int) -> OCRResult:
        """使用PaddleOCR处理图像"""
        ...
        try:
            if self.ocr_engines['paddleocr'] is None:
                return OCRResult(
                    ...
                    method_used="paddleocr_failed", page_number=page_num
                )
            ...
            if result and len(result) > 0 and result[0]:
                ...
                logger.info(f"页面 {page_num+1} OCR成功: {len(texts)}行文本, 平均置信度: {avg_confidence:.3f}")
                ...
                return OCRResult(
                    text="", confidence=0.0, processing_time=processing_time,
                    method_used="paddleocr_no_text", page_number=page_num
                )
            ...
            return OCRResult(
                text="", confidence=0.0, processing_time=processing_time,
                method_used="paddleocr_error", page_number=page_num
            )
    ...
    def _extract_text_with_pymupdf(self, page, page_num: int) -> OCRResult:
        ...
        try:
            ...
            if text and len(text.strip()) > 0:
                ...
                return OCRResult(
                    text="", confidence=0.0, processing_time=processing_time,
                    method_used="pymupdf_no_text", page_number=page_num
                )
            ...
            return OCRResult(
                text="", confidence=0.0, processing_time=processing_time,
                method_used="pymupdf_error", page_number=page_num
            )

    async def process_pdf_page(self, page, page_num: int, use_preprocessing: bool = True) -> OCRResult:
        """处理单个PDF页面"""
        ...
        if len(pymupdf_result.text) < 10:  # 原生文本很少，使用OCR
            try:
                ...
                if img_array is not None:
                    ...
                    # PaddleOCR处理
                    ocr_result = self._process_with_paddleocr(img_array, page_num)
        ...
        logger.info(f"页面 {page_num+1} 处理完成，最佳方法: {best_result.method_used}")
        return best_result

    async def process_pdf_document(self, pdf_path: str, max_pages: int = 10) -> Dict[str, Any]:
        """处理整个PDF文档"""
        ...
        try:
            ...
            logger.info(f"PDF处理完成: {total_pages}页, {len(total_text)}字符, 用时{total_processing_time:.2f}秒")
    ...
    def _assess_quality(self, results: List[OCRResult]) -> Dict[str, Any]:
        ...
        return {
            ...
            "pages_with_high_confidence": len([r for r in successful_results if r.confidence >= 0.8]),
            "pages_with_text": text_pages
        }

    async def extract_structured_data(self, ocr_result: Dict[str, Any]) -> Dict[str, Any]:
        """提取结构化数据"""
        try:
            ...
            structured_data = {
                ...
                "extraction_quality": "good" if len(nlp_result.get("entities", [])) > 5 else "limited"
            }

            logger.info(f"结构化数据提取完成: {structured_data['entities_found']}个实体")
