...
class ContractSemanticValidator:
    ...
    async def _validate_field(
        ...
        try:
            ...
            if field_type in self.validation_rules:
                ...
                # 合并验证结果
                validation_result.validation_level = type_validation.get('level', ValidationLevel.SUCCESS)
                validation_result.error_messages.extend(type_validation.get('errors', []))
                validation_result.warning_messages.extend(type_validation.get('warnings', []))
            ...
            validation_result.warning_messages.extend(config_validation.get('warnings', []))
    ...
    async def _validate_text(
        ...
        """验证文本字段"""
        result = {'level': ValidationLevel.SUCCESS, 'errors': [], 'warnings': [], 'suggestions': []}
    ...
    async def _validate_address(
        ...
        # 地址结构检查
        if not any(keyword in value for keyword in ['省', '市', '区', '县', '镇', '街道', '路', '号']):
            result['level'] = ValidationLevel.WARNING
    ...
    async def _validate_id_card(
        ...
        if len(value) == 18:
            ...
            # 15位身份证验证
            if not re.match(r'^[1-9]\d{5}\d{2}(0[1-9]|1[0-2])(0[1-9]|[12]\d|3[01])\d{3}$', value):
                result['level'] = ValidationLevel.ERROR
    ...
    async def _validate_organization(
        ...
        # 机构后缀检查
        company_suffixes = ['公司', '有限', '股份', '集团', '企业', '工厂', '店', '中心']
    ...
    async def _validate_boolean(
        ...
        if not isinstance(value, bool):
            ...
            if str_value not in ['true', 'false', '1', '0', 'yes', 'no', '是', '否', '有', '无']:
                result['level'] = ValidationLevel.ERROR
    ...
    async def _check_party_completeness(
        ...
        # 检查甲方完整性
        party_a_fields = ['party_a_name', 'party_a_id', 'party_a_phone', 'party_a_address']
        party_a_complete = all(contract_data.get(field) for field in party_a_fields)

        # 检查乙方完整性
        party_b_fields = ['party_b_name', 'party_b_id', 'party_b_phone', 'party_b_address']
    ...
    async def _check_address_validity(
        ...
        # 检查地址重复
        addresses = [addr for addr in [party_a_address, party_b_address, property_address] if addr]
    ...
    def _generate_summary_and_recommendations(
        ...
        error_count = sum(1 for r in field_results.values() if r.validation_level == ValidationLevel.ERROR)
        warning_count = sum(1 for r in field_results.values() if r.validation_level == ValidationLevel.WARNING)
