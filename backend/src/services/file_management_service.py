...
class FileManagementService:
    ...
    async def cleanup_temp_files(
        ...
        try:
            ...
            active_file_paths = {
                Path(session.file_path) for session in active_sessions if session.file_path
            }
                ...
                if file_path.is_file():
                    try:
                        ...
                        if file_age.days >= self.max_file_age_days or force:
                            ...
                            logger.info(f"删除临时文件: {file_path.name} ({file_size} bytes)")
            ...
            logger.info(f"临时文件清理完成: 删除 {len(deleted_files)} 个文件, "
    ...
    async def storage_statistics(self) -> Dict[str, Any]:
        ...
        try:
            ...
            total_size = temp_stats['total_size_bytes'] + storage_stats['total_size_bytes']
            ...
            return {
                ...
                'storage_usage_percentage': round(
                    (total_size / (1024 * 1024 * 1024 * self.max_storage_size_gb)) * 100, 2
                )
            }
    ...
    async def schedule_cleanup(
        ...
        while True:
            try:
                ...
                # 清理过期的会话数据
                deleted_sessions = await pdf_session_service.cleanup_old_sessions(db, days=7)
    ...
    def validate_file(self, file_content: bytes, filename: str) -> Dict[str, Any]:
        ...
        if validation_result['file_info']['extension'] not in allowed_extensions:
            validation_result['is_valid'] = False
            validation_result['errors'].append(f"不支持的文件类型: {validation_result['file_info']['extension']}")
