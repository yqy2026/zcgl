...
class DataFilterService:
    ...
    # 前端不需要的字段列表
    REMOVED_FIELDS = {
        'annual_income', 'annual_expense', 'net_income',  # 财务字段
        'tenant_contact',  # 租户联系方式
        'last_audit_date', 'audit_status', 'auditor'  # 审核字段（保留audit_notes）
    }
    ...
    def get_field_mapping_info() -> Dict[str, Any]:
        ...
        return {
            ...
            "retained_fields": {
                ...
                "relations": [
                    "tenant_id", "project_id", "ownership_id"
                ]
            },
            "calculated_fields": [
                "unrented_area", "occupancy_rate"  # 这些是计算字段，不存储在数据库
            ],
