...
class ErrorRecoveryEngine:
    ...
    async def _execute_recovery_strategy(
        ...
        **kwargs
        ...
        logger.info(f"开始执行恢复策略: {strategy.name}, 最大尝试次数: {strategy.max_attempts}")
        ...
        for attempt in range(strategy.max_attempts):
            try:
                ...
                if result.get('success', False):
                    ...
                    # 记录成功恢复
                    self._record_recovery_success(error_context, strategy, attempt + 1, total_time)
                ...
                recovery_actions.append(f"第 {attempt + 1} 次尝试失败: {str(last_exception)}")
        ...
        **kwargs
        ...
        try:
            ...
            if error_context.category == ErrorCategory.VALIDATION:
                return await self._recover_validation_error(recovery_func, error_context, *args, **kwargs)
            elif error_context.category == ErrorCategory.AUTHENTICATION:
                return await self._recover_authentication_error(recovery_func, error_context, *args, **kwargs)
            elif error_context.category == ErrorCategory.DATABASE:
                return await self._recover_database_error(recovery_func, error_context, *args, **kwargs)
            elif error_context.category == ErrorCategory.FILE_SYSTEM:
                return await self._recover_file_system_error(recovery_func, error_context, *args, **kwargs)
            else:
                # 默认重试策略
        ...
        if 'data' in kwargs:
            ...
            try:
                ...
                return {
                    'success': True,
                    'result': result,
                    'metrics': {'auto_corrected': True, 'fields_corrected': len(corrected_data)}
                }
            except Exception as e:
                return {'success': False, 'error': e}
    ...
    def _should_open_circuit_breaker(self, category: str) -> bool:
        """判断是否应该开启熔断器"""
        recent_failures = [
            r for r in self.recovery_history[-10:]  # 最近10次记录
    ...
    def _record_recovery_success(self, error_context: ErrorContext, strategy: RecoveryStrategy, attempts: int, total_time: float):
        """记录成功恢复"""
        record = {
        ...
        self.metrics_collector.record_recovery_success(error_context.category.value, attempts, total_time)

    def _record_recovery_failure(self, error_context: ErrorContext, strategy: RecoveryStrategy, error: Exception):
        """记录失败恢复"""
        record = {
            'error_id': error_context.error_id,
            'category': error_context.category.value,
    ...
    async def _refresh_authentication_token(self) -> Optional[str]:
        """刷新认证token"""
        # 这里实现token刷新逻辑
        ...
        for category in ErrorCategory:
            ...
            if category_history:
                ...
                stats[category.value] = {
                    ...
                    'success_rate': (successful / total) * 100,
                    'average_attempts': sum(r.get('attempts', 1) for r in category_history) / total
                }
...
class MetricsCollector:
    ...
    def record_recovery_success(self, category: str, attempts: int, total_time: float):
        """记录恢复成功"""
        self.metrics['success_count'][category] = self.metrics['success_count'].get(category, 0) + 1
        self.metrics['total_attempts'][category] = self.metrics['total_attempts'].get(category, 0) + attempts
        self.metrics['total_time'][category] = self.metrics['total_time'].get(category, 0) + total_time

    def record_recovery_failure(self, category: str):
        """记录恢复失败"""
        self.metrics['failure_count'][category] = self.metrics['failure_count'].get(category, 0) + 1
