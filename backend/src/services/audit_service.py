...
class EnhancedAuditLogger:
    ...
    def log_security_event(self, 
                          ...
                          additional_data: Optional[Dict[str, Any]] = None) -> Optional[AuditLog]:
        """记录安全事件"""
        try:
            # 记录到数据库
            audit_log = AuditLog(
            ...
            self.logger.info(f"Security Event: {json.dumps(log_data, ensure_ascii=False)}")
    ...
    def _cleanup_expired_events(self):
        ...
        for action in list(self.security_events.keys()):
            self.security_events[action] = [
                event for event in self.security_events[action]
    ...
    def get_user_activity_summary(self, user_id: str, hours: int = 24) -> Dict[str, Any]:
        ...
        return {
            ...
            "unique_ips": len(set(event["ip_address"] for event in user_events if event["ip_address"])),
            "most_active_resource": self._get_most_active_resource(user_events)
        }
    
    def _get_most_active_resource(self, events: List[Dict]) -> Optional[str]:
        """获取最活跃的资源类型"""
    ...
    def detect_suspicious_activity(self, user_id: str) -> List[Dict[str, Any]]:
        ...
        login_events = [
            event for event in user_events
            if event["action"] in ["user_login", "login"]
        ]
        ...
        # 检测3: 多IP登录
        unique_ips = set(event["ip_address"] for event in user_events if event["ip_address"])
    ...
    def get_security_statistics(self, days: int = 30) -> Dict[str, Any]:
        ...
        # 查询数据库获取统计信息
        total_logs = self.session.query(AuditLog).filter(
            AuditLog.created_at >= cutoff_date
        ).count()
        
        failed_logins = self.session.query(AuditLog).filter(
            AuditLog.action == "user_login_failed",
            AuditLog.created_at >= cutoff_date
        ).count()
        
        successful_logins = self.session.query(AuditLog).filter(
            AuditLog.action == "user_login",
            AuditLog.created_at >= cutoff_date
        ).count()
        
        # 获取活跃用户数
        active_users = self.session.query(AuditLog.user_id).filter(
            AuditLog.created_at >= cutoff_date
        ).distinct().count()
