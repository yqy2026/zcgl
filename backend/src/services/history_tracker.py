...
class HistoryTracker:
    ...
    # 需要忽略的字段（系统字段，不记录变更）
    IGNORED_FIELDS = {"id", "created_at", "updated_at", "version"}

    # 敏感字段（需要特殊处理的字段）
    SENSITIVE_FIELDS = {"notes", "description"}
    ...
    def create_history_record(
        ...
        try:
            ...
            logger.info(
                f"创建变更历史记录: asset_id={asset_id}, type={change_type}, fields={len(changed_fields)}"
            )
            return history
    ...
    def get_field_history(
        ...
        try:
            ...
            for history in histories:
                old_value = (
                    history.old_values.get(field_name) if history.old_values else None
                )
                new_value = (
                    history.new_values.get(field_name) if history.new_values else None
                )
        ...
        try:
            ...
            for history in histories:
                ...
                stats["change_types"][change_type] = (
                    stats["change_types"].get(change_type, 0) + 1
                )

                # 统计变更�?
                changed_by = history.changed_by
                stats["changed_by"][changed_by] = (
                    stats["changed_by"].get(changed_by, 0) + 1
                )

                # 统计变更字段
                for field in history.changed_fields or []:
                    stats["most_changed_fields"][field] = (
                        stats["most_changed_fields"].get(field, 0) + 1
                    )

                # 统计每日变更
                date_key = history.changed_at.date().isoformat()
                stats["daily_changes"][date_key] = (
                    stats["daily_changes"].get(date_key, 0) + 1
                )
