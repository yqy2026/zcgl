...
class ContractTableAnalyzer:
    ...
    async def _analyze_table_structure(
        self, table_region: dict, original_image: np.ndarray
    ) -> TableStructure | None:
        """分析表格结构"""
        try:
    ...
    async def _ocr_table_content(self, table_image: np.ndarray) -> Dict[str, Any][str, str]:
        ...
        try:
            ...
            config = (
                '--psm 6 --oem 3 -c tessedit_char_whitelist 0123456789年月日，。；：""'
                '（）【】《》""'
                "—–至起止面积平方米元"
            )
    ...
    def _extract_lease_terms_headers(self, cells: list[list[TableCell]]) -> List[str]:
        ...
        first_col = [
            cell.text.strip() for cell in cells if cell.col == 0 and cell.text.strip()
        ]
        headers = first_col
    ...
    def _calculate_table_confidence(
        self, cells: list[list[TableCell]], text_data: dict[str, str]
    ) -> float:
        """计算表格识别置信度"""
        if not cells:
            return 0.0
        ...
        filled_cells = sum(
            len([cell for cell in row if cell.cell_type == CellType.DATA])
            for row in cells
        )
    ...
    def _assess_table_regularity(self, cells: list[list[TableCell]]) -> float:
        ...
        # 计算标准差
        if len(row_counts) > 1:
            variance = sum((count - avg_count) ** 2 for count in row_counts) / (
                len(row_counts) - 1
            )
            # 标准差越小，表格越规整
    ...
    async def extract_structured_data_from_tables(
        ...
        for table in table_structures:
            if table.table_type == TableType.PAYMENT_SCHEDULE:
                structured_data[
                    "payment_schedule"
                ] = await self._extract_payment_schedule_data(table)
            elif table.table_type == TableType.PARTY_INFO:
                structured_data["party_info"] = await self._extract_party_info_data(
                    table
                )
            elif table.table_type == TableType.LEASE_TERMS:
                structured_data["lease_terms"] = await self._extract_lease_terms_data(
                    table
                )
