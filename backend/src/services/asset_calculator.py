...
class AssetCalculator:
    ...
    def calculate_occupancy_rate(
        rentable_area: float | None, rented_area: float | None
    ) -> float | None:
        """
        计算出租率
        ...
        return float(
            Decimal(str(rate)).quantize(Decimal("0.01"), rounding=ROUND_HALF_UP)
        )

    @staticmethod
    def calculate_unrented_area(
        rentable_area: float | None, rented_area: float | None
    ) -> float | None:
        """
        计算未出租面积
    ...
    def calculate_occupancy_by_category(self) -> dict[str, Any]:
        ...
        for asset in assets:
            ...
            if rentable_area > 0:
                ...
                categories[category]["rented_area"] += float(
                    min(rented_area, rentable_area)
                )
        ...
        for category, data in categories.items():
            occupancy_rate = AssetCalculator.calculate_occupancy_rate(
                data["rentable_area"], data["rented_area"]
            )
