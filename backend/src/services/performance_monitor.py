...
class PerformanceMonitor:
    ...
    async def get_real_time_performance(self) -> Dict[str, Any]:
        ...
        try:
            ...
            # 计算统计指标
            successful_processes = [p for p in recent_processing if p.get('success', True)]
            failed_processes = [p for p in recent_processing if not p.get('success', True)]

            success_rate = len(successful_processes) / len(recent_processing) if recent_processing else 0
            error_rate = len(failed_processes) / len(recent_processing) if recent_processing else 0

            avg_processing_time = statistics.mean([p['total_processing'] for p in recent_processing])
            avg_throughput = len(recent_processing) / 60 if recent_processing else 0  # 每分钟处理数
            ...
            # 检查告警
            alerts = await self._check_performance_alerts(system_metrics, success_rate, avg_processing_time)
    ...
    def _calculate_performance_score(
        ...
        # 处理时间评分 (权重30%) - 越快越好
        time_score = max(0, 100 - min(avg_processing_time / 60 * 100, 100))  # 60秒为基准
    ...
    async def _generate_optimization_recommendations(
        ...
        if best_method:
            recommendations.append(f"建议优先使用 {best_method} 处理方法，平均时间: {best_avg_time:.1f}秒")

        # 文件大小对性能影响分析
        size_performance = [(p.get('file_size', 0), p.get('total_processing', 0)) for p in recent_processing if p.get('file_size')]
        if size_performance:
            # 简单的线性回归检测趋势
    ...
    async def _handle_performance_anomalies(self, session_id: str, anomalies: List[Dict]):
        """处理性能异常"""
        try:
            for anomaly in anomalies:
                logger.warning(f"检测到性能异常: {anomaly['type']} - {anomaly['description']}")
    ...
    async def get_performance_report(self, hours: int = 24) -> Dict[str, Any]:
        ...
        try:
            ...
            avg_performance = statistics.mean([m['performance_score'] for m in recent_data])
            peak_cpu = max([m['cpu_usage'] for m in recent_data])
            peak_memory = max([m['memory_usage'] for m in recent_data])

            # 趋势分析
            if len(recent_data) >= 10:
                performance_trend = self._analyze_trend([m['performance_score'] for m in recent_data[-10:]])
            else:
            ...
            return {
                ...
                'optimization_opportunities': await self._identify_optimization_opportunities(recent_data)
            }
...
class AnomalyDetector:
    ...
    async def detect_anomalies(self, metrics: ProcessingTimeMetrics) -> List[Dict[str, Any]]:
        """检测性能异常"""
        anomalies = []
    ...
    async def _detect_processing_time_anomaly(self, metrics: ProcessingTimeMetrics) -> Optional[Dict[str, Any]]:
        """检测处理时间异常"""
        # 这里可以实现更复杂的异常检测算法
    ...
    async def _detect_success_rate_anomaly(self, metrics: ProcessingTimeMetrics) -> Optional[Dict[str, Any]]:
        """检测成功率异常"""
        # 这里可以实现成功率异常检测逻辑
        return None
