...
class PDFProcessingMonitor:
    ...
    def _collect_system_health(self) -> SystemHealth:
        ...
        try:
            ...
            status = self._determine_system_status(
                ...
                error_rate, avg_processing_time, active_sessions
            )
    ...
    def _determine_system_status(self, cpu: float, memory: float, disk: float,
                                error_rate: float, avg_time: float, sessions: int) -> str:
        """确定系统状态"""
        # 检查关键指标
        if (cpu > 95 or memory > 95 or disk > 95 or
            error_rate > 0.5 or sessions > 20):
            return "critical"

        if (cpu > 80 or memory > 85 or disk > 90 or
            error_rate > 0.2 or avg_time > 120 or sessions > 15):
            return "warning"
    ...
    def record_operation_end(self, operation_id: str, operation_type: str,
                           success: bool, file_size: Optional[int] = None,
                           file_pages: Optional[int] = None,
                           processing_method: Optional[str] = None,
                           error_type: Optional[str] = None):
        """记录操作结束"""
        ...
        if not success and error_type:
            self.record_error(error_type, "MEDIUM", f"操作失败: {operation_type}",
                           session_id=operation_id, processing_step=operation_type)

    def record_error(self, error_type: str, severity: ErrorSeverity, message: str,
                    session_id: Optional[str] = None, file_name: Optional[str] = None,
                    processing_step: Optional[str] = None, context: Optional[Dict[str, Any]] = None):
        """记录错误"""
    ...
    @asynccontextmanager
    async def monitor_operation(self, operation_type: str, session_id: Optional[str] = None,
                              file_size: Optional[int] = None, file_pages: Optional[int] = None,
                              processing_method: Optional[str] = None):
        """操作监控上下文管理器"""
    ...
    def _determine_error_severity(self, error: Exception, operation_type: str) -> ErrorSeverity:
        """确定错误严重程度"""
        error_message = str(error).lower()

        # 系统级错误
        if any(keyword in error_message for keyword in ["memory", "disk", "system", "crash"]):
        ...
        # 文件相关错误
        if any(keyword in error_message for keyword in ["file not found", "permission", "corrupt"]):
    ...
    def get_performance_stats(self, hours: int = 24) -> Dict[str, Any]:
        ...
        for op_type, metrics in stats_by_type.items():
            ...
            type_success_rate = sum(1 for m in metrics if m.success) / len(metrics) if metrics else 0

            type_stats[op_type] = {
                "count": len(metrics),
                "success_rate": type_success_rate,
                "avg_duration": sum(type_durations) / len(type_durations) if type_durations else 0,
    ...
    def get_error_stats(self, hours: int = 24) -> Dict[str, Any]:
        ...
        # 错误率计算
        total_operations = len([m for m in self.performance_metrics if m.timestamp > cutoff_time])
