...
class DataValidator:
    ...
    def validate_contract_data(self, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        self._merge_validation_result(validation_result, consistency_validation, 'consistency')

        # 6. 业务逻辑校验
        business_validation = self._validate_business_logic(data)
        self._merge_validation_result(validation_result, business_validation, 'business')
    ...
    def _validate_rent_amounts(self, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            parsed_deposit = self._parse_amount(total_deposit) if total_deposit else None
            ...
            if parsed_deposit and parsed_rent:
                ...
                if deposit_months < 1:
                    result['warnings'].append(f"押金较低 ({deposit_months:.1f}个月租金)")
                elif deposit_months > 6:
                    result['warnings'].append(f"押金较高 ({deposit_months:.1f}个月租金)")
    ...
    def _validate_areas(self, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            parsed_rentable = self._parse_amount(rentable_area) if rentable_area else None
    ...
    def _validate_field_consistency(self, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
                ...
                if '包装合字' in contract_number:
                    ...
                    if not re.match(pattern, contract_number):
                        ...
                        if corrected != contract_number:
                            ...
                            result['suggestions'].append(f"已标准化合同编号: {corrected}")
            ...
            if address:
                ...
                # 检查地址是否包含必要信息
                if not any(keyword in address for keyword in ['号', '路', '街', '区', '市']):
    ...
    def _validate_business_logic(self, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            if monthly_rent and rentable_area:
                ...
                if unit_rent < 10:
                    result['warnings'].append(f"单位租金较低: {unit_rent:.2f}元/平方米/月")
                elif unit_rent > 1000:
                    result['warnings'].append(f"单位租金较高: {unit_rent:.2f}元/平方米/月")

                # 商业物业合理范围：20-200元/平方米/月
                if not (20 <= unit_rent <= 200):
                    result['suggestions'].append(f"建议核对租金标准，当前单位租金: {unit_rent:.2f}元/平方米/月")

            # 检查合同完整性
            required_fields = ['contract_number', 'tenant_name', 'start_date', 'end_date', 'monthly_rent']
            ...
            if '022' in str(data.get('contract_number', '')):
                ...
                if data.get('tenant_name') != expected_tenant:
                    result['suggestions'].append(f"根据合同编号，承租方应为: {expected_tenant}")
    ...
    def _merge_validation_result(self, main_result: Dict[str, Any],
                                 sub_result: Dict[str, Any], category: str):
        """合并验证结果"""
