...
class MLEnhancedExtractor:
    ...
    async def extract_contract_info(self, text: str, method: str = "hybrid") -> ExtractionResult:
        """提取合同信息"""
        ...
        try:
            ...
            # 后处理和验证
            validated_fields = await self._post_process_fields(extracted_fields, cleaned_text)
    ...
    async def _post_process_fields(self, fields: List[ExtractedField], text: str) -> List[ExtractedField]:
        """后处理字段"""
        processed_fields = []
    ...
    def _validate_id_card(self, value: str) -> Dict[str, Any]:
        ...
        if not re.match(r'^[1-9]\d{5}(?:19|20)\d{2}(?:0[1-9]|1[0-2])(?:0[1-9]|[12]\d|3[01])\d{3}[\dXx]$', value):
            result["confidence_multiplier"] = 0.3
            ...
            if len(value) == 18:
                ...
                try:
                    ...
                    if value[17].upper() != checksum:
                        result["confidence_multiplier"] = 0.5
                        result["metadata"]["validation_warning"] = "身份证校验码可能有误"
                except:
    ...
    def _validate_amount(self, value: Union[str, float]) -> Dict[str, Any]:
        """验证金额"""
        result = {"confidence_multiplier": 1.0, "metadata": {}}
    ...
    def _validate_area(self, value: Union[str, float]) -> Dict[str, Any]:
        """验证面积"""
        result = {"confidence_multiplier": 1.0, "metadata": {}}
    ...
    def _validate_business_logic(self, field: ExtractedField, all_fields: List[ExtractedField], text: str) -> Dict[str, Any]:
        """验证业务逻辑"""
        result = {"confidence_multiplier": 1.0, "metadata": {}}

        # 日期逻辑验证
        if field.name in ["start_date", "end_date", "sign_date"]:
            other_dates = {f.name: f.value for f in all_fields if f.name in ["start_date", "end_date", "sign_date"]}
            ...
            if field.name == "sign_date" and "start_date" in other_dates:
                try:
                    ...
                    start_date = datetime.strptime(other_dates["start_date"], "%Y-%m-%d")
        ...
        # 金额逻辑验证
        if field.name == "monthly_rent" and "total_deposit" in {f.name: f.value for f in all_fields}:
            try:
                rent = float(field.value)
                deposit = float(next(f.value for f in all_fields if f.name == "total_deposit"))
                # 押金通常是1-3个月租金
    ...
    def _analyze_contract_context(self, text: str) -> Dict[str, Any]:
        """分析合同上下文"""
        return {
            "is_rental_contract": any(keyword in text for keyword in ["租赁合同", "房屋租赁", "租房协议"]),
            "has_landlord_tenant": "甲方" in text and "乙方" in text,
            "text_length": len(text),
            "complexity": "high" if len(text) > 2000 else "medium" if len(text) > 500 else "low"
        }

    def _analyze_party_relationship(self, text: str) -> Dict[str, Any]:
        """分析当事方关系"""
        return {
            "party_structure": "bilateral" if "甲方" in text and "乙方" in text else "unknown",
    ...
    def _analyze_temporal_context(self, text: str) -> Dict[str, Any]:
        ...
        return {
            ...
            "time_structure": "standard" if "起租" in text and "到期" in text else "custom"
        }
    ...
    def _generate_suggestions(self, fields: List[ExtractedField]) -> List[str]:
        ...
        # 检查关键字段缺失
        key_fields = ["landlord_name", "tenant_name", "property_address", "monthly_rent"]
        ...
        if low_confidence_fields:
            suggestions.append(f"以下字段置信度较低，建议人工核对: {', '.join(low_confidence_fields)}")
        ...
        if len(date_fields) == 2:
            try:
                start_date = datetime.strptime(next(f.value for f in date_fields if f.name == "start_date"), "%Y-%m-%d")
                end_date = datetime.strptime(next(f.value for f in date_fields if f.name == "end_date"), "%Y-%m-%d")
