...
class ContractExtractorManager:
    ...
    def extract_contract_info(self, text: str, strategy: str = 'auto') -> Dict[str, Any]:
        """
        提取合同信息
    ...
    def _auto_extract(self, text: str) -> Dict[str, Any]:
        ...
        # 如果所有提取器都失败，返回最佳结果
        if results:
            best_result = max(results.values(), key=lambda x: x.get('overall_confidence', 0))
            logger.warning(f"所有提取器效果不佳，返回最佳结果 (置信度: {best_result.get('overall_confidence', 0):.2f})")
            return best_result
    ...
    def get_strategy_info(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'recommended_strategy': 'primary' if 'primary' in self.extractors else 'standard',
