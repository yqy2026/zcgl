...
# 确保配置一致性：设置最小120分钟的访问令牌有效期
if ACCESS_TOKEN_EXPIRE_MINUTES < 120:
    print(
        f"警告：ACCESS_TOKEN_EXPIRE_MINUTES设置为{ACCESS_TOKEN_EXPIRE_MINUTES}分钟，建议设置为至少120分钟以确保用户体验"
    )
    # 动态调整为120分钟
...
class AuthService:
    ...
    def create_user(self, user_data: UserCreate) -> User:
        ...
        if existing_user:
            if existing_user.username == user_data.username:  # pragma: no cover
                raise BusinessLogicError("用户名已存在")  # pragma: no cover
            else:  # pragma: no cover
                raise BusinessLogicError("邮箱已存在")  # pragma: no cover

        # 验证密码强度
        if not self.validate_password_strength(user_data.password):  # pragma: no cover
            raise BusinessLogicError("密码不符合安全要求")  # pragma: no cover
    ...
    def authenticate_user(self, username: str, password: str) -> User | None:
        ...
        if not self.verify_password(password, user.password_hash):
            ...
            if (
                ...
                user.is_locked = True  # pragma: no cover
                user.locked_until = datetime.now() + timedelta(  # pragma: no cover
        ...
        # 检查密码是否过期
        if self.is_password_expired(user):  # pragma: no cover
    ...
    def validate_refresh_token(
        ...
        try:
            ...
            # 验证受众和签发者
            if (
                aud != "land-property-system" or iss != "land-property-auth"
            ):  # pragma: no cover
                return None  # pragma: no cover

            # 验证生效时间（防止时间攻击）
            if nbf and nbf > datetime.now(UTC).timestamp():  # pragma: no cover
                return None  # pragma: no cover

            # 验证过期时间
            if exp and exp < datetime.now(UTC).timestamp():  # pragma: no cover
                return None  # pragma: no cover
        ...
        if not session or session.is_expired():  # pragma: no cover
            return None  # pragma: no cover
        ...
        if not user or not user.is_active:  # pragma: no cover
            # 用户已被停用，撤销会话
            session.is_active = False  # pragma: no cover
            self.db.commit()  # pragma: no cover
            return None  # pragma: no cover

        # 检查令牌是否已被撤销（通过JTI）
        if self._is_token_revoked(jti):  # pragma: no cover
            session.is_active = False  # pragma: no cover
            self.db.commit()  # pragma: no cover
            return None  # pragma: no cover
        ...
        # 检查会话ID匹配
        if (
            session_id and getattr(session, "session_id", None) != session_id
        ):  # pragma: no cover
            # 会话ID不匹配，可能存在安全问题
            session.is_active = False  # pragma: no cover
            self.db.commit()  # pragma: no cover
        ...
        # 检查设备指纹（如果提供）
        if device_fingerprint and client_ip and user_agent:  # pragma: no cover
            import hashlib  # pragma: no cover

            current_fingerprint_data = [  # pragma: no cover
                user_agent or "",  # pragma: no cover
                client_ip or "",  # pragma: no cover
                getattr(session, "device_id", ""),  # pragma: no cover
                getattr(session, "platform", ""),  # pragma: no cover
            ]  # pragma: no cover
            current_fingerprint_string = "|".join(  # pragma: no cover
                filter(None, current_fingerprint_data)  # pragma: no cover
            )  # pragma: no cover
            current_fingerprint = hashlib.sha256(  # pragma: no cover
                current_fingerprint_string.encode()  # pragma: no cover
            ).hexdigest()[:16]  # pragma: no cover

            if device_fingerprint != current_fingerprint:  # pragma: no cover
        ...
        # 检查IP变化（可选安全检查）
        if client_ip and getattr(session, "ip_address", None):  # pragma: no cover
            session_ip = str(getattr(session, "ip_address", ""))  # pragma: no cover
            if session_ip and session_ip != client_ip:  # pragma: no cover
                # IP地址发生变化，记录警告
                print(  # pragma: no cover
        ...
        if client_ip:  # pragma: no cover
            setattr(session, "ip_address", client_ip)  # pragma: no cover
        if user_agent:  # pragma: no cover
            setattr(session, "user_agent", user_agent)  # pragma: no cover
        self.db.commit()
    ...
    def change_password(
        ...
        if not self.verify_password(
            ...
            raise BusinessLogicError("当前密码不正确")  # pragma: no cover

        # 验证新密码强度
        if not self.validate_password_strength(new_password):  # pragma: no cover
            raise BusinessLogicError("新密码不符合安全要求")  # pragma: no cover

        # 检查新密码是否与历史记录中的密码相同
        if self.is_password_in_history(user, new_password):  # pragma: no cover
            raise BusinessLogicError(
                "新密码不能与最近使用过的密码相同"
            )  # pragma: no cover

        # 更新密码
        new_password_hash = self.get_password_hash(new_password)  # pragma: no cover
        user.password_hash = new_password_hash  # pragma: no cover

        # 将新密码添加到历史记录
        self.add_password_to_history(user, new_password_hash)  # pragma: no cover

        self.db.commit()  # pragma: no cover

        # 撤销所有会话，强制重新登录
        self.revoke_all_user_sessions(user.id)  # pragma: no cover
    ...
    def update_user(self, user_id: str, user_data: UserUpdate) -> User | None:
        ...
        if username and username != user.username:  # pragma: no cover
            existing_user = self.get_user_by_username(username)  # pragma: no cover
            if existing_user:  # pragma: no cover
                raise BusinessLogicError("用户名已被其他用户使用")  # pragma: no cover
