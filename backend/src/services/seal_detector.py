...
class SealDetector:
    ...
    def _load_red_seal_templates(self) -> List[Dict[str, Any]]:
        ...
        return [
            ...
            },
            {
                ...
                'center_text_patterns': [
                    'XX人民政府', 'XX合同专用章', 'XX公安局'
                ]
            },
            {
                'name': 'corporate_seal',
    ...
    def _apply_seal_template(self, hsv_image: np.ndarray, template: Dict[str, Any]) -> List[DetectionResult]:
        """应用单个印章模板"""
        detections = []
    ...
    def _detect_circular_seals(self, hsv_image: np.ndarray, template: Dict[str, Any]) -> List[DetectionResult]:
        """检测圆形印章"""
        try:
            # 转换为二值图像
            ...
            for contour in contours:
                ...
                if actual_ratio < expected_ratio * 0.7 or actual_ratio > expected_ratio * 1.3:
                    continue
                ...
                if self._is_dominantly_red(mask, x, y, radius):
                    detection = DetectionResult(
                        ...
                        confidence=self._calculate_seal_confidence(area, radius_actual, template),
                        seal_type=SealType.RED_CIRCLE_SEAL,
                        bbox=(center_x - radius_actual, center_y - radius_actual,
                              center_x + radius_actual, center_y + radius_actual)
                    )
    ...
    def _detect_elliptical_seals(self, hsv_image: np.ndarray, template: Dict[str, Any]) -> List[DetectionResult]:
        """检测椭圆形印章"""
        try:
            _, mask = cv2.threshold(hsv_image[:, :, 2], 127, 255, cv2.THRESH_BINARY)
            ...
            for contour in contours:
                ...
                detection = DetectionResult(
                    ...
                    confidence=self._calculate_seal_confidence(area, radius_est, template, 'ellipse'),
                    seal_type=SealType.OVAL_SEAL,
                    bbox=(int(ellipse[0][0]), int(ellipse[0][1]),
                          int(ellipse[0][0]) + int(ellipse[1][0]), int(ellipse[0][1]) + int(ellipse[1][0]))
                )
    ...
    def _is_dominantly_red(self, mask: np.ndarray, x: int, y: int, radius: int) -> bool:
        ...
        # 计算红色像素比例
        total_pixels = np.count_nonzero(mask[y - radius:y + radius, x - radius:x + radius])
        red_pixels = np.count_nonzero(cv2.bitwise_and(mask, center_mask, mask, center_mask))

        # 如果红色像素超过60%，认为是主要红色
        return red_pixels / total_pixels > 0.6

    def _filter_seal_detections(self, detections: List[DetectionResult]) -> List[DetectionResult]:
        """过滤重叠的印章检测结果"""
        if not detections:
            return []
        ...
        try:
            ...
            for contour in contours:
                ...
                if signature_type:
                    ...
                    detection.x, detection.y, detection.width, detection.height = x, y, w, h
    ...
    def _classify_signature_type(self, contour, gray: np.ndarray) -> Optional[str]:
        """分类签名类型"""
        # 计算轮廓特征
        area = cv2.contourArea(contour)
    ...
    async def _extract_seal_text(self, image_region: np.ndarray, templates: List[str]) -> str:
        """提取印章文字"""
        try:
            # 使用TesseractOCR
            config = '--psm 7 --oem 3 -c tessedit_char_whitelist 0123456789年月日人民币元角分，。；：""''（）【】《》""''—至起止甲方乙方出租承租法定代表人身份证地址电话金额'
    ...
    async def _extract_signature_text(self, image_region: np.ndarray, signature_type: str) -> str:
        """提取签名文字"""
        try:
            ...
            resized_image = resize(image_region, None, fx=scale_factor, fy=scale_factor,
                             interpolation=cv2.INTER_CUBIC)

            # 根据签名类型选择OCR配置
            if signature_type == 'chinese_character_signature':
                config = '--psm 7 --oem 3 -c tessedit_char_whitelist 1234567890号一二三四五六七八九十百千万年月日，。；：""''（）【】《》""''—至起止'
    ...
    def _calculate_seal_confidence(self, area: float, radius: float, template: Dict[str, Any], shape: str = None) -> float:
        """计算印章置信度"""
        confidence = 0.0
