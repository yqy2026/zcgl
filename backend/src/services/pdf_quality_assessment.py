...
class PDFProcessingQualityAssessment:
    ...
    def assess_processing_quality(
        ...
        # 计算各项质量指标
        text_completeness = self._assess_text_completeness(original_text, processing_metadata)
        field_extraction_rate = self._assess_field_extraction_rate(extracted_data)
        confidence_score = self._assess_confidence_score(extracted_data, processing_metadata)
        ocr_accuracy = self._assess_ocr_accuracy(original_text, ocr_results)
        structural_integrity = self._assess_structural_integrity(extracted_data, original_text)
        ...
        assessment_report = {
            ...
            'processing_time_seconds': processing_metadata.get('processing_time_seconds', 0),
            'detailed_scores': {
            ...
            'text_analysis': {
                ...
                'text_quality_issues': self._identify_text_quality_issues(original_text)
            },
            'improvement_suggestions': improvement_suggestions,
            'quality_weights': self.quality_weights,
            'recommendations': self._generate_processing_recommendations(extracted_data, processing_metadata)
        }

        processing_time = (datetime.now(timezone.utc) - assessment_start).total_seconds()
        logger.info(f"PDF处理质量评估完成，耗时: {processing_time:.3f}秒，质量分数: {quality_score:.3f}")

        return assessment_report

    def _assess_text_completeness(self, original_text: str, processing_metadata: Dict[str, Any]) -> float:
        """评估文本完整性"""
        ...
        # 综合文本完整性分数
        completeness_score = (length_score * 0.4 + cleanliness_score * 0.3 + engine_reliability * 0.2 + page_factor * 0.1)
    ...
    def _assess_confidence_score(self, extracted_data: Dict[str, Any], processing_metadata: Dict[str, Any]) -> float:
        """评估置信度分数"""
        if not extracted_data:
            return 0.0
    ...
    def _assess_ocr_accuracy(self, original_text: str, ocr_results: Optional[Dict[str, Any]]) -> float:
        """评估OCR准确度"""
        if not original_text:
            return 0.0
    ...
    def _assess_structural_integrity(self, extracted_data: Dict[str, Any], original_text: str) -> float:
        """评估结构完整性"""
        if not extracted_data:
            return 0.0
        ...
        # 检查关键字段的完整性
        critical_fields = [
            'contract_number', 'property_address', 'rent_amount',
            'party_a', 'party_b', 'start_date', 'end_date'
        ]

        present_critical_fields = sum(1 for field in critical_fields if field in extracted_data and extracted_data[field])
    ...
    def _generate_improvement_suggestions(self, quality_metrics: Dict[str, float]) -> List[Dict[str, Any]]:
        """生成改进建议"""
        suggestions = []
    ...
    def _assess_logic_consistency(self, extracted_data: Dict[str, Any]) -> float:
        ...
        if start_date and end_date:
            ...
            if any(word in start_str for word in ['开始', '起始', 'start']) and \
               any(word in end_str for word in ['结束', '终止', 'end']):
        ...
        if address:
            ...
            # 地址应该包含省市区和详细地址
            if any(word in address_str for word in ['省', '市', '区', '县', '路', '街', '号']):
                consistency_score += 1.0
            else:
