...
class ContractTemplateLearner:
            ...
            payment_template = ContractTemplate(
                ...
                fields=[
                    TemplateField(
                        ...
                        validation_rules={"options": ["现金", "银行转账", "支票"], "required": True},
                        weight=1.0
                    ),
                    TemplateField(
                        ...
                        validation_rules={"min_length": 3, "max_length": 6, "required": True},
                        weight=0.8
                    ),
                    TemplateField(
                        field_name="amount",
            ...
            party_template = ContractTemplate(
                ...
                fields=[
                    TemplateField(
                        ...
                        validation_rules={"options": ["出租方", "承租方", "买方", "卖方"], "required": True},
                        weight=1.0
                    ),
                    TemplateField(
                        ...
                        validation_rules={"min_length": 2, "max_length": 50, "chinese_name": True},
                        weight=2.0
                    ),
                    TemplateField(
            ...
            lease_template = ContractTemplate(
                ...
                fields=[
                    TemplateField(
                        ...
                        validation_rules={"min_years": 1, "max_years": 20, "required": True},
                        weight=1.0
                    ),
                    TemplateField(
                    ...
                    TemplateField(
                        ...
                        validation_rules={"positive": True, "min_value": 0, "required": True},
                        weight=1.5
                    ),
                    TemplateField(
                        ...
                        validation_rules={"options": ["月", "季", "半年", "年", "一次性"], "required": True},
                        weight=0.7
                    )
                ],
                sample_texts=[
    ...
    async def learn_from_contracts(self, contract_texts: List[str], feedback_scores: Optional[List[float]] = None) -> bool:
        """从合同文本中学习模板"""
        try:
            # 分析合同文本中的关键字段
            ...
            # 基于模式频率和重要性创建模板
            new_templates = self._create_templates_from_patterns(all_patterns, feedback_scores)
        ...
        try:
            ...
            for text in contract_texts:
                ...
                for word, pos in zip(words, pos_tags):
                    if len(word.strip()) > 1:
                        # 付款相关模式
                        if pos.startswith('v') and any(keyword in word for keyword in ['支付', '付款', '缴纳', '期数', '金额']):
                            patterns['payment_structure'][word].append({
                                'word': word,
                                'pos': pos,
                                'context': self._get_context_word(word, words, pos_tags, i)
                            })

                        # 当事人相关模式
                        elif any(keyword in word for keyword in ['甲方', '乙方', '出租', '承租', '公司', '法定代表人']):
                        ...
                        # 租赁条款相关模式
                        elif any(keyword in word for keyword in ['租赁', '期限', '租金', '面积', '用途', '违约', '起', '止']):
    ...
    def _create_templates_from_patterns(self, patterns: Dict[str, Any], feedback_scores: Optional[List[float]] = None) -> List[ContractTemplate]:
        """从模式中创建模板"""
        new_templates = []
        ...
        for template_type, pattern_data in patterns.items():
            ...
            try:
                ...
                # 生成模板ID和名称
                template_id = f"{template_type}_auto_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
                ...
                template = ContractTemplate(
                    ...
                    confidence_threshold=self._calculate_confidence_threshold(pattern_importance),
    ...
    def _analyze_pattern_for_fields(self, pattern_data: Dict[str, Any]) -> List[TemplateField]:
        """分析模式以提取字段"""
        template_fields = []
    ...
    def _get_context_word(self, word: str, words: List[str], pos_tags: List[str], index: int) -> str:
        """获取上下文词汇"""
        # 获取前后词汇
        context_words = []
    ...
    def _is_field_required(self, field_name: str) -> bool:
        """判断字段是否必填"""
        required_fields = [
            'party_name', 'property_address', 'lease_amount', 'lease_dates'
        ]
    ...
    def _calculate_pattern_importance(self, pattern_data: Dict[str, Any], feedback_scores: Optional[List[float]]) -> float:
        """计算模式重要性"""
        base_importance = 0.1
        ...
        if word_types:
            type_scores = [self.field_weights.get(word_type, 0.5) for word_type in word_types]
    ...
    async def match_contract_to_templates(self, contract_text: str) -> List[TemplateMatch]:
        """匹配合同到模板"""
        try:
            matches = []
            ...
            for template_id, template in self.templates.items():
                match = await self._match_template_to_fields(
                    extracted_fields, template
                )
    ...
    def _validate_field_value(self, value: str, field_type: str) -> bool:
        ...
        if field.field_type == 'text':
            ...
            try:
                ...
                return field.validation_rules.get('min_value', 0) <= number <= field.validation_rules.get('max_value', 10000000)
            except ValueError:
                return False
