...
class AssetDataExporter:
    ...
    FIELD_TO_COLUMN_MAPPING = {
        ...
        "business_model": "经营模式",
        "include_in_occupancy_rate": "是否计入出租�?,
        "occupancy_rate": "出租�?,
...
def export_statistics_report(
    ...
    try:
        ...
        if start_date and end_date:
            ...
            overview_sheet.merge_range(1, 0, 1, 3, f'统计期间: {start_date} 至 {end_date}')
        ...
        ownership_headers = ['权属方名称', '简称', '合同数量', '应收总额', '已收总额', '欠款总额', '收缴率']
        ...
        for row_idx, record in enumerate(ownership_data, start=1):
            ...
            ownership_sheet.write(row_idx, 3, record.get('total_due_amount', 0), currency_format)
            ownership_sheet.write(row_idx, 4, record.get('total_paid_amount', 0), currency_format)
            ownership_sheet.write(row_idx, 5, record.get('total_overdue_amount', 0), currency_format)
            ownership_sheet.write(row_idx, 6, record.get('payment_rate', 0) / 100, percentage_format)

        # 3. 资产统计工作表
        asset_sheet = workbook.add_worksheet('资产统计')
        asset_headers = ['资产名称', '地址', '合同数量', '应收总额', '已收总额', '欠款总额', '收缴率']
        ...
        for row_idx, record in enumerate(asset_data, start=1):
            ...
            asset_sheet.write(row_idx, 3, record.get('total_due_amount', 0), currency_format)
            asset_sheet.write(row_idx, 4, record.get('total_paid_amount', 0), currency_format)
            asset_sheet.write(row_idx, 5, record.get('total_overdue_amount', 0), currency_format)
            asset_sheet.write(row_idx, 6, record.get('payment_rate', 0) / 100, percentage_format)

        # 4. 月度统计工作表
        monthly_sheet = workbook.add_worksheet('月度统计')
        monthly_headers = ['年月', '合同数', '应收总额', '已收总额', '欠款总额', '收缴率']
        ...
        for row_idx, record in enumerate(monthly_data, start=1):
            ...
            monthly_sheet.write(row_idx, 2, record.get('total_due_amount', 0), currency_format)
            monthly_sheet.write(row_idx, 3, record.get('total_paid_amount', 0), currency_format)
            monthly_sheet.write(row_idx, 4, record.get('total_overdue_amount', 0), currency_format)
            monthly_sheet.write(row_idx, 5, record.get('payment_rate', 0) / 100, percentage_format)
