...
from ..models.pdf_import_session import ProcessingStep
...
class FieldValidator:
    ...
    def validate_phone_number(phone: str) -> Dict[str, Any]:
        ...
        is_valid = (
            re.match(mobile_pattern, phone) or
            re.match(landline_pattern, phone)
        )

        return {
            'is_valid': is_valid,
            'normalized_value': phone,
            'phone_type': 'mobile' if re.match(mobile_pattern, phone) else 'landline' if is_valid else 'unknown',
    ...
    def validate_date(date_str: str) -> Dict[str, Any]:
        ...
        return {
            ...
            'errors': ['日期格式不正确，支持的格式：YYYY-MM-DD, YYYY/MM/DD, YYYY年MM月DD日等']
        }
    ...
    async def find_matching_ownerships(
        ...
        try:
            ...
            for ownership in ownerships:
                # 计算名称匹配度
                name_score = fuzz.partial_ratio(
                    ownership_name.lower(),
                    (ownership.ownership_name or "").lower()
                ) / 100.0

                # 计算联系方式匹配度
                contact_score = 0
                if contact_info and ownership.contact_info:
                    contact_score = fuzz.partial_ratio(
                        contact_info.lower(),
                        ownership.contact_info.lower()
                    ) / 100.0
        ...
        try:
            ...
            for contract in contracts:
                ...
                contract_number_match = (
                    contract.contract_number and
                    contract.contract_number.strip().lower() == contract_number.strip().lower()
                )

                # 承租方名称匹配
                tenant_match = 0
                if tenant_name and contract.tenant_name:
                    tenant_match = fuzz.ratio(
                        tenant_name.lower(),
                        contract.tenant_name.lower()
                    ) / 100.0

                # 地址匹配
                address_match = 0
                if property_address and contract.property_address:
                    address_match = fuzz.partial_ratio(
                        property_address.lower(),
                        contract.property_address.lower()
                    ) / 100.0
...
class PDFValidationMatchingService:
    ...
    async def validate_extracted_data(
        ...
        for field_name, validation_func in validation_rules.items():
            if field_name in extracted_data:
                ...
                if value is not None and str(value).strip():
                    ...
                    if result['is_valid']:
                        validated_data[field_name] = result.get('numeric_value') or result.get('parsed_date') or result.get('normalized_value') or value
                    else:
                        validation_errors.extend([f"{field_name}: {error}" for error in result.get('errors', [])])

        # 计算验证分数
        total_fields = len(validation_rules)
        valid_fields = sum(1 for result in validation_results.values() if result.get('is_valid', False))
        ...
        if missing_required:
            validation_errors.extend([f"缺少必填字段: {field}" for field in missing_required])
    ...
    async def perform_data_matching(
        ...
        # 权属方匹配
        ownership_matches = []
        if extracted_data.get('landlord_name') or extracted_data.get('ownership_entity'):
            ownership_name = extracted_data.get('landlord_name') or extracted_data.get('ownership_entity', '')
        ...
        # 计算综合匹配置信度
        max_asset_confidence = max([m['match_confidence'] for m in asset_matches], default=0)
        max_ownership_confidence = max([m['match_confidence'] for m in ownership_matches], default=0)
        max_duplicate_confidence = max([m['duplicate_probability'] for m in duplicate_contracts], default=0)
        ...
        if asset_matches:
            ...
            if best_asset['match_confidence'] > 0.8:
                recommendations.append(f"建议选择资产: {best_asset['property_name']} (匹配度: {best_asset['match_confidence']:.1%})")

        if ownership_matches:
            best_ownership = ownership_matches[0]
            if best_ownership['match_confidence'] > 0.8:
                recommendations.append(f"建议选择权属方: {best_ownership['ownership_name']} (匹配度: {best_ownership['match_confidence']:.1%})")

        if duplicate_contracts:
            best_duplicate = duplicate_contracts[0]
            if best_duplicate['duplicate_probability'] > 0.8:
                recommendations.append(f"警告: 可能与现有合同重复 (概率: {best_duplicate['duplicate_probability']:.1%})")
    ...
    async def validate_and_match(
        ...
        return {
            ...
            'ready_for_import': (
                ...
                len(matching_result['duplicate_contracts']) == 0 and
