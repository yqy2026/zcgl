...
class ContractExtractor:
    ...
    def extract_contract_info(self, text: str) -> Dict[str, Any]:
        ...
        try:
            ...
            for field_name, patterns in self.extraction_rules.items():
                for pattern in patterns:
                    try:
                        ...
                        if match:
                            value = self._clean_extracted_value(field_name, match.group(1).strip())

                            if value and self._validate_field_value(field_name, value):
                                confidence = self._calculate_confidence(field_name, match.group(0))
                                ...
                                field_count += 1
                                logger.info(f"成功提取字段 {field_name}: {value} (置信度: {confidence:.2f})")
                                break
            ...
            logger.info(f"提取完成，共提取 {field_count} 个字段，总体置信度: {overall_confidence:.2f}")
            return result
    ...
    def _validate_real_contract_text(self, text: str) -> bool:
        ...
        fake_data_patterns = [
            ...
            r"北京某某", r"某某小区", r"某某街道",  # 示例地址
            r"13800138000", r"HT-2024",  # 示例电话和合同编号
            r"示例", r"test"  # 测试标识 (保留中文"测试"用于正常测试)
        ]
    ...
    def _calculate_confidence(self, field_name: str, source_text: str) -> float:
        ...
        # 根据字段重要性调整置信度
        important_fields = ['contract_number', 'tenant_name', 'landlord_name', 'monthly_rent']
