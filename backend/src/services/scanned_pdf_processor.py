...
class ScannedPDFProcessor:
    ...
    def convert_pdf_page_to_image_array(self, page) -> Optional[np.ndarray]:
        """将PDF页面转换为图像数组（简化版）"""
        try:
            import fitz
            ...
            if pix.n == 4:  # RGBA
                img_array = np.frombuffer(img_data, dtype=np.uint8).reshape((pix.height, pix.width, 4))
                img_array = img_array[:, :, :3]  # 移除alpha
            elif pix.n == 3:  # RGB
                img_array = np.frombuffer(img_data, dtype=np.uint8).reshape((pix.height, pix.width, 3))
            elif pix.n == 1:  # 灰度
                img_array = np.frombuffer(img_data, dtype=np.uint8).reshape((pix.height, pix.width))
                # 转换为RGB格式
    ...
    def extract_text_with_pymupdf_fallback(self, doc, max_pages: int = 3) -> ScanProcessingResult:
        """使用PyMuPDF作为备用文本提取"""
        ...
        try:
            ...
            for page_num in range(total_pages):
                ...
                if text.strip():
                    all_text += text + "\n\n"
                    logger.info(f"页面 {page_num+1} 使用PyMuPDF提取文本: {len(text)}字符")
    ...
    async def extract_with_simplified_paddleocr(self, pdf_path: str, max_pages: int = 3) -> ScanProcessingResult:
        """使用简化的PaddleOCR提取"""
        start_time = time.time()
                ...
                try:
                    ...
                    if result and len(result) > 0:
                        ...
                        if page_text.strip():
                            all_text += page_text + "\n\n"
                            successful_pages += 1
                            logger.info(f"页面 {page_num+1} OCR成功: {len(page_text)}字符")
                        else:
    ...
    async def extract_text_with_combined_methods(self, pdf_path: str, max_pages: int = 3) -> ScanProcessingResult:
        """使用组合方法提取文本"""
        start_time = time.time()
        ...
        paddle_result = await self.extract_with_simplified_paddleocr(pdf_path, max_pages)
        ...
        try:
            ...
            results.append(ScanProcessingResult(
                success=False, processing_method="pymupdf_error",
                text_extracted="", confidence_score=0.0,
                pages_processed=0, total_pages=0,
                processing_time=0.0, error_message=str(e)
            ))

        # 选择最佳结果
        best_result = max(results, key=lambda r: (len(r.text_extracted), r.confidence_score))
    ...
    async def process_scanned_pdf(self, pdf_path: str, max_pages: int = 5) -> Dict[str, Any]:
        """处理扫描件PDF的主入口"""
        ...
        processing_result = {
            ...
            "pages_per_second": result.total_pages / result.processing_time if result.processing_time > 0 else 0,
