...
def validate_revenue_mode_compatibility(
    ...
    allowed = _VALID_RELATION_TYPES.get(revenue_mode, frozenset())
    if group_relation_type not in allowed:
        mode_label = "承租模式" if revenue_mode == RevenueMode.LEASE else "代理模式"
        allowed_labels = [r.value for r in allowed]
        raise OperationNotAllowedError(
            f"{mode_label}合同组只允许以下合同角色：{allowed_labels}，"
            f"当前值：{group_relation_type.value}"
        )
...
class ContractGroupService:
    ...
    async def create_contract_group(
        ...
        *,
        ...
        return await contract_group_crud.create(
            db, data=data, asset_ids=obj_in.asset_ids or None
        )
    ...
    async def get_group_detail(
        ...
        group_dict = {
            ...
            for col in group.__table__.columns.keys()
        }
    ...
    async def soft_delete_group(
        ...
        active_count = await contract_crud.count_active_in_group(
