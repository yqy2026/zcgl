    ...
    with engine.connect() as conn:
        ...
        if missing_columns:
            ...
            if 'password_history' in missing_columns:
                try:
                    conn.execute(text("ALTER TABLE users ADD COLUMN password_history TEXT"))
            ...
            if 'password_last_changed' in missing_columns:
                try:
                    ...
                    conn.execute(text("ALTER TABLE users ADD COLUMN password_last_changed DATETIME"))
                    print("   + 添加 password_last_changed 字段")

                    # 为现有记录设置默认值
                    conn.execute(text("UPDATE users SET password_last_changed = CURRENT_TIMESTAMP WHERE password_last_changed IS NULL"))
                    print("   + 设置现有记录的默认值")
                except Exception as e:
        ...
        try:
            result = conn.execute(text("SELECT name FROM sqlite_master WHERE type='table' AND name='user_role_assignments'"))
            if result.fetchone():
        ...
        # 检查 permissions 表
        try:
            result = conn.execute(text("SELECT name FROM sqlite_master WHERE type='table' AND name='permissions'"))
            if result.fetchone():
