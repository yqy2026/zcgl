...
def check_overall_status():
    ...
    for error_type in error_types:
        success, stdout, stderr = run_command(f"uv run ruff check src/ --select={error_type} --statistics")
    ...
    # 检查E402总数
    success, stdout, stderr = run_command("uv run ruff check src/ --select=E402 | head -5")
