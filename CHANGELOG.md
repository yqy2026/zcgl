...
- 组织统计口径修复：`backend/src/services/organization/service.py` 的 `get_statistics` 改为按 `status/type/level` 聚合查询，`active` 仅统计 `status=active`，`inactive` 统计其余状态，`by_type` 返回真实类型分布，修复组织页“组织类型”统计恒为 0 的问题
- 组织枚举前端回归补齐：`frontend/src/pages/System/__tests__/OrganizationPage.test.tsx` 新增“字典枚举加载并提交”用例，覆盖 `organization_type/organization_status` 在组织创建表单中的加载、选择与提交值校验
- 修复迁移 `backend/alembic/versions/20260208_add_phone_make_email_nullable.py`：将 `users.phone` 的回填从固定占位值改为按用户生成唯一 11 位占位号码，避免创建唯一索引 `ix_users_phone` 时因重复值导致升级失败
- 认证响应字段修复：`backend/src/api/v1/auth/auth_modules/authentication.py` 为登录响应与 `/auth/me` 补齐 `phone` 字段，修复 `UserResponse.phone` 缺失导致登录接口响应校验失败（500/400）
- 组织创建接口 `405 Method Not Allowed` 修复：`backend/src/api/v1/auth/organization.py` 的创建路由新增无尾斜杠声明 `@router.post("")`，与现有 `@router.post("/")` 并存，确保前端 `POST /api/v1/organizations` 与 `POST /api/v1/organizations/` 均可创建组织
- 组织架构字段精简：`backend/src/schemas/organization.py` 移除组织“联系信息/负责人信息”输入输出字段（`phone/email/address`、`leader_name/leader_phone/leader_email`）；`frontend/src/types/organization.ts` 同步删除对应类型字段；`frontend/src/pages/System/OrganizationPage.tsx` 删除联系人与负责人表单区块及“负责人”列表列，组织管理页面聚焦基础结构字段
- 组织架构字段继续精简：彻底删除“主要职能”字段 `functions`，后端模型与 schema 不再暴露该字段（`backend/src/models/organization.py`、`backend/src/schemas/organization.py`），查询白名单移除对应搜索字段（`backend/src/crud/field_whitelist.py`），组织精简迁移补充删除 `organizations.functions`（`backend/alembic/versions/20260208_simplify_organization_schema.py`）；前端类型与组织管理表单同步删除该字段（`frontend/src/types/organization.ts`、`frontend/src/pages/System/OrganizationPage.tsx`）
- 组织架构二次收口与枚举统一：后端删除 `organizations` 联系/负责人列与 `positions/employees` 表、移除 `users.employee_id` 链路（`backend/src/models/organization.py`、`backend/src/models/auth.py`、`backend/src/services/organization_permission_service.py`、`backend/src/middleware/auth.py` 等），新增迁移 `backend/alembic/versions/20260208_simplify_organization_schema.py`（含 `organization_type/organization_status` 枚举种子）；`backend/src/services/organization/service.py` 新增组织类型/状态的统一枚举校验；`backend/src/api/v1/auth/organization.py` 与 `backend/src/schemas/organization.py` 补齐树接口字段 `code/type/status`；前端 `frontend/src/pages/System/OrganizationPage.tsx` 改为通过字典接口驱动组织类型/状态并补充系统字段展示（`path/created_by/updated_by`），`frontend/src/services/dictionary/config.ts` 新增组织字典配置
- 组织架构收尾修复：将 `backend/alembic/versions/20260208_add_phone_make_email_nullable.py` 的 `down_revision` 接到 `20260208_simplify_organization_schema`，消除 Alembic 双 `head`；修复 `backend/src/services/core/user_management_service.py` 在创建用户时遗漏 `phone` 写入的问题，并补齐对应单测回归
- 组织架构测试漂移与收集冲突修复：重写并对齐当前异步实现的 `backend/tests/unit/api/v1/test_users.py`、`backend/tests/unit/crud/test_organization.py`，修复 `backend/tests/unit/api/v1/test_authentication.py` 中刷新令牌返回类型与 debug patch 顺序断言；新增 `backend/tests/unit/api/__init__.py`、`backend/tests/unit/api/v1/__init__.py`、`backend/tests/unit/crud/__init__.py` 解决 `test_organization.py` 同名文件的 pytest 收集冲突
