...
- 远端同步冲突处置（2026-02-09）：将本地分支 `backup/pre-sync-20260209-081218` 基于 `origin/develop` 进行 rebase，处理冲突文件 `CHANGELOG.md`、`backend/tests/unit/api/v1/test_users.py`、`backend/tests/unit/crud/test_organization.py`；测试文件保留本地版本并完成冲突标记清理，保证本地改动不丢失且可继续验证
...
- Excel 异步任务失败处理分层收口：`backend/src/services/excel/excel_task_service.py` 新增 `mark_task_failed`，统一封装“回滚 + 失败状态写回”；`backend/src/api/v1/documents/excel/import_ops.py` 与 `backend/src/api/v1/documents/excel/export_ops.py` 去除路由层直接 `rollback`/失败状态更新，改为委托 task service，并新增 `_utcnow_naive` 替换路由内 `datetime.utcnow()` 调用；新增 `backend/tests/unit/services/excel/test_excel_task_service.py`，并在 `backend/tests/unit/api/v1/test_excel_import_ops_layering.py`、`backend/tests/unit/api/v1/test_excel_export_ops_layering.py`、`backend/tests/unit/api/v1/test_excel.py` 增加/更新失败路径与时间函数分层断言
