...
const DictionaryPage: React.FC = () => {
  ...
  // 获取所有枚举数据
  const fetchAllEnumData = useCallback(async () => {
    setLoading(true);
    try {
  ...
  }, [fetchTypes, fetchAllEnumData]);

  useEffect(() => {
    void fetchList(activeType);
  ...
  const handleDelete = useCallback(
    ...
    [activeType, detailModalVisible, fetchAllEnumData, fetchList]
  );
  ...
  const submit = async () => {
    try {
      ...
      if (editingRecord !== null && editingRecord !== undefined) {
        // 更新现有枚举值
  ...
  const handleToggleActive = useCallback(
    async (record: SystemDictionary, checked: boolean) => {
      try {
  ...
  const overviewColumns: ColumnsType<EnumFieldWithType> = useMemo(
    () => [
      ...
      },
      {
        ...
        width: 100,
        render: (_, record) => (
          <Button type="link" size="small" onClick={() => void handleViewDetail(record.type.code)}>
            查看详情
          </Button>
        ),
      },
  ...
  const columns: ColumnsType<SystemDictionary> = useMemo(
    () => [
      ...
      },
      {
        ...
        render: (v: boolean, record) => (
          <Switch checked={v} onChange={checked => void handleToggleActive(record, checked)} />
        ),
      },
      {
        title: '更新时间',
    ...
    [enumTypes, handleDelete, handleEdit, handleToggleActive]
  );
  ...
  return (
    <div style={{ padding: '24px' }}>
          ...
          {enumTypes.find(t => t.code === activeType) && (
            ...
            >
              ...
              {enumTypes.find(t => t.code === activeType)?.description !== null &&
                enumTypes.find(t => t.code === activeType)?.description !== undefined &&
      ...
      <Modal
        ...
        title={
          activeType !== null && activeType !== undefined && activeType !== ''
