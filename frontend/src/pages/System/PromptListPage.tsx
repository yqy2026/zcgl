...
const getToneClassName = (tone: MetricTone): string => {
  if (tone === 'success') {
      ...
      const versionColumns: ColumnsType<PromptVersion> = [
        ...
        },
        {
          ...
          render: (autoGenerated?: boolean) =>
            autoGenerated === true ? <Tag color="purple">自动</Tag> : <Tag>手动</Tag>,
        },
      ];
  ...
  const columns: ColumnsType<PromptTemplate> = [
    {
      ...
      render: (text: string, record: PromptTemplate) => (
        <div className={styles.nameCell}>
          ...
          <span className={styles.versionText}>v{record.version}</span>
        </div>
      ),
    },
    {
    ...
    <PageContainer title="LLM Prompt 管理" subTitle="统一管理模板版本、状态与性能指标">
      ...
      {statistics != null && (
        <Row gutter={[16, 16]} className={styles.summaryRow}>
          ...
          <Col xs={24} sm={12} xl={6}>
            <Card className={`${styles.summaryCard} ${styles.toneSuccess}`}>
              <Statistic
                title="活跃 Prompt"
                value={
          ...
          <Col xs={24} sm={12} xl={6}>
            <Card
              className={`${styles.summaryCard} ${getToneClassName(
