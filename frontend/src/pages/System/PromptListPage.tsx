      ...
      const versionColumns: ColumnsType<PromptVersion> = [
        ...
        },
        {
          ...
          render: (autoGenerated?: boolean) => {
            const sourceMeta = autoGenerated === true ? VERSION_SOURCE_META.auto : VERSION_SOURCE_META.manual;
            return (
              <Tag
  ...
  const columns: ColumnsType<PromptTemplate> = [
    ...
    },
    {
      ...
      render: (provider: LLMProvider) => {
        ...
        return (
          <Tag className={`${styles.semanticTag} ${styles.providerTag} ${getToneClassName(meta.tone)}`}>
            {meta.label}
          </Tag>
        );
      },
