...
export class ApiCacheManager {
  ...
  set<T>(url: string, data: T, options: CacheOptions = {}, params?: unknown): void {
    ...
    const ttl: number = ttlValue !== undefined ? ttlValue : CACHE.DEFAULT_TTL;
...
export function cached(options: CacheOptions = {}) {
  return function (target: unknown, propertyKey: string, descriptor: PropertyDescriptor) {
    ...
    descriptor.value = async function (...args: unknown[]) {
      const constructor = (target as Record<string, unknown>).constructor as {
        name: string | undefined;
      };
      const constructorName = constructor.name;
      const className: string =
        constructorName !== undefined && constructorName !== null
          ? constructorName
          : 'UnknownClass';
      ...
      // 检查缓存
      const cachedResult = cacheManager.get<unknown>(cacheKey, params);
      if (cachedResult !== null && cachedResult !== undefined && options.force !== true) {
        return cachedResult;
      }
