  ...
  private setupInterceptors(): void {
    ...
    this.instance.interceptors.response.use(
      ...
      async error => {
        ...
        // 处理401未授权错误
        if (axiosError.response?.status === 401 && originalRequest._retry !== true) {
          ...
          try {
            ...
            // 重新发送原请求
            return this.instance(originalRequest as AxiosRequestConfig);
          } catch (refreshError) {
            // 刷新失败，重定向到登录页
  ...
  async requestWithRetry<T = unknown>(
    ...
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
      try {
        ...
        // 计算延迟时间
        const delay =
          API_CONFIG.RETRY.DELAY * Math.pow(API_CONFIG.RETRY.BACKOFF_FACTOR, attempt - 1);
