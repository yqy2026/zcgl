...
export const roleService = {
  ...
  getRoleStatistics() {
    ...
    // return response.data
    return Promise.resolve({ total: 0, active: 0, inactive: 0 });
  },
};
...
export const logService = {
  ...
  // 获取操作日志统计
  getLogStatistics(_params?: { start_date?: string; end_date?: string }) {
    // 暂时注释，后端可能没有这个端点
    // const response = await api.get('/logs/statistics', { params })
    // return response.data
    return Promise.resolve({ total: 0, today: 0, thisWeek: 0, thisMonth: 0 });
  },
