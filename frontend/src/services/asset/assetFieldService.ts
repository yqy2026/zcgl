...
export class AssetFieldService {
    ...
    async getAssetCustomFieldValues(assetId: string): Promise<CustomFieldValue[]> {
        try {
            const result = await enhancedApiClient.get<{ values: CustomFieldValue[] }>(`/assets/${assetId}/custom-field-values`, {
                cache: true,
                retry: { maxAttempts: 2, delay: 500, backoffMultiplier: 2 },
                smartExtract: true
            });
    ...
    async updateAssetCustomFieldValues(
        ...
        try {
            const result = await enhancedApiClient.put<{ values: CustomFieldValue[] }>(`/assets/${assetId}/custom-field-values`, { values }, {
                retry: false,
                smartExtract: true
            });
    ...
    async batchSetCustomFieldValues(
        ...
        try {
            const result = await enhancedApiClient.post<void>("/assets/batch-custom-field-values", { updates }, {
                retry: false,
                smartExtract: true
            });
    ...
    async validateCustomFieldValue(fieldId: string, value: unknown): Promise<FieldValidationResult> {
        try {
            const result = await enhancedApiClient.post<{ valid: boolean; error?: string }>(`/asset-custom-fields/${fieldId}/validate`, { value }, {
                retry: false,
                smartExtract: true
            });
