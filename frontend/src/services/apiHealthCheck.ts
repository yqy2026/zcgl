...
export class ApiHealthCheckService {
  ...
  async checkEndpoint(endpoint: string, name: string = endpoint): Promise<HealthCheckResult> {
    ...
    try {
      ...
      // 如果HEAD请求不支持，尝试GET请求
      if ((response as { status?: number }).status === 405) {
        const getResponse = await api.get(endpoint);
        const getResponseTime = Date.now() - startTime;

        const getResponseStatus = (getResponse as { status?: number }).status;
        result.status =
          getResponseStatus !== undefined &&
   ...
   * 检查所有定义的API端点
   */
  async checkAllEndpoints(): Promise<{
    total: number;
    healthy: number;
    unhealthy: number;
    unknown: number;
  ...
  generateReport(): HealthReport {
    const criticalResults = this.config.criticalEndpoints.map(
      endpoint =>
  ...
  async checkSystemHealth(): Promise<{
    ...
    // 检查响应时间
    const slowEndpoints = report.details.filter(
      r => r.responseTime !== null && r.responseTime !== undefined && r.responseTime > 3000
    );
...
if (process.env.NODE_ENV === 'development') {
  ...
  setTimeout(() => {
    try {
      ...
      logger.error(
        'Failed to start API Health Check Service',
        error instanceof Error ? error : new Error(String(error))
      );
    }
  }, 2000);
