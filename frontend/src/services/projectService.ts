...
export class ProjectService {
  ...
  async getProjects(params?: ProjectSearchParams): Promise<ProjectListResponse> {
    try {
      ...
      const requestParams = {
        ...
        size: (params?.size !== null && params?.size !== undefined) ? params?.size : 10,
  ...
  async getProjectsByIds(ids: string[]): Promise<Project[]> {
    try {
      ...
      return projects.filter(project => (project !== null && project !== undefined) && project.is_active);
    } catch (error) {
      const enhancedError = ApiErrorHandler.handleError(error);
  ...
  async canDeleteProject(id: string): Promise<{ canDelete: boolean; reason?: string }> {
    try {
      ...
      if ((project.asset_count !== null && project.asset_count !== undefined) && project.asset_count > 0) {
        return {
          canDelete: false,
  ...
  async getProjectFullDetails(id: string): Promise<{
    ...
    try {
      ...
      return {
        project,
        assetCount: (projectStats.asset_count !== null && projectStats.asset_count !== undefined) ? projectStats.asset_count :
                     (project.asset_count !== null && project.asset_count !== undefined) ? project.asset_count : 0,
        totalArea: (projectStats.total_area !== null && projectStats.total_area !== undefined) ? projectStats.total_area : 0,
