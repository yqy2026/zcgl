 ...
 * @author Claude Code
 *
 * 此文件为 re-export hub，实际类型定义和缓存管理位于 baseTypes.ts
 */
...
import { dictionaryCache as cache } from './baseTypes';

// Re-export 所有类型（向后兼容）
export type { DictionaryServiceResult, DictionaryStatistics, PreloadResult } from './baseTypes';

// 导入类型供本模块内部使用
import type { DictionaryServiceResult, DictionaryStatistics, PreloadResult } from './baseTypes';
