...
class DictionaryManagerService {
          ...
          (item: unknown, index: number): EnumFieldType => {
            ...
            return {
              ...
              name:
                ...
                config.name !== ''
                  ? config.name
                  : typeCode.replace(/_/g, ' ').replace(/\b\w/g, l => l.toUpperCase()),
              code: typeCode,
              category:
                config !== null &&
                config !== undefined &&
  ...
  async getEnumFieldValues(typeId: string): Promise<EnumFieldValue[]> {
    try {
      ...
      // 映射数据到标准格式
      const mappedData = dataArray.map((option: Record<string, unknown>, index: number) => ({
  ...
  async getEnumFieldUsageStats(typeId: string): Promise<DictionaryUsageStats> {
    try {
      ...
      return {
        ...
        usage_by_field:
          data.usage_by_field !== null && data.usage_by_field !== undefined
            ? data.usage_by_field
            : {},
