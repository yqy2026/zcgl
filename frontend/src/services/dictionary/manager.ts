...
class DictionaryManagerService {
    ...
    try {
      ...
      if (Array.isArray(data) && data.length > 0 && typeof data[0] === 'string') {
        ...
        const enumTypes: EnumFieldType[] = (data as unknown[]).map((item: unknown, index: number): EnumFieldType => {
          ...
          return {
            id: `enum-type-${index}`,
            name: (config?.name !== null && config?.name !== undefined && config?.name !== '') ? config.name : typeCode.replace(/_/g, ' ').replace(/\b\w/g, l => l.toUpperCase()),
            code: typeCode,
            category: (config?.category !== null && config?.category !== undefined && config?.category !== '') ? config.category : '系统字典',
            description: (config?.description !== null && config?.description !== undefined && config?.description !== '') ? config.description : `${typeCode} 枚举类型`,
            is_system: true,
            is_multiple: false,
  ...
  async getEnumFieldValues(typeId: string): Promise<EnumFieldValue[]> {
    try {
      ...
      const mappedData = dataArray.map((option: Record<string, any>, index: number) => ({
        ...
        is_default: (option.is_default !== null && option.is_default !== undefined) ? (option.is_default as boolean) : index === 0,
      ...
      if (config) {
        return config.fallbackOptions.map((option, index) => {
          ...
          return {
            ...
            description: '',
            level: 1,
            sort_order: (option.sort_order !== null && option.sort_order !== undefined) ? option.sort_order : index + 1,
            color: option.color,
            icon: option.icon,
            is_active: true,
            is_default: index === 0,
  ...
  async getEnumFieldUsageStats(typeId: string): Promise<DictionaryUsageStats> {
    try {
      ...
      // 确保返回完整的统计信息
      return {
        total_records: (data.total_records !== null && data.total_records !== undefined) ? data.total_records : 0,
        active_records: (data.active_records !== null && data.active_records !== undefined) ? data.active_records : 0,
        usage_by_field: (data.usage_by_field !== null && data.usage_by_field !== undefined) ? data.usage_by_field : {},
        last_updated: new Date().toISOString(),
        popular_values: (data.popular_values !== null && data.popular_values !== undefined) ? data.popular_values : []
      };
    } catch (error) {
      const enhancedError = ApiErrorHandler.handleError(error);
