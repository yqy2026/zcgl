...
const LazyWrapper: React.FC<{ children: React.ReactNode }> = ({ children }) => (
  <Suspense fallback={<SkeletonLoader type="detail" rows={5} />}>{children}</Suspense>
);
...
export const useRoutePreload = () => {
  const preloadRoute = (routeName: keyof typeof preloadRoutes) => {
    preloadRoutes[routeName]().catch((error: unknown) =>
      routeLogger.error(`Failed to preload route ${routeName}:`, error as Error)
    );
  };
