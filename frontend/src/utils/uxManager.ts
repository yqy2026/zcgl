...
class UXManager {
  ...
  private setupErrorHandling() {
    ...
    window.addEventListener('unhandledrejection', event => {
      const reason = event.reason as string | Error | { message?: string };
      const message =
        typeof reason === 'string' ? reason : (reason?.message ?? 'Unknown promise rejection');
  ...
  public handleError(error: Error, context?: ErrorContext) {
    ...
    // 发送错误报告
    if (this.config.errorReportingEndpoint != null) {
      void this.sendErrorReport(error, context);
    }
  }
  ...
  public endPerformanceMeasure(name: string) {
    ...
    if (entries.length > 0) {
      this.recordPerformanceMetric(name, entries[0].duration);
    }
  }
...
export const startMeasure = (name: string) => uxManager.startPerformanceMeasure(name);

export const endMeasure = (name: string) => uxManager.endPerformanceMeasure(name);

export const setLoading = (key: string, loading: boolean) => uxManager.setLoading(key, loading);

export const isLoading = (key: string) => uxManager.isLoading(key);
