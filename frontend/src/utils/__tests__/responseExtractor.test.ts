...
describe('ResponseExtractor - 便捷方法', () => {
  ...
  describe('extractAndTransform', () => {
    it('应该提取并转换数据', () => {
      ...
      const result = ResponseExtractor.extractAndTransform(standardResponse, transformer);
    ...
    it('应该在原始提取失败时返回错误', () => {
      ...
      const result = ResponseExtractor.extractAndTransform(errorResponse, transformer);
