...
class Logger {
  ...
  private formatMessage(
    ...
    if (
      this.config.useTimestamp !== null &&
      this.config.useTimestamp !== undefined &&
      this.config.useTimestamp === true
    ) {
    ...
    if (meta !== null && meta !== undefined && Object.keys(meta).length > 0) {
      parts.push(meta);
    }
  ...
  child(prefix: string): Logger {
    return new Logger({
      ...this.config,
      prefix:
        this.config.prefix !== null && this.config.prefix !== undefined && this.config.prefix !== ''
