...
export const calculateOccupancyRate = (asset: Partial<Asset>): number | undefined => {
  ...
  return DecimalUtils.safeDivide(
    DecimalUtils.safeMultiply(rentedArea, 100),
    rentableArea
  )
}
 ...
 */
export const validateAreaData = (asset: Partial<Asset>): {
  isValid: boolean
  errors: string[]
} => {
  const errors: string[] = []
...
export const getAssetSummary = (assets: Partial<Asset>[]) => {
  ...
  assets.forEach(asset => {
    ...
    if (usageStatus === '出租' || usageStatus === '转租') {
      rentedAssets++
    } else if (usageStatus === '空置' || usageStatus === '空置规划') {
      vacantAssets++
    }
  })

  const totalUnrentedArea = DecimalUtils.safeSubtract(totalRentableArea, totalRentedArea)
  const overallOccupancyRate = totalRentableArea > 0
    ? DecimalUtils.safeDivide(
        DecimalUtils.safeMultiply(totalRentedArea, 100),
        totalRentableArea
      )
    : 0
  ...
  return {
    ...
    summary: {
      ...
      utilizationRate: totalRentableArea + totalNonCommercialArea > 0
        ? ((totalRentedArea + totalNonCommercialArea) / (totalRentableArea + totalNonCommercialArea)) * 100
        : 0
    }
  }
}
 ...
 * @returns 完整性检查结果
 */
export const checkAssetDataCompleteness = (asset: Partial<Asset>): {
  completeness: number
  ...
  const completeness = totalFields > 0
    ? ((totalFields - allMissingFields.length) / totalFields) * 100
    : 100
