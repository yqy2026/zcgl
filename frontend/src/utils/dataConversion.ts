...
// 需要Decimal转换的字段列表
const DECIMAL_FIELDS: AssetFieldKey[] = [
  // 面积字段
  'land_area',
  'actual_property_area',
  'rentable_area',
    ...
    for (const key in data) {
      if (Object.prototype.hasOwnProperty.call(data, key)) {
        ...
        if (value === null || value === undefined) {
          result[key] = value;
        } else if (DECIMAL_FIELDS.includes(key as AssetFieldKey) && typeof value === 'string') {
          // 处理Decimal字段
  ...
  if (isObject(data)) {
    ...
    for (const key in data) {
      if (Object.prototype.hasOwnProperty.call(data, key)) {
        ...
        if (value === null || value === undefined) {
          result[key] = value;
        } else if (DECIMAL_FIELDS.includes(key as AssetFieldKey) && typeof value === 'number') {
          // 处理Decimal字段
...
export const validateNumericFields = (asset: Record<string, unknown>): string[] => {
  ...
  if (
    ...
    const rArea =
      typeof rentableArea === 'number'
        ? rentableArea
        : typeof rentableArea === 'string'
          ? parseFloat(rentableArea)
          : 0;
    const rdArea =
      typeof rentedArea === 'number'
        ? rentedArea
        : typeof rentedArea === 'string'
          ? parseFloat(rentedArea)
          : 0;
