...
export class DataCache {
  ...
  constructor(config: CacheConfig = {}) {
    this.config = {
      ttl:
        config.ttl !== null && config.ttl !== undefined && config.ttl !== 0
          ? config.ttl
          : 5 * 60 * 1000, // 5 minutes default
      maxSize:
        config.maxSize !== null && config.maxSize !== undefined && config.maxSize !== 0
          ? config.maxSize
          : 100,
  ...
  set<T>(key: string, data: T, customTTL?: number): void {
    ...
    this.cache.set(fullKey, {
      ...
      timestamp: Date.now(),
      ttl:
        customTTL !== null && customTTL !== undefined && customTTL !== 0
          ? customTTL
          : this.config.ttl,
    });
  }
  ...
  has(key: string, params?: unknown): boolean {
    const fullKey = this.generateKey(key, params);
    const entry = this.cache.get(fullKey);
    return entry !== null && entry !== undefined ? !this.isExpired(entry) : false;
  }
...
export class BatchRequestOptimizer {
  ...
  async batchRequest<T>(
    ...
    const requestKey =
      batchKey !== null && batchKey !== undefined && batchKey !== '' ? batchKey : key;
    ...
    try {
      if (priority) {
        ...
        if (typeof requestIdleCallback !== 'undefined') {
          requestIdleCallback(() => {
            void Promise.all(this.preloadQueue.map(fn => fn()));
          });
        } else {
          // 降级方案
          setTimeout(() => {
...
export const createCachedRequest = <T>(
  ...
  return async (params?: unknown): Promise<T> => {
    ...
    // 执行请求
    const result = await batchOptimizer.batchRequest(cacheKey, requestFn, {
      batchKey:
