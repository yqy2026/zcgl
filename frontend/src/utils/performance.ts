...
class PreloadManager {
  ...
  private schedulePreload<T>(importFn: () => Promise<T>, priority: 'high' | 'low'): Promise<T> {
    ...
    return new Promise((resolve, reject) => {
      ...
      if (window !== null && window !== undefined && 'requestIdleCallback' in window) {
...
class PerformanceMonitor {
  ...
  private observeWebVitals() {
    ...
    this.observeMetric('layout-shift', entry => {
      const entryRecord = entry as unknown as Record<string, unknown>;
      if (
        entryRecord.hadRecentInput === null ||
        entryRecord.hadRecentInput === undefined ||
        entryRecord.hadRecentInput === false
      ) {
  ...
  private observeMetric(entryType: string, callback: (entry: PerformanceEntry) => void) {
    if (window === null || window === undefined || !('PerformanceObserver' in window)) {
      return;
    }
