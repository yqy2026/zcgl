...
export const getAssetsHandler = http.get(`${API_BASE_URL}/assets`, async ({ request }) => {
  ...
  if (params.search) {
    return HttpResponse.json({
      ...assetListResponse,
      data: assetListResponse.data.filter((asset: Record<string, unknown>) =>
        (asset.propertyName as string).includes(params.search)
      ),
    });
  }
...
export const createAssetHandler = http.post(`${API_BASE_URL}/assets`, async ({ request }) => {
  ...
  const propertyName = body.propertyName as string | undefined;
...
export const loginHandler = http.post(`${API_BASE_URL}/auth/login`, async ({ request }) => {
  ...
  const body = (await request.json()) as { username: string | null; password: string | null };
 ...
 * 401 未授权错误
 */
export const unauthorizedHandler = http.get(`${API_BASE_URL}/error/unauthorized`, () => {
 ...
 * 403 权限不足错误
 */
export const forbiddenHandler = http.get(`${API_BASE_URL}/error/forbidden`, () => {
 ...
 * 404 资源不存在错误
 */
export const notFoundHandler = http.get(`${API_BASE_URL}/error/notfound`, () => {
  return HttpResponse.json(notFoundResponse, { status: 404 });
});
 ...
 * 500 服务器错误
 */
export const serverErrorHandler = http.get(`${API_BASE_URL}/error/server`, () => {
  return HttpResponse.json(serverErrorResponse, { status: 500 });
});
