...
const OccupancyRateChart: React.FC<OccupancyRateChartProps> = ({ filters, height = 400 }) => {
  ...
  const trendChartConfig = {
    ...
    tooltip: {
      formatter: (datum: Record<string, unknown>) => ({
        name: '出租率',
        value: `${typeof datum.rate === 'number' ? datum.rate.toFixed(2) : 'N/A'}%`,
      }),
      customContent: (_title: unknown, dataList: Array<{ data: Record<string, unknown> }>) => {
        ...
        return (
          <div style={{ padding: '8px' }}>
            <div style={{ fontWeight: 'bold', marginBottom: '4px' }}>{datum.month as string}</div>
            <div>出租率: {(datum.rate as number).toFixed(2)}%</div>
            <div>总面积: {(datum.total_area as number)?.toLocaleString()} ㎡</div>
            <div>已租面积: {(datum.rented_area as number)?.toLocaleString()} ㎡</div>
          </div>
        );
      },
    },
  ...
  const propertyNatureChartConfig = {
    ...
    tooltip: {
      formatter: (datum: Record<string, unknown>) => ({
        ...
        value: `${typeof datum.value === 'number' ? datum.value.toFixed(2) : 'N/A'}%`,
      }),
      customContent: (_title: unknown, data: Array<{ data: Record<string, unknown> }>) => {
        ...
        return (
          <div style={{ padding: '8px' }}>
            ...
            <div>出租率: {(datum.value as number).toFixed(2)}%</div>
            <div>总面积: {(datum.total_area as number)?.toLocaleString()} ㎡</div>
            <div>已租面积: {(datum.rented_area as number)?.toLocaleString()} ㎡</div>
          </div>
        );
  ...
  const ownershipChartConfig = {
    data:
      data?.by_ownership_entity?.map(item => ({
        ownership:
    ...
    label: {
      ...
      formatter: (datum: Record<string, unknown>) =>
        `${typeof datum.rate === 'number' ? datum.rate.toFixed(1) : 'N/A'}%`,
      style: {
        fill: '#333',
        fontSize: 12,
      },
    ...
    tooltip: {
      formatter: (datum: Record<string, unknown>) => {
        const nameValue = datum.full_name ?? datum.ownership ?? '';
        let nameStr: string;
        if (typeof nameValue === 'string') {
          ...
          nameStr = JSON.stringify(nameValue);
        }
        return {
          name: nameStr,
          value: `${typeof datum.rate === 'number' ? datum.rate.toFixed(2) : 'N/A'}%`,
        ...
        return (
          <div style={{ padding: '8px' }}>
            <div style={{ fontWeight: 'bold', marginBottom: '4px' }}>
              {datum.full_name !== null && datum.full_name !== undefined
                ? datum.full_name
                : datum.ownership}
            </div>
            <div>出租率: {rate.toFixed(2)}%</div>
            ...
            <Statistic
              ...
              valueStyle={{
                color:
                  ...
                  data?.overall_rate !== undefined &&
                  data.overall_rate >= 80
                    ? '#52c41a'
                    : data?.overall_rate !== null &&
                        data?.overall_rate !== undefined &&
                        data.overall_rate >= 60
                      ? '#faad14'
                      : '#ff4d4f',
              }}
            />
