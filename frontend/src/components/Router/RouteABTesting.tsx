...
class ABTestManager {
  ...
  private getAssignedVariant(testId: string): ABTestVariant | null {
    ...
    if (test === null || test === undefined || this.userId === null || this.userId === undefined) {
      return null;
    }
  ...
  public trackConversion(testId: string, metric: string, value?: unknown) {
    ...
    // 上报转化数据
    void this.reportConversion(testId, metric, value);
  }
  ...
  private trackEvent(eventType: string, data: Record<string, unknown>) {
    ...
    if (process.env.NODE_ENV === 'production') {
      // 实际项目中发送到分析服务
      void fetch('/api/analytics/abtest-events', {
        method: 'POST',
        headers: {
  ...
  public getTestResults(testId: string): Record<string, unknown> {
    const conversions = this.conversions.get(testId) || new Map();
    return {
      conversions: Object.fromEntries(conversions) as Record<string, unknown>,
      testId,
      userId: this.userId,
    };
  }
