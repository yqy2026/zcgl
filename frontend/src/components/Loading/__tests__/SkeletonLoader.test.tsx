...
describe('SkeletonLoader - 边界情况测试', () => {
  ...
  it('应该处理null children', async () => {
    const SkeletonLoader = (await import('../SkeletonLoader')).default;
    const element = React.createElement(SkeletonLoader, { loading: false }, null);
    expect(element).toBeTruthy();
  });
...
describe('SkeletonLoader - 组合属性测试', () => {
  ...
  it('应该支持所有属性组合 - list', async () => {
    const SkeletonLoader = (await import('../SkeletonLoader')).default;
    const element = React.createElement(SkeletonLoader, { type: 'list', rows: 5, loading: true });
    expect(element).toBeTruthy();
  });

  it('应该支持所有属性组合 - table', async () => {
    const SkeletonLoader = (await import('../SkeletonLoader')).default;
    const element = React.createElement(SkeletonLoader, { type: 'table', rows: 10, loading: true });
    expect(element).toBeTruthy();
  });

  it('应该支持所有属性组合 - form', async () => {
    const SkeletonLoader = (await import('../SkeletonLoader')).default;
    const element = React.createElement(SkeletonLoader, { type: 'form', rows: 8, loading: true });
    expect(element).toBeTruthy();
  });
});
