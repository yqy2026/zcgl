...
export const PropertyCertificateReview: React.FC<PropertyCertificateReviewProps> = ({
  ...
  const confidenceLevel = confidence_score > 0.8 ? 'success' : confidence_score > 0.5 ? 'warning' : 'error';
  ...
  return (
    <Space direction="vertical" size="large" style={{ width: '100%' }}>
      {/* Confidence Score */}
      <Card title="提取结果" extra={<Tag color={confidenceLevel}>置信度: {(confidence_score * 100).toFixed(1)}%</Tag>}>
        <Collapse
          items={[
            {
              key: 'errors',
              label: `验证错误 (${validation_errors.length})`,
              children: validation_errors.length > 0 ? (
                <List
      ...
      <Card title="确认并编辑信息">
        <Form
          ...
          initialValues={{
            ...extracted_data,
            registration_date: extracted_data.registration_date ? dayjs(extracted_data.registration_date as string) : null,
            land_use_term_start: extracted_data.land_use_term_start ? dayjs(extracted_data.land_use_term_start as string) : null,
            land_use_term_end: extracted_data.land_use_term_end ? dayjs(extracted_data.land_use_term_end as string) : null
          }}
          onFinish={handleConfirm}
        >
          <Form.Item label="证书编号" name="certificate_number" rules={[{ required: true, message: '请输入证书编号' }]}>
            <Input />
          </Form.Item>

          <Form.Item label="坐落地址" name="property_address" rules={[{ required: true, message: '请输入坐落地址' }]}>
            <Input />
          </Form.Item>
