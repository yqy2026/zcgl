...
const getColumns = (
...
): ColumnsType<Asset> => [
  {
    ...
    render: (text: string, record: Record<string, unknown>) => {
      ...
      const projectName: string = (record.project_name as string | undefined) ?? text ?? '';
  ...
  },
  {
    ...
    render: (text: unknown, _record: Asset) => {
      // 权属类别映射
      let displayText: string = text as string;
  ...
  },
  {
    ...
    render: (rate: unknown, record: unknown) => {
      ...
      // 计算出租率
      if (rec.rentable_area != null && rec.rented_area != null) {
