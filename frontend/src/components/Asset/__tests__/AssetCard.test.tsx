...
vi.mock('antd', () => ({
  ...
  Statistic: ({ title, value, suffix, precision, valueStyle }: any) => (
    <div data-testid="statistic" data-title={title}>
      <div data-value={value} data-precision={precision} data-value-style={JSON.stringify(valueStyle)}>
        {value}
        {suffix}
      </div>
  ...
  Progress: ({ percent, strokeColor, size, showInfo }: any) => (
    <div data-testid="progress" data-percent={percent} data-stroke-color={strokeColor} data-size={size} data-show-info={showInfo}>
      {percent}%
    </div>
  ),
}))
...
describe('AssetCard - 边界情况测试', () => {
  ...
  it('应该处理0值area', async () => {
    const { default: AssetCard } = await import('../AssetCard')
    const asset = { land_area: 0, actual_property_area: 0, rentable_area: 0, rented_area: 0 } as any
    const element = React.createElement(AssetCard, {
      asset,
      onEdit: vi.fn(),
      onDelete: vi.fn(),
