...
jest.mock("antd", () => ({
  ...
  Select: ({ children, onChange }: any) => (
    <select onChange={(e) => onChange?.(e.target.value)} data-testid="select">
      {children}
    </select>
  ),
...
describe("AssetForm Basic Jest Tests", () => {
  ...
  test("组件应该能够渲染", () => {
    render(
      <TestWrapper>
        <AssetForm onSubmit={mockSubmit} onCancel={mockCancel} />
      </TestWrapper>,
    );
  ...
  test("应该能够处理编辑模式", () => {
    ...
    render(
      <TestWrapper>
        <AssetForm initialData={mockAsset} mode="edit" />
      </TestWrapper>,
    );
