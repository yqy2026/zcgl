...
vi.mock('antd', () => ({
  ...
  Input: {
    Search: ({ children: _children, value, onChange, placeholder, onSearch }: any) => (
      <input
  ...
  Modal: {
    confirm: ({ onOk: _onOk, onCancel: _onCancel }: any) => ({
      then: (callback: any) => callback && callback({ result: 'ok' }),
    }),
  },
  Option: ({ _children, value }: any) => <option value={value}>{value}</option>,
