...
describe('EnhancedPDFImportUploader 组件测试', () => {
  ...
  describe('成功状态显示', () => {
    ...
    it('上传成功应该调用 onUploadSuccess', async () => {
      const handleSuccess = vi.fn()
      const element = await createElement({ uploadStatus: 'success', onUploadSuccess: handleSuccess })
      expect(element).toBeTruthy()
    })
  ...
  describe('重试上传', () => {
    ...
    it('重试应该使用原文件', async () => {
      const element = await createElement({ uploadStatus: 'error', file: new File([''], 'test.pdf') })
      expect(element).toBeTruthy()
    })
  })
