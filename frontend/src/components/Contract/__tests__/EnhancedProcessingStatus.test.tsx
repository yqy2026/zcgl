  ...
  List: ({ dataSource, renderItem }: any) => (
    <div data-testid="list" data-count={dataSource?.length ?? 0}>
      {dataSource?.map((item: any, index: number) => (
        <div key={index} data-list-item={index}>
...
describe('EnhancedProcessingStatus 组件测试', () => {
  ...
  describe('错误处理', () => {
    it('OCR失败应该显示错误信息', async () => {
      const element = await createElement({
        progressData: { error: { step: 'ocr', message: 'OCR失败' } },
      });
      expect(element).toBeTruthy();
    });
    ...
    it('验证失败应该显示错误信息', async () => {
      const element = await createElement({
        progressData: { error: { step: 'validating', message: '文件无效' } },
      });
      expect(element).toBeTruthy();
    });
    ...
    it('超时应该显示错误信息', async () => {
      const element = await createElement({
        progressData: { error: { step: 'timeout', message: '处理超时' } },
      });
      expect(element).toBeTruthy();
    });
