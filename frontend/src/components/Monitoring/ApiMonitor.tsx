...
const ApiMonitor: React.FC = () => {
  ...
  const columns = [
    ...
    },
    {
      ...
      width: 120,
      render: (time?: number) => (
        <span style={{ color: (time !== null && time !== undefined && time > 3000) ? '#ff4d4f' : (time !== null && time !== undefined && time > 1000) ? '#fa8c16' : '#52c41a' }}>
    ...
    },
    {
      ...
      key: 'error',
      ellipsis: true,
      render: (error?: string) => (error !== null && error !== undefined && error !== '') ? <span style={{ color: '#ff4d4f' }}>{error}</span> : '-'
    },
    {
      title: '最后检查',
