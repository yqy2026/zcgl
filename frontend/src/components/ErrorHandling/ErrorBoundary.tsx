...
class ErrorBoundaryComponent extends Component<ErrorBoundaryProps, ErrorBoundaryState> {
  ...
  private reportError = (error: Error, errorInfo: ErrorInfo) => {
    ...
    if (process.env.NODE_ENV === 'development') {
      (window as Record<string, unknown>).__lastError = errorReport;
    }
  ...
  private sendErrorReport = async (errorReport: ErrorReport) => {
    try {
      if (process.env.NODE_ENV === 'production') {
        ...
        // 开发环境: 记录错误 (不显示在控制台，生产环境已上报)
        // 错误信息已包含在 errorReport 对象中
      }
    } catch (reportingError) {
      logger.warn('错误报告发送失败', {
    ...
    >
      ...
      {showErrorDetails && error && (
        ...
        >
          ...
          {error.stack !== null && error.stack !== undefined && (
          ...
          {errorInfo?.componentStack !== null && errorInfo?.componentStack !== undefined && (
            <details style={{ marginTop: '10px' }}>
...
export const useErrorHandler = () => {
  ...
  const captureError = React.useCallback((error: Error) => {
    ...
    // 存储错误到 window 对象用于调试
    if (process.env.NODE_ENV === 'development') {
      (window as Record<string, unknown>).__lastError = {
...
export const useRouterErrorBoundary = () => {
  ...
  const handleError = useCallback(
    (error: Error, fallbackPath: string = '/dashboard') => {
