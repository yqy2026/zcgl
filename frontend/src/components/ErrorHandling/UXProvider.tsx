...
const UXProvider: React.FC<UXProviderProps> = ({ children, config = {} }) => {
  // 初始化UX管理器
  useEffect(() => {
    if (config.ux) {
    ...
    const handleError = (error: Error, errorInfo?: Record<string, unknown>) => {
      ...
      if (config.errorBoundary?.onError) {
        config.errorBoundary.onError(error, errorInfo as React.ErrorInfo);
      }
    };
    ...
    const handleUnhandledRejection = (event: PromiseRejectionEvent) => {
      const errorMessage =
        typeof event.reason === 'string' ? event.reason : String(event.reason ?? 'Unknown error');
      handleError(new Error(errorMessage), { type: 'unhandledrejection' });
    };
