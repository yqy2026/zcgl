...
vi.mock('../StatisticCard', () => ({
  FinancialStatisticCard: ({ title, value, precision, suffix, isPositive, loading }: any) => (
    <div data-testid="financial-statistic-card" data-title={title} data-value={value} data-precision={precision} data-suffix={suffix} data-is-positive={isPositive} data-loading={loading}>
      {title}: {value}
    </div>
  ),
}))
...
vi.mock('antd', () => ({
  ...
  Progress: ({ percent, size, strokeColor, style }: any) => (
    <div data-testid="progress" data-percent={percent} data-size={size} data-stroke-color={JSON.stringify(strokeColor)} style={style}>
      {percent}%
    </div>
  ),
