...
export const AnalyticsFiltersProvider: React.FC<AnalyticsFiltersProviderProps> = ({
    ...
    const handleDateRangeChange = useCallback((_dates: unknown, dateStrings: [string, string]) => {
        const newFilters = {
            ...localFilters,
    ...
    const activeFiltersCount = (Object.keys(localFilters) as (keyof AssetSearchParams)[]).filter(key => {
        const val = localFilters[key];
        return val !== undefined && val !== null && val !== '';
    }).length;
