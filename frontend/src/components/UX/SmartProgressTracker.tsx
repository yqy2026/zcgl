...
export const SmartProgressProvider: React.FC<SmartProgressProviderProps> = ({
  ...
  const createTracker = useCallback(
    (tracker: Omit<ProgressTracker, 'id' | 'startTime'>) => {
  ...
  const updateStep = useCallback(
    (trackerId: string, stepId: string, updates: Partial<ProgressStep>) => {
      setTrackers(prev =>
        prev.map(tracker => {
      ...
      prev.map(tracker => {
        if (tracker.id === trackerId) {
          if (updates.status === 'completed') {
            // 清除定时器
            if (
              intervals.current[trackerId] !== null &&
              intervals.current[trackerId] !== undefined
            ) {
  ...
  const cancelTracker = useCallback(
    (trackerId: string) => {
  ...
  const calculateTotalProgress = (steps: ProgressStep[]): number => {
    ...
    const completedWeight = steps
      ...
      .reduce((sum, step) => {
        const weight = step.progress !== null && step.progress !== undefined ? step.progress : 100;
        return sum + weight;
      }, 0);
  ...
  const calculateEstimatedEndTime = (tracker: ProgressTracker): Date => {
    if (tracker.startTime === null || tracker.startTime === undefined) {
      return new Date();
    }
...
const ProgressTrackersList: React.FC<ProgressTrackersListProps> = ({ trackers }) => {
  ...
  const getStepIcon = (step: ProgressStep) => {
    if (step.icon !== null && step.icon !== undefined) {
      return step.icon;
    }
  ...
  const formatDuration = (duration: number): string => {
    ...
    if (duration !== null && duration !== undefined && duration < 60000) {
      return `${Math.round(duration / 1000)}s`;
    }
    return `${Math.round((duration ?? 0) / 60000)}m ${Math.round(((duration ?? 0) % 60000) / 1000)}s`;
  };
            ...
            <Steps
              ...
              items={tracker.steps.map(step => ({
                title: (
                  <div style={{ display: 'flex', alignItems: 'center' }}>
                    {getStepIcon(step)}
                    <span style={{ marginLeft: '8px', fontSize: '12px' }}>{step.title}</span>
                ...
                status: step.status === 'paused' ? 'wait' : step.status,
              }))}
            />

            {tracker.error !== null && tracker.error !== undefined && (
              <div
                style={{
                  marginTop: '8px',
                  padding: '8px',
            ...
            >
              ...
              {tracker.status === 'error' && (
                <Button
                  ...
                  type="primary"
                  onClick={() => {
                    if (tracker.onRetry !== null && tracker.onRetry !== undefined) {
                      tracker.onRetry();
                    }
                  }}
                >
