...
const SmartFormOptimizer: React.FC<SmartFormOptimizerProps> = ({
  ...
  const renderField = (field: FieldConfig) => {
    ...
    switch (field.type) {
      case 'number':
        return (
          <InputNumber
            ...
            parser={value =>
              (value?.replace(new RegExp(`${field.suffix ?? ''}$`), '') as string) ?? ''
            }
          />
        );
      ...
      case 'select':
        return (
          <Select {...commonProps} style={{ width: '100%' }} options={field.options} allowClear />
        );

      case 'textarea':
        return <Input.TextArea {...commonProps} rows={4} maxLength={field.maxLength} showCount />;
    ...
    <Card style={style} size={size}>
      ...
      {Object.keys(errors).length > 0 && (
        <div style={{ marginTop: 16 }}>
          {errors.general && (
            <div style={{ color: '#ff4d4f', marginBottom: 8 }}>{errors.general}</div>
          )}
          {Object.keys(errors)
