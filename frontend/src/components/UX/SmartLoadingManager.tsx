...
export const SmartLoadingManager: React.FC<SmartLoadingManagerProps> = ({
  ...
  const showLoading = useCallback(
    (state: LoadingState) => {
      ...
      const timestamp =
        state.timestamp !== null && state.timestamp !== undefined ? state.timestamp : Date.now();
          ...
          >
            ...
            <div style={{ marginTop: 16, marginLeft: 16 }}>
              <Text style={{ color: 'white', fontSize: 16 }}>
                {state.message !== null && state.message !== undefined && state.message !== ''
                  ? state.message
                  : '加载中...'}
              </Text>
            </div>
...
export const SmartPreloader: React.FC<PreloaderProps> = ({
  ...
  React.useEffect(() => {
    if (resource && !isPreloaded && !isPreloading) {
      ...
      link.onload = () => {
        ...
        if (onPreloadComplete !== null && onPreloadComplete !== undefined) {
          onPreloadComplete();
        }
        if (
          document.head !== null &&
  ...
  }, [resource, isPreloaded, isPreloading, onPreloadComplete]);

  if (fallback !== null && fallback !== undefined && isPreloaded === false) {
    return <>{fallback}</>;
  }
