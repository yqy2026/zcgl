...
export const useErrorHandler = (options: UseErrorHandlerOptions = {}) => {
  const { showNotification = true, redirectOnError = false, logErrors = true } = options;
  ...
  const handleApiError = useCallback(
    (error: ApiError) => {
      ...
      if (error?.response) {
        ...
        (error as Record<string, unknown>).request !== undefined
      ) {
        // 网络错误
        errorInfo = {
          code: 'NETWORK_ERROR',
  ...
  const handleBusinessError = useCallback(
    (message: string, code?: string) => {
      const errorInfo: ErrorInfo = {
        code: (code ?? null) !== null ? code : 'BUSINESS_ERROR',
        message,
        timestamp: new Date().toISOString(),
      };
