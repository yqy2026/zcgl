...
export const useUserExperience = (options: UseUserExperienceOptions = {}) => {
  ...
  useEffect(() => {
    ...
    const handleOnline = () => {
      setIsOnline(true);
      void SuccessNotification.notify({
        type: 'success',
        title: '网络已连接',
  ...
  const endPerformanceTimer = useCallback(
    (key: string, threshold: number = 1000) => {
      if (!enablePerformanceMonitoring || !performanceRef.current[key]) {
        return;
      }
  ...
  const withLoadingState = useCallback(
    async <T>(operation: () => Promise<T>, loadingText: string = '处理中...'): Promise<T> => {
