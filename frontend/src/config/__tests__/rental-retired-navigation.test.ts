...
describe('rental retired navigation config', () => {
  it('collapses rental menu to a single retired entry and keeps rental paths selected on that entry', () => {
    ...
    expect(getSelectedKeys('/rental/contracts/pdf-import')).toEqual(['/contract-groups']);
  ...
  it('adds a dedicated contract group navigation entry without removing retired rental routing', () => {
    const contractGroupEntry = (MENU_ITEMS ?? []).find(item => item?.key === '/contract-groups');

    expect(contractGroupEntry?.label).toBe('合同组管理');
