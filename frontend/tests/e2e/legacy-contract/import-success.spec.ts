...
import {
  ...
  buildLegacyContractWorkbookFilename,
...
test.describe('@legacy-contract-import-success 导入成功路径', () => {
  ...
  test('excel import should return structured outcome for minimal workbook', async ({ page }) => {
    ...
    const importResponsePromise = page.waitForResponse(response => {
      return (
        response.request().method() === 'POST' && response.url().includes(LEGACY_CONTRACT_EXCEL_IMPORT_API)
      );
    });

    await uploadInput.setInputFiles({
      name: buildLegacyContractWorkbookFilename(contractNumber),
      mimeType: XLSX_MIME,
