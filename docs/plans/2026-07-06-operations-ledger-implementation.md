...
- [x] `GET /api/v1/ledger/service-fees` 查询月度服务费台账和来源租金台账。
...
- [x] 服务费结算视图支持生成服务费、登记服务费收款和查看来源账期。
- [x] 项目详情四组摘要可跳转到经营台账过滤视图。
- [x] 合同/协议号、资产、客户在项目详情、合同详情和经营台账之间互相跳转。

说明：服务费结算视图通过 `GET /api/v1/ledger/service-fees` 查询服务费台账，展示服务费账期、来源租金台账 ID、计算基数、费率、应收/实收和状态，并支持从行内登记服务费收款。
...
- [x] 综合分析返回终端租户收缴、运营方收入、运营方成本、经营结果四组指标。
- [x] 项目分析按租金账期归属，流水发生日期只用于查询/导出字段。
- [x] 导出文件标记统计口径版本和账期归属口径。
- [x] 通知扫描只对终端租户租金生成到期/逾期提醒。
- [x] 合同/协议到期提醒文案收口，不暴露内部聚合对象。
