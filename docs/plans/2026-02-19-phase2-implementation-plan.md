...
#### [MODIFY] [query_builder.py](file:///d:/ccode/zcgl/backend/src/crud/query_builder.py) — 在现有实现上替换 `TenantFilter` 为 `PartyFilter`
...
| 文件 | 变更 |
|---|---|
| [base.py](file:///d:/ccode/zcgl/backend/src/crud/base.py) | `TenantFilter` → `PartyFilter`，`_apply_tenant_filter` → `_apply_party_filter` |
| [asset.py](file:///d:/ccode/zcgl/backend/src/crud/asset.py) | 同上 + `organization_id` 列 → `owner_party_id`/`manager_party_id` |
| [project.py](file:///d:/ccode/zcgl/backend/src/crud/project.py) | 同上 + `organization_id` → `manager_party_id` |
| [property_certificate.py](file:///d:/ccode/zcgl/backend/src/crud/property_certificate.py) | 同上 |
| [rbac.py](file:///d:/ccode/zcgl/backend/src/crud/rbac.py) | 同上 + `organization_id` → `party_id` |
| [ownership.py](file:///d:/ccode/zcgl/backend/src/crud/ownership.py) | 标记 deprecated |
| [custom_field.py](file:///d:/ccode/zcgl/backend/src/crud/custom_field.py) | `TenantFilter` → `PartyFilter` |
| [pdf_import_session.py](file:///d:/ccode/zcgl/backend/src/crud/pdf_import_session.py) | 同上 |
| [system_dictionary.py](file:///d:/ccode/zcgl/backend/src/crud/system_dictionary.py) | 同上 |
| [rent_contract.py](file:///d:/ccode/zcgl/backend/src/crud/rent_contract.py) | `ownership_id`/`ownership_ids` → `owner_party_id`/`owner_party_ids`（合同 + 台账统计链路） |
| [collection.py](file:///d:/ccode/zcgl/backend/src/crud/collection.py) | 台账收款聚合链路使用 `owner_party_id` |
| [field_whitelist.py](file:///d:/ccode/zcgl/backend/src/crud/field_whitelist.py) | 逐类字段替换清单见下（`OrganizationWhitelist` 删除，`OwnershipWhitelist` 标记 deprecated） |

**Service/API 层（10+ 文件）**：

| 文件 | 变更 |
|---|---|
| [asset_service.py](file:///d:/ccode/zcgl/backend/src/services/asset/asset_service.py) | `TenantFilter` → `PartyFilter` |
| [service.py](file:///d:/ccode/zcgl/backend/src/services/project/service.py) | 同上 |
| [service.py](file:///d:/ccode/zcgl/backend/src/services/property_certificate/service.py) | 同上 |
| [service.py](file:///d:/ccode/zcgl/backend/src/services/custom_field/service.py) | 同上 |
| [pdf_import_service.py](file:///d:/ccode/zcgl/backend/src/services/document/pdf_import_service.py) | 同上 |
| [rbac_service.py](file:///d:/ccode/zcgl/backend/src/services/permission/rbac_service.py) | 同上 + 权限链路改造 |
| [service.py](file:///d:/ccode/zcgl/backend/src/services/rent_contract/service.py) | `ownership_id` 查询参数/查库链路迁移为 `owner_party_id`（旧参数保留 deprecated alias） |
| [statistics_service.py](file:///d:/ccode/zcgl/backend/src/services/rent_contract/statistics_service.py) | `ownership_ids` 过滤链路迁移为 `owner_party_ids` |
| [processing_tracker.py](file:///d:/ccode/zcgl/backend/src/services/document/processing_tracker.py) | 当前无 `TenantFilter` 直接依赖（Phase 1 已预清理）；保留 `accessible_organization_ids` 参数兼容 |
| [organization.py](file:///d:/ccode/zcgl/backend/src/api/v1/auth/organization.py) | `organization_ids` 业务参数保留（不属于 TenantFilter） |
| [pdf_batch_routes.py](file:///d:/ccode/zcgl/backend/src/api/v1/documents/pdf_batch_routes.py) | `accessible_organization_ids` 保留；鉴权改走 ABAC/Party 关系 |
...
#### [MODIFY] [asset.py](file:///d:/ccode/zcgl/backend/src/models/asset.py)
...
#### [MODIFY] [rent_contract.py](file:///d:/ccode/zcgl/backend/src/models/rent_contract.py)
...
#### [MODIFY] [project.py](file:///d:/ccode/zcgl/backend/src/models/project.py)
...
#### [MODIFY] [property_certificate.py](file:///d:/ccode/zcgl/backend/src/models/property_certificate.py)

- 标记 deprecated：`organization_id` + `organization` 关系

#### [MODIFY] [rbac.py](file:///d:/ccode/zcgl/backend/src/models/rbac.py)
...
#### [MODIFY] [rbac_service.py](file:///d:/ccode/zcgl/backend/src/services/permission/rbac_service.py)
