...
- **状态**：🔄 进行中
- **创建日期**：2026-04-07
- **修订**：v3（2026-04-07），基于 `docs/archive/reviews/2026-04-07-orthogonal-restructure-blueprint-review-v2.md` 的 9 项修订
...
| 2.4 | **一资产一合同组 DB 约束**：`contract_group_assets` 表增加 `asset_id` 全局唯一约束（Alembic 迁移） | `backend/src/models/associations.py`, `backend/alembic/versions/` | 先写 migration test：重复绑定 → DB 级拒绝 |
| 2.5 | **一资产一合同组 Service 校验**：`_replace_assets()` 调用前检查资产是否已属于其他合同组，重复则拒绝 | `backend/src/crud/contract_group.py`, `backend/src/services/` | 先写 integration test：资产已绑定合同组 A → 绑定合同组 B → 拦截并返回明确错误 |
...
| 6 | **分析端点全量审计**：`analytics.py` 下 `comprehensive`/`export`/`trend`/`distribution` 四类端点统一接入 `view_mode` / `require_data_scope_context`，确保契约一致 | `backend/src/api/v1/analytics/analytics.py` | 先写 test：trend/distribution 端点带 view_mode → 按口径过滤 |
...
- **`trend`/`distribution` 端点同样接入 `view_mode` / scope context**，分析域契约一致
...
9. `analyticsService.ts` 各方法请求均携带 view_mode 参数
...
| 1b | **通过 `backfill_role_policies.py` 绑定角色-策略**：6 角色 × N 策略 → `abac_role_policies` 表 | `backend/src/scripts/migration/party_migration/backfill_role_policies.py` | 先写 test：绑定行数正确 |
...
- `abac_role_policies` 表有完整的角色-策略绑定（通过 `backfill_role_policies.py` 创建）
...
3. 日志模式 → deny 被记录但不拦截
4. 通过 `DataPolicyService` 创建 → 幂等（重复运行不重复创建）
5. system_admin → ABAC bypass
6. 无 party binding 的用户 → deny（除 system_admin/perm_admin）
7. 通过 API 修改策略后缓存正确失效
...
| A8 | 7 个 ABAC 数据策略模板未入库 | P0 | Phase 5（通过 DataPolicyService 创建） |
