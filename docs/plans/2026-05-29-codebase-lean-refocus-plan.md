...
| REQ | 状态 | 核心痛点 |
|-----|------|----------|
| REQ-PRJ-003 | 🔄 开发中 | 项目运营台账，经历 Phase 1a→2c 仍未完结 |
| REQ-RNT-001 | 🔄 开发中 | 合同关系作为用户可见业务对象，UI 交互仍在迭代 |
| REQ-RNT-002 | 🔄 开发中 | 承租/代理双模式切换，业务文案与枚举映射反复调整 |
| REQ-SCH-001 | 🔄 开发中 | 全局搜索 |
...
2. **聚焦**：资源集中到 REQ-PRJ-003 / REQ-RNT-001 / REQ-RNT-002 三个核心缺口
...
当前状态：Phase 1a→2c 已完成大部分，但备注列仍有"后续仍需补齐更完整的项目分析趋势和空置风险"。
...
**验收标准**：
- `REQ-PRJ-003` 实现状态从"开发中"变更为"已有证据"
- `docs/traceability/requirements-trace.md` 更新

#### 6.2 REQ-RNT-001/002 收尾：合同关系与双模式

当前状态：核心逻辑已完成，但 UI 交互经历了多次迭代（隐藏枚举 → 业务文案 → 四类合同入口）。

**动作**：
- 清理 `contract_group_service.py` 中 Phase 标注的临时代码
- 确认承租/代理模式切换在所有入口的行为一致性
- 前端合同关系表单中移除"临时隐藏"类注释，确认所有交互路径覆盖

**验收标准**：
- `REQ-RNT-001`、`REQ-RNT-002` 实现状态从"开发中"变更为"已有证据"
- `docs/traceability/requirements-trace.md` 更新
- 合同关系新建/编辑/查看全流程烟测通过
