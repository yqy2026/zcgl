...
【待确认】目前项目启动流程中是否有额外代码调用 `initialize()`。
...
**复核进展（2026-01-31）**：
- 复核 `pdf_import_service` 与 `error_recovery` 后未发现 `raise Exception` 直接抛出；现有路径均通过 `BaseBusinessError`/`BusinessValidationError` 或 `internal_error` 包装。【待确认】是否仍存在其它模块的直接 `raise Exception`。
