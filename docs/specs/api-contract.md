...
| 产权证管理（2026-08-10 #77 开放） | `GET/POST /api/v1/property-certificates`、`GET/PUT/DELETE /api/v1/property-certificates/{certificate_id}` | 顶层产权证 CRUD（route_registry 挂载 `assets/property_certificate.py`）：列表/详情要求 `property_certificate:read`，创建 `create`、更新 `update`、删除 `delete`；列表端点双注册（`""` 与 `"/"`）避免 307 重定向 |
