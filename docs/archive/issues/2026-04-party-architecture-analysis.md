...
**状态**: ✅ 2026-07-29 已由 REQ-PTY-003 实施完成；旧 A+C 方案仍未采纳，禁止直接实施
**分析日期**: 2026-04-05
**架构评审**: 2026-04 的 A+C 是候选设计，不是当前权威决策；当前角色事实仍从合同/资产关系派生，`customer_type` 仅作展示标签。
...
1. **体验问题仍成立**：当前主体列表仍主要按 `party_type`、状态和审核状态筛选，缺少按产权方、运营方、租户等业务关系切片。
2. **旧实现方案未获 SSOT 授权**：`business_roles` 与 `party_business_profiles` 未进入 `docs/prd.md`、`docs/specs/domain-model.md` 或代码；不得把本报告中的 JSONB 派生缓存当成已批准设计直接实施。
3. **当前权威口径**：客户角色从合同关系派生，`CustomerProfile.contract_roles` 是展示集合；`customer_type` 只是内部/外部展示标签，不参与经营指标过滤。
4. **完成结果**：REQ-PTY-003 已实现当前角色切片、服务端筛选和角色标签，ADR-0021 的实时派生决策已落地；实施记录归档至 `docs/archive/backend-plans/2026-07-29-party-business-role-slices.md`。除非后续压测给出证据，不得引入读模型或旧 JSONB 缓存。

下文保留 2026-04 的分析与候选方案作为历史输入，不代表当前实施指令。
...
经过复核 `docs/prd.md` 与 `docs/specs/domain-model.md`，当前的架构决策必须在以下边界内进行：
...
1. **直面需求 SSOT 约束**：根据 `docs/prd.md` 与 `docs/specs/domain-model.md` 中的 REQ-PTY-001、REQ-CUS-001，本系统项目不允许脱离 Party 重建一套孤立的业务侧客户/角色管理视图档案。系统强一致性地要求所有附加属性（如配置账期等）集中在关联的同一个 Party 实体上维护。
...
| 产品需求 | `docs/prd.md` (REQ-PTY-001, REQ-PTY-002) |
| 领域契约 | `docs/specs/domain-model.md` (Party, CustomerProfile) |
