...
**状态**：🔄 进行中 / 阶段 2 已完成，阶段 3 已完成组织代表主体、单用户显式 Party 范围、Party 启停、human 用户调动、单组织移动、Organization 与用户主体范围批量 preview/commit、稳定业务 403、诊断端点与缓存边界；前端阻断视图和只读范围界面仍在实施
...
- 主体详情增加只读“代表组织”反向列表；不提供双向编辑。
- 用户管理普通创建只产生停用 human 账号；通过独立影响预览完成 Organization 归属后才允许启用。账号类型只读，用户调动不能复用普通资料编辑。
