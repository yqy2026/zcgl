...
### `check-all.bat` (Windows) / `check-all-parallel.sh` (Linux/Mac)

**完整项目自动化检查脚本** - 一键验证代码质量、测试和构建状态。
...
```bash
# 添加执行权限并运行
chmod +x scripts/check-all-parallel.sh
...
- **快速模式**: 约 2-3 分钟 (仅代码质量 + 测试)
- **完整模式**: 约 8-15 分钟 (包含所有检查)
...
【阶段 3: 运行时诊断】
✓ 前端 Puppeteer 诊断          (0 errors, 0 failed requests)
...
所有检查的详细日志保存在 `logs/` 目录：
- `ruff.log` - 后端代码检查
- `ts.log` - 前端类型检查
- `backend-unit.log` - 后端单元测试
- `frontend-test.log` - 前端测试
- `diagnose.log` - 前端诊断
- `build.log` - 前端构建
...
- **前端诊断详细指南**: `frontend/docs/frontend-diagnostics.md`
- **Claude Code 命令**: `.claude/commands/README.md`
- **/check-all 命令**: `.claude/commands/check-all.md`
