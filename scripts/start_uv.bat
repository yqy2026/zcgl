...
REM Handle both cases: running from scripts/ or from project root
if exist "backend\pyproject.toml" (
    REM Already in project root
    set PROJECT_ROOT=%CD%
) else (
    REM Running from scripts/ directory, go up one level
    cd ..
    set PROJECT_ROOT=%CD%
)
...
REM 设置日志文件
set LOG_FILE=%PROJECT_ROOT%\logs\startup_%date:~0,4%%date:~5,2%%date:~8,2%_%time:~0,2%%time:~3,2%.log
...
REM 检查项目文件
if not exist "%PROJECT_ROOT%\backend\pyproject.toml" (
